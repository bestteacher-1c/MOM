...
Процедура ВывестиМакетT2(ДокументРезультат, СтрокаДанных, Уровень, ДатаОтчета, ДополнительныеСведения, СоответствиеПользовательскихПолей, РаботникКадровойСлужбыРасшифровкаПодписи, КлючВарианта, ОбъектыПечати) 
	...
	Иначе
		...
		Если НастройкиПечатныхФорм.УдалятьПрефиксыОрганизацииИИБИзНомеровКадровыхПриказов Тогда
			ЧетвертаяСтраницаТ2Продолжение2Параметры.Вставить("ПриказОбУвольненииНомер", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СтрокаДанных.РаботаПриказОбУвольненииНомер, Истина, Истина));
		Иначе
			ЧетвертаяСтраницаТ2Продолжение2Параметры.Вставить("ПриказОбУвольненииНомер", СтрокаДанных.РаботаПриказОбУвольненииНомер);
		КонецЕсли;
