...
Процедура СреднемесячныйУровеньЗарплаты(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	Если Год(НачалоПериода) <= 2017 Тогда
		ИмяМакетаСреднемесячнойЗарплаты = "ПредельныеЗначенияЗарплаты2017";
	Иначе
		ИмяМакетаСреднемесячнойЗарплаты = "ПредельныеЗначенияЗарплаты2018";
	КонецЕсли;
	МакетСреднемесячнойЗарплаты = Отчеты.ОценкаРискаНалоговойПроверки.ПолучитьМакет(ИмяМакетаСреднемесячнойЗарплаты);
