...
Процедура УстановитьНадписьПоСтатусамРегламентныхОпераций(ПриОткрытии = Ложь)
	...
	Если НЕ ВедетсяУчетПостоянныхИВременныхРазниц Тогда
		УстановитьТекстНеВедетсяУчетРазниц(НадписьДанныеНеАктуальны);
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьТекстНеВедетсяУчетРазниц(НадписьДанныеНеАктуальны)
	
	НадписьДанныеНеАктуальны.Наименование =
		НСтр("ru = 'Отчет не применяется при отсутствии учета постоянных и временных разниц'");
	НадписьДанныеНеАктуальны.Гиперссылка  = Ложь;
	НадписьДанныеНеАктуальны.Картинка     = Новый Картинка;
	НадписьДанныеНеАктуальны.Шрифт        = Новый Шрифт(НадписьДанныеНеАктуальны.Шрифт,,9);
