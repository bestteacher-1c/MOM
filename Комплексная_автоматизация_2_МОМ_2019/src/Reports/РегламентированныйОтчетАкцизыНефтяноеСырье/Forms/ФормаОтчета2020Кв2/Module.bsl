...
Процедура УстановитьКодПоМестуПоУмолчанию()
	...
	= ?(СтруктураРеквизитовФормы.Организация.КрупнейшийНалогоплательщик, "213", "214");
	...
	ИначеЕсли ИмяРаздела = "Раздел21" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если ТДО["П000210009003"].Значение = 0 Тогда
					ТДО["П000210008003"].Значение = 0;
				Иначе
					ТДО["П000210008003"].Значение = Окр(
					(ТДО["П000210009003"].Значение - 0.55 * ТДО["П000210010003"].Значение - 0.3 * ТДО["П000210011003"].Значение
					- 0.065 * ТДО["П000210012003"].Значение - ТДО["П000210013003"].Значение) / ТДО["П000210009003"].Значение, 4);
				КонецЕсли;
				
				ТДО["П000210005003"].Значение = Окр(
				((ТДО["П000210006003"].Значение * 7.3 - 182.5) * 0.3 + 29.2) * ТДО["П000210007003"].Значение
				* ТДО["П000210008003"].Значение * ТДО["П000210014003"].Значение * ТДО["П000210015003"].Значение);
				
				ТДО["П000210002003"].Значение = Окр(ТДО["П000210005003"].Значение * ТДО["П000210009003"].Значение);
	...
	ИначеЕсли ИмяРаздела = "Раздел21" Тогда
		...
		Для каждого Ветка_2 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка Из Ветка_2.Строки[0].Строки Цикл
				...
				Если Ветка.Данные["П000210009003"] = 0 Тогда
					Ветка.Данные.Вставить("П000210008003", 0);
				Иначе
					Ветка.Данные.Вставить("П000210008003", Окр(
					(Ветка.Данные["П000210009003"] - 0.55 * Ветка.Данные["П000210010003"] - 0.3 * Ветка.Данные["П000210011003"]
					- 0.065 * Ветка.Данные["П000210012003"] - Ветка.Данные["П000210013003"]) / Ветка.Данные["П000210009003"], 4));
				КонецЕсли;
				
				Ветка.Данные.Вставить("П000210005003", Окр(
				((Ветка.Данные["П000210006003"] * 7.3 - 182.5) * 0.3 + 29.2) * Ветка.Данные["П000210007003"]
				* Ветка.Данные["П000210008003"] * Ветка.Данные["П000210014003"] * Ветка.Данные["П000210015003"]));
				
				Ветка.Данные.Вставить("П000210002003", Окр(Ветка.Данные["П000210005003"] * Ветка.Данные["П000210009003"]));
