...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Если ПараметрыПечати.ИмяСтраницы = "Раздел2" Тогда
		...
		СтавкаНалога = ТабличныйДокумент.Область("П000020010503").Значение;
		
		Если Цел(СтавкаНалога * 10) = СтавкаНалога * 10 Тогда // 3 знакоместа
			
			МакетМЧБ.УдалитьОбласть(МакетМЧБ.Область("Ставка_4_2"), ТипСмещенияТабличногоДокумента.ПоВертикали);
			
		Иначе // 4 знакоместа
			
			МакетМЧБ.УдалитьОбласть(МакетМЧБ.Область("Ставка_3_1"), ТипСмещенияТабличногоДокумента.ПоВертикали);
