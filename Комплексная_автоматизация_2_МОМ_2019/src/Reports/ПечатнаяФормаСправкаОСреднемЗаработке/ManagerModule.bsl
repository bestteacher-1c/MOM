...
Процедура Сформировать(ДокументРезультат, РезультатКомпоновки, ОбъектыПечати = Неопределено) Экспорт
	...
	ПараметрыДатаСправки = Новый Структура("ПараметрыДанныхДатаСправки", ТекущаяДатаСеанса());
	...
	Для Каждого ДанныеНаПечать Из РезультатКомпоновки.ДанныеОтчета.Строки Цикл
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			ДанныеПолучаемыеНаХоду.ДанныеГрафикаПолногоРабочегоДня = СтрШаблон(НСтр("ru='%1 часовой рабочий день (смена), %2 дневная неделя (часовая неделя)'"),
			...
			ДанныеПолучаемыеНаХоду.ДанныеГрафикаСокращенногоРабочегоДня = СтрШаблон(НСтр("ru='%1 часовой рабочий день (смена), %2 дневная неделя (часовая неделя)'"),
			...
			Если Не ДанныеДетальныхЗаписей.РаботаГрафикРаботыНедельный Тогда
				
				Если СуммированныйУчетРабочегоВремениИспользуется И ДанныеДетальныхЗаписей.РаботаГрафикРаботыСуммированныйУчетРабочегоВремени Тогда
				...
				ПредставлениеГрафика = СтрШаблон(ПредставлениеГрафика, ДанныеДетальныхЗаписей.РаботаГрафикРаботыЧасовВНеделю);
				
				Если ДанныеДетальныхЗаписей.РаботаГрафикРаботыСокращенный Тогда
					ДанныеПолучаемыеНаХоду.ДанныеГрафикаСокращенногоРабочегоДня = ПредставлениеГрафика;
				Иначе
					ДанныеПолучаемыеНаХоду.ДанныеГрафикаПолногоРабочегоДня = ПредставлениеГрафика;
				КонецЕсли;
