...
Процедура СформироватьРаздел1_1()
	...
	Если СтрРазделить("229,231,235", ",", Ложь).Найти(ПоМесту) = Неопределено Тогда
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(ОКТМО) Тогда
				ОКТМО = СокрЛП(РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
...
Процедура ПроверкаКС_1_105(СтруктураРезультата)
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Арифметическая ошибка'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru='Сумма неперенесенного убытка в стр.010 Приложения 4 к Листу 02 равна стр.160 - стр.161 '") +
		НСтр("ru='Приложения 4 к Листу 02 за предыдущий налоговый период'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru='%1 = %2 - %3 Итог [%4]'"));
...
Процедура ПроверкаКС_1_01_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru='Сумма убытка или части убытка, уменьшающего налоговую базу за отчетный (налоговый) период в стр.150 '") +
		НСтр("ru='Приложения 4 к Листу 02 должна быть меньше или равна стр.140 Приложения 4 к Листу 02 / 2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru='%1 <= %2 / 2 Итог [%3]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru='Ошибочная сумма убытка, уменьшающего налоговую базу'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
