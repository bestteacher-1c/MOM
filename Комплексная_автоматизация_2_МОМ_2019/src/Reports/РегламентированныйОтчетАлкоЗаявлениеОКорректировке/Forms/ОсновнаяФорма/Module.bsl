...
Процедура ЗаполнитьСписокАлкоОтчетов()
	...
	Для каждого Элемент Из СоставАЛКО Цикл
		...
		Если Позиция = 1 Тогда
			...
			ЭтоОтчет2019года = (Число(СтрокаНомера) > 18);
			...
			СтрокаОтчет2019года = "";
			Если ЭтоОтчет2019года Тогда			
				СтрокаОтчет2019года = "(с 2019 г.) Алко ";				
			ИначеЕсли ЭтоВиноградная Тогда				
				СтрокаОтчет2019года = "(до 2019 г.) Алко ";			
			КонецЕсли; 
			
			СтрокаНомерОтчета = ?(ЭтоОтчет2019года, Строка(Число(СтрокаНомера) - 18), СтрокаНомера);
			...
			Элементы.НомерПриложения.СписокВыбора.Добавить(Число(СтрокаНомера), 
			 СтрокаОтчет2019года + "Приложение № " + СтрокаНомерОтчета);
