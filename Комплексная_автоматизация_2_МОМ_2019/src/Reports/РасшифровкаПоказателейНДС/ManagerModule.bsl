...
Функция ПолучитьПараметрыЗапроса(ПараметрыОтчета) Экспорт
	...
	Запрос.УстановитьПараметр("ТекстНеПроизведенаОплатаПоставшику", 
...
Процедура ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов)
	...
	|	И РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток < РасчетыСКлиентами.ПредоплатаРеглНачальныйОстаток
	...
	|				И ВЫРАЗИТЬ(ОбъектРасчетов КАК Справочник.ДоговорыКонтрагентов).НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ))
	|		) КАК РасчетыСКлиентами
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеРасчетныхДокументов
	|		ПО РасчетыСКлиентами.РасчетныйДокумент = ДанныеРасчетныхДокументов.Документ
	...
	ПодстановкаСтавкаНДС = УчетНДСРФ.ТекстВыбораСтавкиНДСПоУмолчанию("ДанныеРасчетныхДокументов.Дата", Истина);
...
Процедура ПолучитьНаборДанныхНачислениеНалоговыйАгент(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.Сумма) * &ПроцентНДС / 100 КАК ЧИСЛО(31,2)) КАК СуммаНДСОборот
	...
	ПодстановкаСтавкаНДС = УчетНДСРФ.ТекстВыбораСтавкиНДСПоУмолчанию("ОплатыПоставщику.Период", Истина);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СтавкаНДСПоУмолчанию", ПодстановкаСтавкаНДС);
	ПодстановкаСтавкаНДС = УчетНДСРФ.ТекстВыбораСтавкиНДСПоУмолчанию("ВложенныйЗапрос.Период", Истина);
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям(Запрос, СтруктураВнешнихНаборов) 
	...
	|			ТОГДА &ТекстНеПроизведенаОплатаПоставшику
...
Процедура ПолучитьНаборДанныхВычетАвансыВыданныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ДанныеРасчетныхДокументов.Дата
	|ИМЕЮЩИЕ
	|	СУММА(РасчетыСПоставщиками.ПредоплатаРегл) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РасчетыСПоставщиками.ДокументРегистратор КАК ДокументРегистратор,
	|	&СтавкаНДСПоУмолчанию КАК СтавкаНДСПредположительная,
	|	СУММА(ВЫРАЗИТЬ(РасчетыСПоставщиками.ПредоплатаРегл - РасчетыСПоставщиками.ПредоплатаРегл * &ПроцентНДС / (100 + &ПроцентНДС) КАК ЧИСЛО(31,2))) КАК СуммаБезНДС,
	|	СУММА(ВЫРАЗИТЬ(РасчетыСПоставщиками.ПредоплатаРегл * &ПроцентНДС / (100 + &ПроцентНДС) КАК ЧИСЛО(31,2))) КАК СуммаНДС
	|ИЗ
	...
	|	И ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И АналитикаУчетаПоПартнерам.Организация В (&Организация)
	|	И НЕ(ОбъектРасчетов ССЫЛКА Документ.ЗаказПоставщику
	|		И ВЫРАЗИТЬ(ОбъектРасчетов КАК Документ.ЗаказПоставщику).ЗакупкаПодДеятельность = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ))
	|	И НЕ(ОбъектРасчетов ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|		И ВЫРАЗИТЬ(ОбъектРасчетов КАК Документ.ПриобретениеТоваровУслуг).ЗакупкаПодДеятельность = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ))
	|	И НЕ(ОбъектРасчетов ССЫЛКА Справочник.ДоговорыКонтрагентов
	|		И ВЫРАЗИТЬ(ОбъектРасчетов КАК Справочник.ДоговорыКонтрагентов).ЗакупкаПодДеятельность = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ))
	|СГРУППИРОВАТЬ ПО
