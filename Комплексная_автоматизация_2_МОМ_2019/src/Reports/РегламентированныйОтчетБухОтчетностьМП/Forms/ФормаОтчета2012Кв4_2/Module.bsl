...
Процедура ПроверитьКодСтрокиБаланс(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) ИЛИ ЗначениеЗаполнено(Графа05) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан код строки """ + НаименованиеСтроки + """ бухгалтерского баланса", "Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Неверно указан код строки """ + НаименованиеСтроки + """ бухгалтерского баланса", "Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьКодСтрокиОПУ(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан код строки """ + НаименованиеСтроки + """ отчета о прибылях и убытках", "Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Неверно указан код строки """ + НаименованиеСтроки + """ отчета о прибылях и убытках", "Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиНаПечатьКНД0710098(ПараметрыПечати)
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		...
		ПечатнаяФорма = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаПечати2013Кв1_КНД0710098_1");
	...
	Иначе
		...
		ПечатнаяФорма = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаПечати2012Кв1_КНД0710098_1");
	...
	ПечатнаяФорма = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаПечати2012Кв1_КНД0710098_2");
	
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		ПечатнаяФорма = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаПечати2013Кв1_КНД0710098_2");
	Иначе
		ПечатнаяФорма = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаПечати2012Кв1_КНД0710098_2");
	КонецЕсли;
