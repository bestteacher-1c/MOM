...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		СтруктураРеквизитовФормы.ВерсияФормата = "5.12.04";
				...
				Для НомерСекции = 1 По ВыборкаСегментов.Количество() Цикл
					...
					Если СтрокиСекции <> Неопределено Тогда
						Для Каждого СтрокаСекции Из СтрокиСекции Цикл
							...
							Если ТребуетсяРазыменование И СтрокаСчета.Счет.Валютный
							 ИЛИ НЕ ТребуетсяРазыменование И СтрокаСчета.Валютный Тогда
...
Функция СформироватьФайлыПредставленийРазделов(ВременныйКаталогПакета, ПостфиксПредставлений)
	...
	Если ДействуетСВерсииФормата("5.11.01") Тогда
		СохранитьПредставлениеБанковскихОперацийВCSV(СформированныеФайлы, ПараметрыСохраненияПредставлений);
	КонецЕсли;
...
&НаСервере
Процедура СохранитьПредставлениеБанковскихОперацийВCSV(СформированныеФайлы, ПараметрыСохраненияПредставлений)
	
	ИмяРаздела = "ОперацииПоБанковскимСчетам";
	
	Если НЕ РазделВключенВОтчет(ИмяРаздела) Тогда
		Возврат;
	КонецЕсли;
	...
	ИмяФайлаРаздела = "TRN_" + ПараметрыСохраненияПредставлений.ПостфиксПредставлений + ".csv";
	ПолноеИмяФайлаРаздела = ПараметрыСохраненияПредставлений.ВременныйКаталогПакета + ИмяФайлаРаздела;
	
	ФайлСДанными = Новый ЗаписьТекста(ПолноеИмяФайлаРаздела, "windows-1251");
	
	ЭлементыЗаголовковСтолбцовCSV = Новый Массив;
	// Поля бухгалтерского счета.
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Счет бухгалтерского учета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сальдо на начало периода по бухгалтерскому счету");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Обороты за период по дебету бухгалтерского счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Обороты за период по кредиту бухгалтерского счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сальдо на конец периода по бухгалтерскому счету");
	// Поля банковского счета.
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Банковский счет");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Номер банковского счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("БИК банка");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Банк");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Код валюты");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сальдо на начало периода по банковскому счету");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Обороты за период по дебету банковского счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Обороты за период по кредиту банковского счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сальдо на конец периода по банковскому счету");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сальдо на начало периода по банковскому счету в валюте счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Обороты за период по дебету банковского счета в валюте счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Обороты за период по кредиту банковского счета в валюте счета");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сальдо на конец периода по банковскому счету в валюте счета");
	// Поля операции.
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Дата совершения операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Номер документа операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Дата документа операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Вид банковской операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Назначение платежа");
	Если ДействуетСВерсииФормата("5.12.01") Тогда
		ЭлементыЗаголовковСтолбцовCSV.Добавить("Вид движения");
	КонецЕсли;
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Код счета дебета операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Код счета кредита операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сумма операции");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Сумма операции в валюте");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Наименование контрагента");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("ИНН контрагента");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("КПП контрагента");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Договор с контрагентом");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("Номер банковского счета контрагента");
	ЭлементыЗаголовковСтолбцовCSV.Добавить("БИК банка контрагента");
	
	СтрокаЗаголовковСтолбцов = СтрСоединить(ЭлементыЗаголовковСтолбцовCSV, ",");
	ФайлСДанными.ЗаписатьСтроку(СтрокаЗаголовковСтолбцов);
	
	ИмяПодраздела = "Операции51";
	ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[0].Строки[0];
	ДобавитьСведенияПоБанковскимОперациям(ФайлСДанными, ИмяПодраздела, ДанныеРаздела);
	
	ИмяПодраздела = "Операции52";
	ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[1].Строки[0];
	ДобавитьСведенияПоБанковскимОперациям(ФайлСДанными, ИмяПодраздела, ДанныеРаздела);
	
	ИмяПодраздела = "Операции55";
	ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[2].Строки[0];
	ДобавитьСведенияПоБанковскимОперациям(ФайлСДанными, ИмяПодраздела, ДанныеРаздела);
	
	ФайлСДанными.Закрыть();
...
&НаСервере
Процедура ДобавитьСведенияПоБанковскимОперациям(ФайлСДанными, ИмяРаздела, ДанныеРаздела)
	...
	ЭлементыБухгалтерскогоСчета = Новый Массив;
	// Счет бухгалтерского учета.
	ЭлементыБухгалтерскогоСчета.Добавить(ЗначениеCSV(СтрокаСчетаВерхнегоУровня.Представление));
	// Сальдо на начало периода по бухгалтерскому счету.
	ЭлементыБухгалтерскогоСчета.Добавить(ЗначениеCSV(СтрокаСчетаВерхнегоУровня.СальдоНачальноеДт));
	// Обороты за период по дебету бухгалтерского счета.
	ЭлементыБухгалтерскогоСчета.Добавить(ЗначениеCSV(СтрокаСчетаВерхнегоУровня.ОборотыДт));
	// Обороты за период по кредиту бухгалтерского счета.
	ЭлементыБухгалтерскогоСчета.Добавить(ЗначениеCSV(СтрокаСчетаВерхнегоУровня.ОборотыКт));
	// Сальдо на конец периода по бухгалтерскому счету.
	ЭлементыБухгалтерскогоСчета.Добавить(ЗначениеCSV(СтрокаСчетаВерхнегоУровня.СальдоКонечноеДт));
	
	КонтектсФормированияCSV = Новый Структура;
	КонтектсФормированияCSV.Вставить("ФайлСДанными", ФайлСДанными);
	КонтектсФормированияCSV.Вставить("ПрефиксИдентификатораДанных", ПрефиксИдентификатораДанных);
	КонтектсФормированияCSV.Вставить("ПодстрокаБухгалтерскогоСчета", СтрСоединить(ЭлементыБухгалтерскогоСчета, ","));
	
	ДобавитьСведенияПоБанковскимОперациямПоСубсчетам(КонтектсФормированияCSV, СтрокаСчетаВерхнегоУровня);
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьСведенияПоБанковскимОперациямПоСубсчетам(КонтектсФормированияCSV, СтрокаСчетаВерхнегоУровня)
	
	ФайлСДанными = КонтектсФормированияCSV.ФайлСДанными;
	ПодстрокаБухгалтерскогоСчета = КонтектсФормированияCSV.ПодстрокаБухгалтерскогоСчета;
	
	ОписаниеТипаБанковскиеСчета = ОтчетностьВБанкиСлужебный.ТипыСущности("БанковскиеСчета");
	
	Если СтрокаСчетаВерхнегоУровня.Строки.Количество() = 0 Тогда
		ПустыеПоля = Новый Массив();
		ПустыеПоля.Добавить(""); // Банковский счет
		ПустыеПоля.Добавить(""); // Номер банковского счета
		ПустыеПоля.Добавить(""); // БИК банка
		ПустыеПоля.Добавить(""); // Банк
		ПустыеПоля.Добавить(""); // Код валюты
		ПустыеПоля.Добавить(""); // Сальдо на начало периода по банковскому счету
		ПустыеПоля.Добавить(""); // Обороты за период по дебету банковского счета
		ПустыеПоля.Добавить(""); // Обороты за период по кредиту банковского счета
		ПустыеПоля.Добавить(""); // Сальдо на конец периода по банковскому счету
		ПустыеПоля.Добавить(""); // Сальдо на начало периода по банковскому счету в валюте счета
		ПустыеПоля.Добавить(""); // Обороты за период по дебету банковского счета в валюте счета
		ПустыеПоля.Добавить(""); // Обороты за период по кредиту банковского счета в валюте счета
		ПустыеПоля.Добавить(""); // Сальдо на конец периода по банковскому счету в валюте счета
		ПустыеПоля.Добавить(""); // Дата совершения операции
		ПустыеПоля.Добавить(""); // Номер документа операции
		ПустыеПоля.Добавить(""); // Дата документа операции
		ПустыеПоля.Добавить(""); // Вид банковской операции
		ПустыеПоля.Добавить(""); // Назначение платежа
		ПустыеПоля.Добавить(""); // Код счета дебета операции
		ПустыеПоля.Добавить(""); // Код счета кредита операции
		ПустыеПоля.Добавить(""); // Сумма операции
		ПустыеПоля.Добавить(""); // Сумма операции в валюте
		ПустыеПоля.Добавить(""); // Наименование контрагента
		ПустыеПоля.Добавить(""); // ИНН контрагента
		ПустыеПоля.Добавить(""); // КПП контрагента
		ПустыеПоля.Добавить(""); // Договор с контрагентом
		ПустыеПоля.Добавить(""); // Номер банковского счета контрагента
		ПустыеПоля.Добавить(""); // БИК банка контрагента
		
		ПодстрокаПустыхПолей = СтрСоединить(ПустыеПоля, ",");
		ФайлСДанными.ЗаписатьСтроку(ПодстрокаБухгалтерскогоСчета + "," + ПодстрокаПустыхПолей);
		
	Иначе
		Для Каждого СтрокаСчета Из СтрокаСчетаВерхнегоУровня.Строки Цикл
			Если ОписаниеТипаБанковскиеСчета.СодержитТип(ТипЗнч(СтрокаСчета.Счет)) Тогда
				ДобавитьСведенияОбОперацияхПоБанковскомуСчету(КонтектсФормированияCSV, СтрокаСчета);
				
			Иначе
				ДобавитьСведенияПоБанковскимОперациямПоСубсчетам(КонтектсФормированияCSV, СтрокаСчета);
...
&НаСервере
Процедура ДобавитьСведенияОбОперацияхПоБанковскомуСчету(КонтектсФормированияCSV, СтрокаСчета)
	
	ФайлСДанными = КонтектсФормированияCSV.ФайлСДанными;
	ПрефиксИдентификатораДанных = КонтектсФормированияCSV.ПрефиксИдентификатораДанных;
	ПодстрокаБухгалтерскогоСчета = КонтектсФормированияCSV.ПодстрокаБухгалтерскогоСчета;
	
	ЭлементыБанковскогоСчета = Новый Массив;
	
	// Банковский счет.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.Представление));
	// Номер банковского счета.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.НомерСчета));
	// БИК банка.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.БИКБанка));
	// Банк.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.НаименованиеБанка));
	// Код валюты.
	Если СтрокаСчета.Валютный Тогда
		ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.КодВалюты));
	Иначе
		ЭлементыБанковскогоСчета.Добавить("");
	КонецЕсли;
	
	// Сальдо на начало периода по банковскому счету.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.СальдоНачальноеДт));
	// Обороты за период по дебету банковского счета.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.ОборотыДт));
	// Обороты за период по кредиту банковского счета.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.ОборотыКт));
	// Сальдо на конец периода по банковскому счету.
	ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.СальдоКонечноеДт));
	
	Если СтрокаСчета.Валютный Тогда
		// Сальдо на начало периода по банковскому счету в валюте счета.
		ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.СальдоНачальноеВалДт));
		// Обороты за период по дебету банковского счета в валюте счета.
		ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.ОборотыВалДт));
		// Обороты за период по кредиту банковского счета в валюте счета.
		ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.ОборотыВалКт));
		// Сальдо на конец периода по банковскому счету в валюте счета.
		ЭлементыБанковскогоСчета.Добавить(ЗначениеCSV(СтрокаСчета.СальдоКонечноеВалДт));
	...
	ПодстрокаБанковскогоСчета = СтрСоединить(ЭлементыБанковскогоСчета, ",");
	...
	Если ВыборкаСегментов.Количество() > 0 Тогда
		Для НомерСекции = 1 По ВыборкаСегментов.Количество() Цикл
			...
			Если СтрокиСекции <> Неопределено Тогда
				Для Каждого СтрокаСекции Из СтрокиСекции Цикл
					ЭлементыОперации = Новый Массив;
					
					// Дата совершения операции.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.ДатаОперации));
					// Номер документа, по которому совершена операция.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.НомерДокумента));
					// Дата документа, по которому совершена операция.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.ДатаДокумента));
					// Вид банковской операции.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.ВидОперации));
					// Назначение платежа.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.Назначение));
					// Вид движения денег по счету.
					Если ДействуетСВерсииФормата("5.12.01") Тогда
						ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.ВидДвиженияПоСчету));
					КонецЕсли;
					// Код счета, по дебету которого операция отражена в бухгалтерском учете.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.СчетДтКод));
					// Код счета, по кредиту которого операция отражена в бухгалтерском учете.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.СчетКтКод));
					// Сумма операции.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.Сумма));
					
					// Сумма операции в валюте.
					Если СтрокаСчета.Счет.Валютный Тогда
						ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.СуммаВал));
					Иначе
						ЭлементыОперации.Добавить("");
					КонецЕсли;
					
					// Наименование контрагента.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.Контрагент));
					// ИНН контрагента.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.ИНН));
					// КПП контрагента.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.КПП));
					// Договор с контрагентом.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.Договор));
					
					// Номер банковского счета.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.СчетКонтрагентаНомер));
					// БИК банка, в котором открыт счет.
					ЭлементыОперации.Добавить(ЗначениеCSV(СтрокаСекции.СчетКонтрагентаБИК));
					
					ПодстрокаОперации = СтрСоединить(ЭлементыОперации, ",");
					ФайлСДанными.ЗаписатьСтроку(ПодстрокаБухгалтерскогоСчета + "," + ПодстрокаБанковскогоСчета + "," + ПодстрокаОперации);
	...
	Иначе
		...
		ПустыеПоля.Добавить(""); // Назначение платежа
		Если ДействуетСВерсииФормата("5.12.01") Тогда
			ПустыеПоля.Добавить(""); // Вид движения денег по счету
		КонецЕсли;
		ПустыеПоля.Добавить(""); // Код счета дебета операции
		ПустыеПоля.Добавить(""); // Код счета кредита операции
		ПустыеПоля.Добавить(""); // Сумма операции
		...
		ПодстрокаПустыхПолей = СтрСоединить(ПустыеПоля, ",");
		ФайлСДанными.ЗаписатьСтроку(ПодстрокаБухгалтерскогоСчета + "," + ПодстрокаБанковскогоСчета + "," + ПодстрокаПустыхПолей);
...
&НаСервере
Функция ЗначениеCSV(ИсходноеЗначение)
	...
	Если ТипЗнч(ИсходноеЗначение) = Тип("Строка") Тогда
		...
		Если СтрЧислоСтрок(ПредставлениеЗначения) > 1 ИЛИ СтрНайти(ПредставлениеЗначения, ",") <> 0
		 ИЛИ СтрНайти(ПредставлениеЗначения, """") <> 0 Тогда
			ПредставлениеЗначения = """" + ПредставлениеЗначения + """";
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(ИсходноеЗначение) = Тип("Число") Тогда
		ПредставлениеЗначения = Формат(ИсходноеЗначение, "ЧРД=.; ЧН=; ЧГ=0");
		
	ИначеЕсли ТипЗнч(ИсходноеЗначение) = Тип("Дата") Тогда
		ПредставлениеЗначения = Формат(ИсходноеЗначение, "ДФ=dd.MM.yyyy");
		
	Иначе
		ПредставлениеЗначения = ЗначениеCSV(Строка(ИсходноеЗначение));
