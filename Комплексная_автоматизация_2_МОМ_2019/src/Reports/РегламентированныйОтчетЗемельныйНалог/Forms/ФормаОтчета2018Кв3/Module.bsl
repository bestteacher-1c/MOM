		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					// Особая обработка в декларации за 2020 год: учтены указания в письме ФНС России от 31.10.2019 № БС-4-21/22311@
					//
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) = 2020 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФормРеорг) Тогда
			СообщитьВТС("В декларации за 2020 год должен быть заполнен код формы реорганизации. Основание: письмо ФНС России от 31.10.2019 № БС-4-21/22311@",
		...
		Иначе
			Если ВозможныеКодыФормыРеорганизции.Найти(ПараметрыВыгрузки.ФормРеорг) = Неопределено Тогда
				...
				+ СтрСоединить(ВозможныеКодыФормыРеорганизции, ", ") + ". Основание: письмо ФНС России от 31.10.2019 № БС-4-21/22311@",
				"Титульный", "КодФормыРеорганизации");
				СтатусПроверки = Ложь;
			Иначе
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2020 Тогда
		Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
			КодФормыРеорганизации = СокрЛП(ТабличныйДокумент.Области.КодФормыРеорганизации.Значение);
		Иначе
			КодФормыРеорганизации = СокрЛП(СтруктураДанныхТитульный.КодФормыРеорганизации);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(КодФормыРеорганизации) Тогда
			ТекстСообщения
			= "В декларации за 2020 год должен быть заполнен код формы реорганизации. Основание: письмо ФНС России от 31.10.2019 № БС-4-21/22311@";
		Иначе
			ДопустимыеКодыФормыРеорганизации = "1, 2, 3, 5, 6";
			Если СтрРазделить(ДопустимыеКодыФормыРеорганизации, ", ", Ложь).Найти(КодФормыРеорганизации) = Неопределено Тогда
				ТекстСообщения
				= "В декларации за 2020 год код формы реорганизации должен быть равен одному из следующих значений: "
				+ ДопустимыеКодыФормыРеорганизации + ". Основание: письмо ФНС России от 31.10.2019 № БС-4-21/22311@";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
