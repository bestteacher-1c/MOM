...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	ИначеЕсли ПараметрыПечати.ИмяСтраницы = "Раздел2" Тогда
		...
		Кабдт = ТабличныйДокумент.Область("П000200012303").Значение;
		
		Если Кабдт <> Цел(Кабдт) Тогда
			
			МакетМЧБ.УдалитьОбласть(МакетМЧБ.Область("Секция_Кабдт_6_0"), ТипСмещенияТабличногоДокумента.ПоВертикали);
			
			НовСтр = Показатели.Добавить();
			НовСтр.Имя = "П200200012303";
			НовСтр.Значение = Кабдт;
			НовСтр.ЗначениеМЧБ = Кабдт;
			
		Иначе
			
			МакетМЧБ.УдалитьОбласть(МакетМЧБ.Область("Секция_Кабдт_10_4"), ТипСмещенияТабличногоДокумента.ПоВертикали);
			
			НовСтр = Показатели.Добавить();
			НовСтр.Имя = "П100200012303";
			НовСтр.Значение = ?(Кабдт = 0, "------",
			СтроковыеФункцииКлиентСервер.ДополнитьСтроку(Формат(Кабдт, "ЧГ="), 6, "-", "Справа"));
...
Процедура СформироватьСпискиВыбора()
	...
	// Определение списка кодов основания налогообложения (кроме раздела 7).
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20200401' Тогда
		ИмяСекции_ОснованияНалогообложения = "ОснованияНалогообложения_20200401";
	Иначе
		ИмяСекции_ОснованияНалогообложения = "ОснованияНалогообложения";
	КонецЕсли;
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
		ИмяСекции_КБК_Раздел1 = "КБК_Раздел1_20200101";
	Иначе
		ИмяСекции_КБК_Раздел1 = "КБК";
	КонецЕсли;
	...
	"Выбор кода основания налогообложения", КоллекцияСписковВыбора[ИмяСекции_ОснованияНалогообложения]);
	...
	//Раздел 4.
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
		ИмяСекции_КБК_Раздел4 = "КБК_Раздел4_20200101";
	Иначе
		ИмяСекции_КБК_Раздел4 = "КБК";
	КонецЕсли;
	...
	"Выбор кода бюджетной классификации", КоллекцияСписковВыбора[ИмяСекции_КБК_Раздел4]);
	...
	//Раздел 5.
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
		ИмяСекции_КБК_Раздел5    = "КБК_Раздел5_20200101";
		ИмяСекции_ВидыПИ_Раздел5 = "ВидыПИ_Раздел5_20200101";
		ИмяСекции_КоэфКподз      = "КоэфКподз_20200101";
	Иначе
		ИмяСекции_КБК_Раздел5    = "КБК_Раздел5";
		ИмяСекции_ВидыПИ_Раздел5 = "ВидыПИ_Раздел5";
		ИмяСекции_КоэфКподз      = "КоэфКподз";
	КонецЕсли;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000500001003",  5, ,
	"Выбор кода вида полезного ископаемого", КоллекцияСписковВыбора[ИмяСекции_ВидыПИ_Раздел5]);
	...
	"Выбор значения коэффициента Кподз", КоллекцияСписковВыбора[ИмяСекции_КоэфКподз]);
	...
	//Раздел 6.
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
		ИмяСекции_ВидыПИ = "ВидыПИ_20200101";
	Иначе
		ИмяСекции_ВидыПИ = "ВидыПИ";
	КонецЕсли;
	...
	"Выбор кода вида полезного ископаемого", КоллекцияСписковВыбора[ИмяСекции_ВидыПИ]);
