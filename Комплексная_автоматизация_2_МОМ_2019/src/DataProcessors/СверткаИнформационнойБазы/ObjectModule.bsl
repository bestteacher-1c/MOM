...
Процедура ПровестиДокументыВводаОстатков() Экспорт
	...
	//++ НЕ УТ
	РеглУчетПроведениеСервер.ОтразитьВсе(КонецДня(ДатаСверткиИБ));
	//-- НЕ УТ
КонецПроцедуры
...
Процедура СформироватьДокументыВводаОстатковРасходыПриУСН()
	...
	|	НДСОстаток КАК НДС,
	|	ПризнаноОстаток КАК Признано,
	|	НЕПризнаноОстаток КАК НеПризнано,
...
Процедура СформироватьДокументыВводаОстатковВнеоборотныхАктивов()
	...
	|	ВЫБОР КОГДА СостоянияОС.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету) ТОГДА
	|		ПервоначальныеСведенияОС.Арендодатель
					...
					Пока Выборка.СледующийПоЗначениюПоля("ОтражатьВРеглУчете") Цикл
						Пока Выборка.СледующийПоЗначениюПоля("Местонахождение") Цикл
							Пока Выборка.СледующийПоЗначениюПоля("Контрагент") Цикл
								...
								ДокОст.ОтражатьВБУиНУ = Выборка.ОтражатьВРеглУчете;
		...
		Пока ВыборкаПодразделение.Следующий() Цикл
			...
			Пока ВыборкаУУ.Следующий() Цикл
				...
				Пока ВыборкаБУ.Следующий() Цикл
					...
					ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, ВыборкаБУ, Истина);
