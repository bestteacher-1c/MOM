...
Процедура ОбработатьЭлементыСопоставленияУпаковки(Знач ЭлементСопоставления)
	...
	УпаковкаСВладельцем = Ложь;
	СвязиПараметровВыбораУпаковки = Элементы.СопоставлениеУпаковка.СвязиПараметровВыбора;
	Если ЗначениеЗаполнено(СвязиПараметровВыбораУпаковки) Тогда
		ПолеВладельцаУпаковки = СтрЗаменить(СвязиПараметровВыбораУпаковки[0].Имя, "Отбор.", "");
		УпаковкаСВладельцем = ЭлементСопоставления.Упаковка[ПолеВладельцаУпаковки] <> Неопределено;
	КонецЕсли;
	...
	Для Каждого Запись Из ЗаписиСопоставления Цикл
		...
		Если ЗначениеЗаполнено(Запись.Упаковка)
			...
			ИЛИ НЕ Запись.ИспользоватьУпаковки
			ИЛИ УпаковкаСВладельцем И Запись.Номенклатура <> ЭлементСопоставления.Номенклатура Тогда
			Продолжить;
		КонецЕсли;
