...
Процедура ЗагрузитьДанныеДокументаДлительнаяОперация(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.ПроверкаНеПоДокументу Тогда
		...
		ДополнитьТаблицуПодобраннойМаркируемойПродукции(ПроверяемыеДанные.МаркированныеТовары, ДанныеДокумента, Параметры.ВидМаркируемойПродукции);
	...
	Иначе
		...
		ДополнитьТаблицуПодобраннойМаркируемойПродукции(
			ДанныеПроверяемогоДокумента.МаркированныеТовары, ДанныеДокумента, Параметры.ВидМаркируемойПродукции);
...
Процедура ЗафиксироватьРезультатПроверкиИПодбора(Параметры, АдресРезультата) Экспорт
	...
	ЗаполнитьСпецификуВЕТИС = Параметры.ВидМаркируемойПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция
		И ТипЗнч(Параметры.ПроверяемыйДокумент) = Тип("ДокументСсылка.МаркировкаТоваровИСМП");
	Если ЗаполнитьСпецификуВЕТИС Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.ПроверяемыйДокумент, "Операция,ОперацияНанесения");
		ЗаполнитьСпецификуВЕТИС = ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(Реквизиты.Операция)<>Неопределено
	...
	Попытка
		...
		Если ЗаполнитьСпецификуВЕТИС Тогда
			...
			ЗаполнитьШтрихкодыВерхнегоУровняВЕТИС(ДеревоМаркированнойПродукции.Строки, ТаблицаШтрихкодовВерхнегоУровня);
		КонецЕсли;
		
		ТаблицаШтрихкодовОбщаяПоДаннымДерева(ДеревоМаркированнойПродукции, ТаблицаШтрихкодовОбщая, ЗаполнитьСпецификуВЕТИС);
...
Процедура ЗаполнитьШтрихкодыВерхнегоУровняВЕТИС(СтрокиДерева, ТаблицаШтрихкодов, ЭтоВерхнийУровень = Истина)
	
	Для Каждого Строка Из СтрокиДерева Цикл
		Если ЭтоВерхнийУровень Или ЗначениеЗаполнено(Строка.ИдентификаторПроисхожденияВЕТИС) Тогда
		...
		Если Не ЗначениеЗаполнено(Строка.ИдентификаторПроисхожденияВЕТИС) Тогда
			ЗаполнитьШтрихкодыВерхнегоУровняВЕТИС(Строка.Строки, ТаблицаШтрихкодов, Ложь);
		КонецЕсли;
	КонецЦикла;
...
//	РежимПодбораСуществующихУпаковок - Булево - признак работы со штрихкодами упаковок, имеющимися в информационной базе.
// Возвращаемое значение:
//	ПеречислениеСсылка.ДетализацияСтруктурыХраненияПродукцииИСМП - сохраненная детализация отображения структуры упаковок
//
...
// Возвращаемое значение:
//	ПеречислениеСсылка.ДетализацияСтруктурыХраненияПродукцииИСМП - расчитаная детализация структуры упаковок
//
...
Процедура ДополнитьТаблицуПодобраннойМаркируемойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ВидПродукции)
