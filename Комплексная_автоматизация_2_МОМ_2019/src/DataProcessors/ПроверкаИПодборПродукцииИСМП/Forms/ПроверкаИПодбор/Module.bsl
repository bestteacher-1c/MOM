...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСМППереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, ВидМаркируемойПродукции,
	...
	Для Каждого ИдентификаторСтроки Из Элемент.ВыделенныеСтроки Цикл
		...
		ИначеЕсли УдаляемыйЭлемент.ТипУпаковки = ПредопределенноеЗначение("Перечисление.ПрочиеЗоныПересчетаПродукцииИСМП.ПродукцияБезУпаковки")
			Или (РодительУдаляемогоЭлемента <> Неопределено
				...
				Если Не ИнтеграцияИСКлиентСервер.ЭтоУпаковка(УдаляемыйЭлемент.ТипУпаковки)
					Или ДобавленныеУпаковки.НайтиПоЗначению(УдаляемыйЭлемент.Штрихкод) = Неопределено Тогда
...
Процедура ПроверкаЗавершена(Команда)
	...
	Если Не ТребуетсяВопросПередЗавершениемПроверки() Тогда
...
Функция ЗапроситьСтатусыКодовМаркировки(СоответствиеШтрихкодовСтрокДерева, ВидПродукции)
	...
	РезультатЗапроса = ИнтерфейсИСМП.СтатусыКодовМаркировки(МассивСтрокКодов);
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	ИначеЕсли ВидМаркируемойПродукции = Перечисления.ВидыПродукцииИС.Табак
...
Процедура ЗаполнитьПараметрыПроверкиКодовМаркировки(Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПараметрыПроверкиКодовМаркировки.ДопустимыеСтатусыУпаковок            = ДопустимыеСтатусыУпаковокИСМП;
	КонецЕсли;
...
Процедура ВывестиДанныеПроверкиИПодбораНаФорму(ДанныеПроверкиИПодбора)
	...
	Если ДанныеПроверкиИПодбора.Свойство("ДеревоМаркированнойПродукции") Тогда
		...
		ДобавитьИЗаполнитьКолонкиПриВосстановленииДерева(ДеревоМаркированнойПродукцииОбъект, Истина, Ложь);
	...
	Если ДанныеПроверкиИПодбора.Свойство("ПодобраннаяМаркируемаяПродукция") Тогда
		...
		ДобавитьИЗаполнитьКолонкиПриВосстановленииДерева(ПодобраннаяМаркируемаяПродукцияОбъект, Ложь, Истина);
		
		Попытка
			ЗначениеВРеквизитФормы(ПодобраннаяМаркируемаяПродукцияОбъект, "ПодобраннаяМаркируемаяПродукция");
		Исключение
			ПроверкаИПодборПродукцииИС.ПроверитьКолонкиИсточникаИПриемникаНаСовместимость(
				ПодобраннаяМаркируемаяПродукцияОбъект, РеквизитФормыВЗначение("ПодобраннаяМаркируемаяПродукция"));
			Возврат;
		КонецПопытки;
...
&НаСервере
Процедура ДобавитьИЗаполнитьКолонкиПриВосстановленииДерева(ДеревоМаркированнойПродукцииОбъект, ПроверятьКолонкуВидПродукции, ПроверятьКолонкуСопоставлениеНоменклатуры)

	// Адаптация старого формата дерева упаковок
	
	ЗаполнитьКолонкуВидУпаковки = Ложь;
	Если ПроверятьКолонкуВидПродукции Тогда
	...
	Если ПроверятьКолонкуСопоставлениеНоменклатуры Тогда
...
Процедура ЗагрузитьДанныеДокумента()
	...
	Иначе
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ДетализацияСтруктурыХранения) Тогда
			ДетализацияСтруктурыХранения = ПредопределенноеЗначение("Перечисление.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная");
		КонецЕсли;
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	ВидПродукцииРодительный = НРег(ПолучитьСклоненияСтроки(ВидМаркируемойПродукции, "Л=ru_RU", "ПД=Родительный")[0]);
	...
	Если ВидМаркируемойПродукции = Перечисления.ВидыПродукцииИС.Обувь Тогда
	...
	Элементы.ПодобраннаяМаркируемаяПродукцияГоденДо.Видимость = (ВидМаркируемойПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция);
	Элементы.ПодобраннаяМаркируемаяПродукцияИдентификаторПроисхожденияВЕТИС.Видимость =
		(ВидМаркируемойПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(Результат.КакУчитыватьНеПроверенныеОтложенные) Тогда
			...
			Если Результат.КакУчитыватьНеПроверенныеОтложенные = ВНаличии 
				И ПроверкаЭлектронногоДокумента
				И АктыОРасхожденияхПослеПриемкиИспользуются
				И ПроверкаИПодборПродукцииИСМПКлиентСервер.ЭтоДокументПриобретения(ПроверяемыйДокумент)
				И ТребуетсяВопросПередЗавершениемПроверки() Тогда
					Возврат;
			КонецЕсли;
...
Функция ТаблицаПодобраннойПродукцииЗаполненаКорректно()
	...
	Если ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.ПриемкаТоваровИСМП") Тогда
...
Процедура ОбработатьСканированиеИмеющегосяВДеревеШтрихкода(ДанныеШтрихкода, ТекущаяСтрокаДерева, ИдентификаторНайденнойСтроки)
	...
	Иначе
		...
		ЗаполнитьСтрокуДереваПоДаннымШтрихкода(НайденнаяСтрокаДерева, ДанныеШтрихкода, ТаблицаИзмененийПодобраннойПродукции);
...
Процедура ОткрытьФормуУточненияДанных(ИдентификаторСтрокиДерева, ТекущаяСтрокаДерева, ПроизвольноеРедактированиеРеквизитов = Ложь)
	...
	ПараметрыОткрытияФормы.ПараметрыУказанияСерий = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(
		ПараметрыСканирования.ПараметрыУказанияСерий, Ложь);
	ПараметрыОткрытияФормы.Склад                  = ПараметрыСканирования.Склад;
	...
	ДополнительныеПараметры = Новый Структура("ИдентификаторСтрокиДерева, ТекущаяСтрокаДерева, ПроизвольноеРедактированиеРеквизитов", ИдентификаторСтрокиДерева, ТекущаяСтрокаДерева, ПроизвольноеРедактированиеРеквизитов);
...
Процедура УточнитьДанныеУПользователяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	Контейнер.Вставить("ВременныйПризнакЭтоПроизвольноеРедактированиеСуществующейСтроки", ЭтоПроизвольноеРедактированиеСуществующейСтроки);
	
	ОбработатьНайденныйВДеревеШтрихкод(НайденнаяСтрокаДерева, ТекущаяСтрокаДерева);
	
	Контейнер.Удалить("ВременныйПризнакЭтоПроизвольноеРедактированиеСуществующейСтроки");
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСтрокуДереваПоДаннымШтрихкода(НайденнаяСтрокаДерева, ДанныеШтрихкода, ТаблицаИзмененийПодобраннойПродукции)
	...
	ЗаполняемыеПоля = СтрРазделить("Номенклатура,Характеристика,Серия,ИдентификаторПроисхожденияВЕТИС,ГоденДо",",");
	...
	Если ЗаполнитьИзмененияПодобраннойПродукции Тогда
		ДобавитьСтрокуВТаблицуИзменений(ТаблицаИзмененийПодобраннойПродукции, НайденнаяСтрокаДерева, -1);
		Для Каждого ИмяПоля Из ЗаполняемыеПоля Цикл
			НайденнаяСтрокаДерева[ИмяПоля] = ДанныеШтрихкода[ИмяПоля];
		КонецЦикла;
		НайденнаяСтрокаДерева.ПредставлениеСодержимоеУпаковки = НайденнаяСтрокаДерева.Номенклатура;
		ДобавитьСтрокуВТаблицуИзменений(ТаблицаИзмененийПодобраннойПродукции, НайденнаяСтрокаДерева, 1);
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуДереваПоДаннымВыбора(НайденнаяСтрокаДерева, ДанныеВыбора, ЗаполнитьИзмененияПодобраннойПродукции = Ложь)
	...
	НайденнаяСтрокаДерева.ПредставлениеСодержимоеУпаковки = ДанныеВыбора.Номенклатура;
...
Функция ТребуетсяСбросСохраненногоВыбораПоДаннымДокумента()
	...
	Отбор = Новый Структура("Номенклатура,Характеристика,Серия,GTIN,ГоденДо,ИдентификаторПроисхожденияВЕТИС");
	...
	Если СтрокиМаркируемойПродукции.Количество()
		И СтрокиМаркируемойПродукции[0].Количество = СтрокиМаркируемойПродукции[0].КоличествоПодобрано Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
...
Процедура ОтметитьСтрокуКакНайденную(СтрокаДерева)
	...
	ДобавитьСтрокуВТаблицуИзменений = Истина;
	
	Если ЗначениеЗаполнено(Контейнер)
		И Контейнер.Свойство("ВременныйПризнакЭтоПроизвольноеРедактированиеСуществующейСтроки")
		И Контейнер.ВременныйПризнакЭтоПроизвольноеРедактированиеСуществующейСтроки Тогда
		ДобавитьСтрокуВТаблицуИзменений = Ложь; // Изменения отражены ранее
	КонецЕсли;
	
	Если Не РежимПодбораСуществующихУпаковок И ДобавитьСтрокуВТаблицуИзменений Тогда
		ДобавитьСтрокуВТаблицуИзменений(ТаблицаИзмененийПодобраннойПродукции, СтрокаДерева, 1);
	КонецЕсли;
...
Процедура ПодобраннаяПродукцияПриУдаленииСтроки(Форма, СтрокаИзменений)
	...
	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриУдаленииСтроки(
		Форма, СтрокаИзменений, СтрокаИзменений.Изменение);
...
Процедура ПодобраннаяПродукцияПриДобавленииСтроки(Форма, СтрокаИзменений)
	...
	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриДобавленииСтроки(
		Форма, СтрокаИзменений, ИзменениеКоличества);
...
Функция ШтрихкодыУпаковокДокументаСоответствуютВидуПродукции(Документ, ПараметрыСканирования) Экспорт
	...
	Для Каждого Штрихкод Из ШтрихкодыУпаковокДокумента Цикл
		ДанныеРазбора = ШтрихкодированиеИССлужебный.РазобратьКодМаркировки(Штрихкод, ДопустимыеВидыПродукции);
...
Процедура Подключаемый_ПолученыДанныеИзТСД(Штрихкоды, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЗагрузкаДанныхТСД = ОбработатьПолученныеДанныеТСДНаСервере(ДанныеДляТСД);
	...
	Если ЗагрузкаДанныхТСД.ЕстьОшибкиВДереве Тогда
		...
		ПараметрыОткрытияФормы = ШтрихкодированиеИСКлиент.ПараметрыОткрытияФормыНевозможностиДобавленияОтсканированного(ВидМаркируемойПродукции);
...
&НаСервере
Функция ОбработатьПолученныеДанныеТСДНаСервере(ДанныеДляТСД)
	...
	// Обработчик после завершения загрузки: восстановить проверяемую упаковку
	Результат.Вставить("ПроверяемаяУпаковка", ИдентификаторТекущейПроверяемойУпаковки);
	...
	Для Каждого СтрокаДанныхТСД Из ДанныеДляТСД.Штрихкоды Цикл
		...
		ДанныеШтрихкода = Новый Структура("Штрихкод, Количество", СтрокаДанныхТСД.НормализованныйШтрихкод, 1);
	...
	РезультатыОбработки = ШтрихкодированиеИС.ОбработатьШтрихкоды(
		ДанныеШтрихкодов, ДанныеДляТСД.ПараметрыСканирования, Неопределено, ЭтотОбъект, КешДанныхРазбора);
	...
	ГрупповаяОбработкаШтрихкодовИС.УпорядочитьДанныеТСДПоРезультатамОбработкиШтрихкодов(
		ДанныеДляТСД.Штрихкоды, РезультатыОбработки);
	...
	Возврат ОбработатьШтрихкодыНаСервере(
...
Функция ОбработатьОчереднойШтрихкодНаСервере(ПараметрыСканирования, СоответствиеШтрихкодовСтрокДерева)
	...
	Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияПродукцииИСМП.ПотребительскиеУпаковки
		И ИнтеграцияИСКлиентСервер.ЭтоУпаковкаПоВиду(ДанныеШтрихкода.ВидУпаковки) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр(СтрШаблон("ru = 'Код маркировки упаковки %1 не соответствует детализации, пропущен'", ДанныеШтрихкода.Штрихкод)));
		Возврат Истина;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаШтрихкод.Штрихкод) Тогда
		...
		Если ТребуетсяУточнениеДанных Тогда
			...
			Если Идентификатор <> -1 Тогда
				...
				УказатьИдентификаторВЕТИС = ДанныеШтрихкода.Свойство("ТребуетсяВыборВСД") И ДанныеШтрихкода.ТребуетсяВыборВСД;
				Если УказатьИдентификаторВЕТИС Тогда
					ТребуетсяУточнениеДанных = Не ЗначениеЗаполнено(СтрокаДерева.ИдентификаторПроисхожденияВЕТИС);
				ИначеЕсли ЗначениеЗаполнено(СтрокаДерева.Серия) Тогда
...
Функция ОбработатьНеНайденныйВДеревеШтрихКодНаСервере(ДанныеШтрихкода, СоответствиеШтрихкодов)
	...
	Если ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
		...
		Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияПродукцииИСМП.КоробаПотребительскиеУпаковки Тогда
			ИдентификаторТекущейПроверяемойУпаковки = -1;
		КонецЕсли;
...
Процедура ОчиститьСопоставлениеНаСервере(ВыделенныеСтроки)
	...
	Для Каждого ИдентификаторИзмененойСтроки из ВыделенныеСтроки Цикл
		СтрокаДерева = НайтиПоИдентификатору(ДеревоМаркированнойПродукции, ИдентификаторИзмененойСтроки);
	...
	РегистрыСведений.КэшОписанияОстатковИСМП.ЗаполнитьТаблицуПредставленийGTINОстатки(
			ТаблицаПредставлениеОстаточныхGTIN,,
	...
	ТаблицаПредставлениеОстаточныхGTIN.Индексы.Добавить("GTIN");
	...
	Для Каждого СтрокаДерева Из МассивСтрокДерева Цикл
		...
		СтрокаТаблицыПредставлений = ТаблицаПредставлениеОстаточныхGTIN.Найти(СтрокаДерева.GTIN, "GTIN");
		
		Если СтрокаТаблицыПредставлений <> Неопределено Тогда
			СтрокаДерева.ПредставлениеСодержимоеУпаковки = СтрокаТаблицыПредставлений.ПредставлениеНоменклатуры;
		КонецЕсли;
