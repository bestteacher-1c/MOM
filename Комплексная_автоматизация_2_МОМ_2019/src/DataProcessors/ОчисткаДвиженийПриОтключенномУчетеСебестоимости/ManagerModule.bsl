...
Процедура ОчиститьНеиспользуемыеДвиженияПоРегистрамСебестоимости(Параметры = "", АдресХранилища = "") Экспорт
	...
	//++ НЕ УТ
	ЗарегистрироватьКОтражениюВРегламентированномУчете(Запрос.МенеджерВременныхТаблиц);
	//-- НЕ УТ
...
Процедура ЗарегистрироватьКОтражениюВРегламентированномУчете(МенеджерВременныхТаблиц)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИзмененныеДокументы.Регистратор КАК Документ,
	|	ИзмененныеДокументы.Организация КАК Организация,
	|	ИзмененныеДокументы.Период      КАК ДатаОтражения
	|ПОМЕСТИТЬ ДокументыКОтражению
	|ИЗ
	|	ВТНеиспользуемыеДвижения КАК ИзмененныеДокументы
	|ГДЕ
	|	ИзмененныеДокументы.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ,
	|	Организация
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(МенеджерВременныхТаблиц, "ДокументыКОтражению");
