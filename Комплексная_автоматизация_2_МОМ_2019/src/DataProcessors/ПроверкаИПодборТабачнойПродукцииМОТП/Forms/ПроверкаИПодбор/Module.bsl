...
Процедура ДеревоМаркированнойПродукцииПередУдалением(Элемент, Отказ)
	...
	Для Каждого ИдентификаторСтроки Из Элемент.ВыделенныеСтроки Цикл
		...
		ИначеЕсли УдаляемыйЭлемент.ТипУпаковки = ПредопределенноеЗначение("Перечисление.ПрочиеЗоныПересчетаПродукцииИСМП.ПачкиБезБлока")
			...
			Если Не ДобавленнаяПотребительскаяУпаковка(ЭтотОбъект, УдаляемыйЭлемент.Штрихкод, "Содержит") Тогда
	...
	Если ИдентификаторТекущейПроверяемойУпаковки <> -1 Тогда
		Если (ИдентификаторСтрокиПачкиБезБлока <> -1 И ИдентификаторТекущейПроверяемойУпаковки = ИдентификаторСтрокиПачкиБезБлока)
			Или (ИдентификаторСтрокиБлокиБезКоробки <> -1 И ИдентификаторТекущейПроверяемойУпаковки = ИдентификаторСтрокиБлокиБезКоробки) Тогда
			ИдентификаторТекущейПроверяемойУпаковки = -1;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПодобраннаяМаркируемаяПродукцияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элемент.ПодчиненныеЭлементы.ПодобраннаяМаркируемаяПродукцияНоменклатура
		...
		И (Не (ЭтоДокументПриобретения И ТекущиеДанные.НоменклатураСопоставленаПоУПД)) Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьЗначение(, ТекущиеДанные.Номенклатура);
	КонецЕсли;
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	ВидМаркируемойПродукции = ВидПродукцииИС();
	...
	Элементы.ДеревоМаркированнойПродукцииКонтекстноеМенюУточнитьДанныеПоШтрихкоду.Видимость = ПроверкаЭлектронногоДокумента;
	Элементы.ОчиститьСопоставление.Видимость = ПроверкаЭлектронногоДокумента;
...
Процедура ЗаполнитьПараметрыПроверкиКодовМаркировки(Отказ)
	...
	Если Не Отказ Тогда
		...
		ПараметрыПроверкиКодовМаркировки.ДопустимыеСтатусыУпаковок            = ДопустимыеСтатусыУпаковокМОТП;
	КонецЕсли;
...
Функция РезультатыПроверкиУспешноСохранены()
	...
	Если ПроверкаИПодборПродукцииИСМП.РезультатыПроверкиУспешноСохранены(ЭтотОбъект, ВидПродукцииИС(), ТекстОшибки) Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ОбработатьНеНайденныйВДеревеШтрихКод(ДанныеШтрихкода, ТекущаяСтрокаДерева)
	...
	Если ДанныеШтрихкода.МаркируемаяПродукция = Истина
		...
		ПараметрыОткрытияФормы.ВидПродукции                    = ВидПродукцииИС();
...
Процедура ПроверитьШтрихкодВДеревеМаркированнойПродукции(ДанныеШтрихкода)
	...
	Если ИдентификаторНайденнойСтроки = -1 Тогда
		ИдентификаторНайденнойСтроки = ИдентификаторСтрокиДереваПоШтрихкоду(
			ШтрихкодированиеИСМПКлиентСервер.КодМаркировкиБезСкобок(ДанныеШтрихкода.Штрихкод));
	КонецЕсли;
...
Функция ТребуетсяСбросСохраненногоВыбораПоДаннымДокумента()
	...
	Отбор = Новый Структура("Номенклатура,Характеристика,Серия,GTIN");
...
Процедура ДобавитьНовуюУпаковку(ДанныеШтрихкода, ТипУпаковки, СпозиционироватьсяНаДобавленной = Истина)
	...
	ВидУпаковки = ВидУпаковкиПоВходящимДанным(ДанныеШтрихкода, ТипУпаковки);
...
Функция НоменклатурыСтрокСовпадают(ИсходнаяСтрока, СтрокаИзменений)
	...
	Возврат ПроверкаИПодборПродукцииИСМПКлиентСервер.НоменклатурыСтрокСовпадают(ИсходнаяСтрока, СтрокаИзменений);
...
&НаКлиентеНаСервереБезКонтекста
Функция ВидУпаковкиПоВходящимДанным(ДанныеШтрихкода, ТипУпаковки)
	...
	Иначе
		РезультатРазбора = РазобратьКодМаркировки(ДанныеШтрихкода.Штрихкод);
...
&НаСервереБезКонтекста
Функция РазобратьКодМаркировки(Штрихкод)
	Возврат ШтрихкодированиеИССлужебный.РазобратьКодМаркировки(Штрихкод, ВидПродукцииИС());
КонецФункции
	...
	Если ЗагрузкаДанныхТСД.ТребуетсяАвторизация Тогда
		...
		ИнтерфейсАвторизацииИСМПКлиент.ЗапроситьКлючСессии(
			ИнтерфейсАвторизацииИСМПКлиент.ПараметрыЗапросаКлючаСессии(
				ПараметрыСканирования.Организация, ВидПродукцииИС()),
...
Функция ОбработатьПолученныеДанныеТСДНаСервере(ДанныеДляТСД)
	...
	Для Каждого СтрокаДанныхТСД Из ДанныеДляТСД.Штрихкоды Цикл
		...
		ДанныеРазбора = ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкода(
			Штрихкод, ВидПродукции, КешДанныхРазбора);
		...
		Если ЗначениеЗаполнено(СтрокаДанныхТСД.ШтрихкодУпаковки) Тогда
			...
			ДанныеРазбора = ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкода(
				ШтрихкодУпаковки, ВидПродукции, КешДанныхРазбора);
	...
	Для Каждого КлючИЗначение Из РезультатыОбработки Цикл
		...
		Если РезультатОбработки.Свойство("ТребуетсяАвторизацияМОТП")
			И РезультатОбработки.ТребуетсяАвторизацияМОТП Тогда
...
Процедура ОбработатьПолученныеДанныеТСД() Экспорт
	...
	Если Не СтрокаШтрихкод.РезультатОбработки.ТребуетсяУточнениеДанных И Не СтрокаШтрихкод.Свойство("ТребуетсяОбработкаНаКлиенте") Тогда
...
&НаСервере
Процедура СпозиционироватьсяНаСервере(СтрокаШтрихкод, ДанныеШтрихкода, СоответствиеШтрихкодов)
...
Процедура УточнитьДанныеУПользователяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоПроизвольноеРедактированиеСуществующейСтроки Тогда
		ДобавитьСтрокуВТаблицуИзменений(ТаблицаИзмененийТабачнойПродукции, НайденнаяСтрокаДерева, -1);
	КонецЕсли;
	...
	Если ТаблицаИзмененийТабачнойПродукции.Количество() Тогда
		ОбработатьТаблицуИзмененийТабачнойПродукции(ЭтотОбъект);
	КонецЕсли;
...
Функция ДобавленнаяПотребительскаяУпаковка(Форма, Данные, Действие)
	...
	Если Не Контейнер.Свойство("ДобавленнаяПотребительскаяУпаковка") Тогда
		Контейнер.Вставить("ДобавленнаяПотребительскаяУпаковка", Новый Массив);
	КонецЕсли;
	
	ДобавленнаяПотребительскаяУпаковка = Контейнер.ДобавленнаяПотребительскаяУпаковка;
	
	Если Действие = "Очистить" Тогда
		ДобавленнаяПотребительскаяУпаковка.Очистить();
		Возврат Истина;
	КонецЕсли;
	...
	ИндексЭлемента = ДобавленнаяПотребительскаяУпаковка.Найти(КодМаркировки);
	...
	Если Действие = "Добавить" Тогда
		...
		ДобавленнаяПотребительскаяУпаковка.Добавить(КодМаркировки);
	...
	ИначеЕсли Действие = "Удалить" Тогда
		...
		ДобавленнаяПотребительскаяУпаковка.Удалить(ИндексЭлемента);
