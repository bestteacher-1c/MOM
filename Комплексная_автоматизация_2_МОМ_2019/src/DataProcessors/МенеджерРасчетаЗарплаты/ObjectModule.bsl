...
Процедура ДобавитьСтрокуВыявленныхНачислений(ИсточникНачислений, СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений, КатегорииПособий)
	
	Если ИсточникНачислений = "ПлановыеНачисления" Тогда
		ДобавитьСтрокуПлановыхНачислений(СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений, КатегорииПособий);
	ИначеЕсли ИсточникНачислений = "ЗарегистрированныеВидыВремени" Тогда
		ДобавитьСтрокуНачисленийПоВидамВремени(СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений);
	ИначеЕсли ИсточникНачислений = "ЗарегистрированныеПоказатели" Тогда	
		ДобавитьСтрокуНачисленийПоПоказателям(СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений);
	КонецЕсли;		
КонецПроцедуры	

Процедура ДобавитьСтрокуПлановыхНачислений(СвойстваПериодаВыявления, СтрокаПлановыхНачислений, СвойстваНачислений, КатегорииПособий)
	...
	ИначеЕсли НастройкиНачислений.РассчитыватьНачисления
		И СвойстваПериодаВыявления.ПрименятьПлановыеНачислений Тогда
...
Процедура ДобавитьСтрокуНачисленийПоВидамВремени(СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений)
...
Процедура ДобавитьСтрокуНачисленийПоПоказателям(СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений)
	...
	Если Не ЗначениеЗаполнено(СтрокаДобавляемыхНачислений.Организация) Тогда
		РассчитываемыеОрганизации = РассчитываемыеОрганизации();
		Для Каждого ОрганизацияНачисления Из РассчитываемыеОрганизации Цикл
			ДобавитьСтрокуНачислений(СтрокаДобавляемыхНачислений, ДатаНачала, ДатаОкончания, РазбиватьНачисление, ОрганизацияНачисления, СвойстваПериодаВыявления, СвойстваТекущегоНачисления);	
		КонецЦикла;
	Иначе
		ДобавитьСтрокуНачислений(СтрокаДобавляемыхНачислений, ДатаНачала, ДатаОкончания, РазбиватьНачисление, СтрокаДобавляемыхНачислений.Организация, СвойстваПериодаВыявления, СвойстваТекущегоНачисления);
	КонецЕсли;	
КонецПроцедуры	
...
Процедура ДобавитьСтрокуНачислений(ШаблонЗаполненияСвойств, ДатаНачала, ДатаОкончания, РазбиватьСтроку, ОрганизацияНачисления = Неопределено,  СвойстваПериодаВыявления, СвойстваНачисления)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		МодульУправленческаяЗарплата = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплата");
		Если МодульУправленческаяЗарплата.НачислениеНесоответствуетОрганизации(ШаблонЗаполненияСвойств, СвойстваНачисления, ОрганизацияНачисления) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРаспределениеРанееНачисленныхСумм()
	...
	ОбщегоНазначенияБЗК.ДобавитьИндексКоллекции(РаспределениеБазовыхНачислений, "ИдентификаторСтроки");
	...
	Для Каждого СтрокаНачисления Из Начисления Цикл
		...
		Для Каждого СтрокаБазы Из СтрокиБазы Цикл
			Отбор.ИдентификаторСтроки = СтрокаБазы.ИдентификаторСтрокиБазы;
			СтрокиРаспределения = РаспределениеБазовыхНачислений.НайтиСтроки(Отбор);
	...
	Если ИндексПервойСтроки <> Неопределено Тогда		
		Для Каждого СтрокаКадровойИстории Из ИнтервалыКадровойИстории Цикл
			Если СтрокаКадровойИстории.Организация <> Организация
				...
				Или СтрокаКадровойИстории.ДополнениеПервойСтрокиПериодомСреза
				Или (ПодразделенияСУчетомИерархииОтбор <> Неопределено И ПодразделенияСУчетомИерархииОтбор[СтрокаКадровойИстории.Подразделение] <> Истина) Тогда
...
Функция СтрокиНачисленийПоТребованиям()
	...
	ПоказателиКоэффициентыИндексации.Вставить(ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСреднегоЗаработка"), Истина);
	ПоказателиКоэффициентыИндексации.Вставить(ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСохраняемогоДенежногоСодержания"), Истина);
