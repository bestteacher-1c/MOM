...
Процедура ТекстЗапросаПараметрыУчетаОС_БУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ПараметрыАмортизацииБУ.СрокПолезногоИспользования                    КАК СрокИспользованияБУ,
...
Процедура ТекстЗапросаПараметрыУчетаПереданныхВАрендуОС(ТекстыЗапроса)
	...
	|			ПО (ПередачаОСАрендатору.Ссылка = ПереданныеВАренду.Регистратор)
	|				И (ПередачаОСАрендатору.Организация = ПереданныеВАренду.Организация)
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов КАК ВводОстатковВнеоборотныхАктивов
	|			ПО (ВводОстатковВнеоборотныхАктивов.Ссылка = ПереданныеВАренду.Регистратор)
	|				И (ВводОстатковВнеоборотныхАктивов.Организация = ПереданныеВАренду.Организация)
	|	ГДЕ
	...
	|						И НЕ ПередачаОСАрендатору.Ссылка ЕСТЬ NULL
	|				ИЛИ ПереданныеВАренду.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов
	|						И НЕ ВводОстатковВнеоборотныхАктивов.Ссылка ЕСТЬ NULL
...
Процедура ОбработатьПроблемныеВложенияВоВнеоборотныеАктивы(РезультатЗапроса, ПараметрыОбработчика, ИмяЭтапа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СуммаРегл < 0 Тогда
			ТекстСтроки = СтрШаблон(НСтр("ru = '- сумма БУ: %1 %2'"), Выборка.СуммаРегл, ПараметрыОбработчика.ВалютаРегл);
			СписокСтрок.Добавить(ТекстСтроки);
		КонецЕсли;
		
		Если (Выборка.СуммаРегл - Выборка.СуммаПР - Выборка.СуммаВР) < 0 Тогда
			ТекстСтроки = СтрШаблон(НСтр("ru = '- сумма НУ: %1 %2'"), Выборка.СуммаРегл - Выборка.СуммаПР - Выборка.СуммаВР, ПараметрыОбработчика.ВалютаРегл);
			СписокСтрок.Добавить(ТекстСтроки);
		КонецЕсли;
...
Процедура ПроверитьВложенияВоВнеоборотныеАктивы(РезультатЗапроса, ПараметрыОбработчика, ИмяЭтапа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ПроверитьСуммуВложений(НСтр("ru = '- сумма БУ'"), Выборка.СуммаРегл, Выборка.СуммаРегл24, ПараметрыОбработчика.ВалютаРегл, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = '- сумма ПР'"), Выборка.СуммаПР, Выборка.СуммаПР24, ПараметрыОбработчика.ВалютаРегл, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = '- сумма ВР'"), Выборка.СуммаВР, Выборка.СуммаВР24, ПараметрыОбработчика.ВалютаРегл, СписокСтрок);
...
Функция ПараметрыОбработчика(Параметры, АдресХранилища)
	...
	ПараметрыОбработчика.Вставить("ВалютаРегл", Строка(Константы.ВалютаРегламентированногоУчета.Получить()));
