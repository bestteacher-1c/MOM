...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСПереопределяемый.ОтображениеПредупрежденияПриРедактировании(
		Элементы.ВестиУчетМаркируемойПродукцииИСМП, НаборКонстант.ВестиУчетМаркируемойПродукцииИСМП);
...
&НаКлиенте
Процедура ВестиУчетМаркируемойПродукцииИСМППриИзменении(Элемент)
...
Процедура СписокМаркируемаяПродукцияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	СписокМаркируемаяПродукцияПередОкончаниемРедактированияНаСервере(ТекущиеДанные.ПолучитьИдентификатор(), Отказ);
...
&НаСервере
Процедура СписокМаркируемаяПродукцияПередОкончаниемРедактированияНаСервере(ИдентификаторСтроки, Отказ)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если НЕ Отказ Тогда
			ПриИзмененииРеквизитаСервер(Элементы.СписокМаркируемаяПродукция.Имя);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьСписокМаркируемойПродукции()
	...
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ НастройкиУчетаМаркируемойПродукции.ВестиУчетПродукции И Константы.ВестиУчетМаркируемойПродукцииИСМП
	|	КОНЕЦ КАК ВестиУчетПродукции,
	|	ВидыПродукции.Ссылка КАК ВидПродукции,
	|	ЕСТЬNULl(НастройкиУчетаМаркируемойПродукции.ДатаОбязательнойМаркировки, ДАТАВРЕМЯ(0001, 01, 01)) КАК ДатаОбязательнойМаркировки
	|ИЗ
	...
	|УПОРЯДОЧИТЬ ПО
	|	ВЫБОР
	|		КОГДА НастройкиУчетаМаркируемойПродукции.ДатаОбязательнойМаркировки ЕСТЬ NULL
	|			ТОГДА ДАТАВРЕМЯ(3000, 01, 01)
	|		ИНАЧЕ НастройкиУчетаМаркируемойПродукции.ДатаОбязательнойМаркировки
	|	КОНЕЦ ВОЗР
	|";
	...
	СписокМаркируемаяПродукция.Загрузить(Результат.Выгрузить());
...
Процедура СохранитьЗначениеРеквизита(РеквизитПутьКДанным, Результат)
	...
	Если КонстантаИмя = "ВестиУчетМаркируемойПродукцииИСМП" Тогда
		ЗаполнитьСписокМаркируемойПродукции();
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ВестиУчетМаркируемойПродукцииИСМП" Тогда
		СобытияФормИСПереопределяемый.ОтображениеПредупрежденияПриРедактировании(
			Элементы.ВестиУчетМаркируемойПродукцииИСМП, НаборКонстант.ВестиУчетМаркируемойПродукцииИСМП);
	КонецЕсли;
	
	ВестиУчетМаркируемойПродукцииИСМП = ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП");
	Элементы.СписокМаркируемаяПродукция.ТолькоПросмотр         = Не ВестиУчетМаркируемойПродукцииИСМП;
