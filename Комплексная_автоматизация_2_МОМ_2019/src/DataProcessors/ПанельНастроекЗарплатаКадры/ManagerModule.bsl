...
Процедура ЗаписатьНастройки(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.Свойство("КадровыйУчетИРасчетЗарплаты") Тогда
		...
		Настройка.ИспользоватьВзаиморасчетыПоПрочимДоходам = Параметры.КадровыйУчетИРасчетЗарплаты;
		Настройка.ИспользоватьВедомостиДляВыплатыПрочихДоходов = Параметры.КадровыйУчетИРасчетЗарплаты;
	...
	Если ПараметрыНастроек.ВыделятьНерабочиеДниОтдельнымВидомВремени Тогда
		...
		УчетРабочегоВремениРасширенный.ЗаполнитьГрафикиСотрудниковЗаГод(
			ТекущаяДатаСеанса(), Параметры.ВыделятьНерабочиеДниОтдельнымВидомВремени);
	КонецЕсли;
