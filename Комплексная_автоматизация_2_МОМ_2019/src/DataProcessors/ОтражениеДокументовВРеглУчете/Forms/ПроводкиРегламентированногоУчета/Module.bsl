...
Процедура ЗаписатьДвиженияЖурналаСФ(Команда)
	...
	Если Не Отказ Тогда
		Оповестить(СобытиеОповещения, , Документ);
		Элементы.СтраницаЖурналСФ.Заголовок = НСтр("ru = 'Журнал учета счетов-фактур'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура УстановитьДоступностьПриОткрытииФормы()
	...
	Элементы.РучнаяКорректировкаЖурналаСФ.Видимость = (ДокументПроводится ИЛИ ДокументПроведен) И ВозможнаКорректировкаЖурналаСФ
		И ПравоРучнойКорректировкиПроводок;
...
Процедура ПрочитатьДвиженияДокументаПоЖурналуСФ()
	...
	Если ВозможнаКорректировкаЖурналаСФ Тогда
		Объект.РучнаяКорректировкаЖурналаСФ = Документ.РучнаяКорректировкаЖурналаСФ;
...
Процедура ОбновитьДанныеРеквизитовДокумента()
	...
	Если Не Документ.Пустая() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОрганизацияПолучатель)
			ИЛИ Организация = ОрганизацияПолучатель Тогда
