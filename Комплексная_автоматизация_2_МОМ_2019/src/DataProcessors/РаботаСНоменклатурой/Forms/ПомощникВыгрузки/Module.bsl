...
Процедура ПараметрыНациональныйКаталогПриИзменении()
	ПараметрыНациональныйКаталогЗаполнены      = (ЗначениеЗаполнено(apikey) И ЗначениеЗаполнено(party_id));
	НадписьПараметрыДоступаНациональныйКаталог = СтрШаблон(НСтр("ru = 'Параметры доступа%1'"), ?(ПараметрыНациональныйКаталогЗаполнены, "", НСтр("ru = ' (не заполнены)'")));
	Элементы.НадписьПараметрыДоступаНациональныйКаталог.ЦветТекста = ?(ПараметрыНациональныйКаталогЗаполнены, ЦветОсновной, ЦветОшибки);
КонецПроцедуры
...
Процедура НоменклатураКВыгрузкеЗакрытие(РезультатЗакрытия, ДополнительныеПараметры) Экспорт 
	...
	ОбновитьСтатистикуИПроблемыБыстро();
КонецПроцедуры
...
Функция ПараметрыФормы(Знач Организация, Знач ЛимитЗаписей, Знач УникальныйИдентификатор)
	...
	Настройка.Вставить("ВыбраноКВыгрузке", НоменклатураКВыгрузке(Организация, ЛимитЗаписей));
	Настройка.Вставить("ПодсчетСтатистикиРезультатов", РаботаСНоменклатуройСлужебный.ПолучитьСтатистикуРезультатов(УникальныйИдентификатор, Организация));
	Настройка.Вставить("ЕстьПроблемыДлительнаяОперация", ДлительнаяОперацияЕстьПроблемыБыстро(УникальныйИдентификатор, Организация));
...
Процедура ПроверитьВыгрузкуНоменклатурыВФорме() Экспорт 
	Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
		ПрогрессВыгрузки = ПрогрессВыгрузки(Организация, ИдентификаторЗадания, РазделениеВключено, СоздатьЗадание);
...
&НаСервереБезКонтекста
Функция ПрогрессВыгрузки(Знач Организация, Знач ИдентификаторЗадания, Знач РазделениеВключено, Знач СоздатьЗадание) Экспорт 
...
&НаСервереБезКонтекста
Функция ПодсчетСтатистикиИПроблемыБыстро(Знач УникальныйИдентификатор, Знач Организация)
	Результат = Новый Структура;
	Результат.Вставить("ПодсчетСтатистикиРезультатов", РаботаСНоменклатуройСлужебный.ПолучитьСтатистикуРезультатов(УникальныйИдентификатор, Организация));
	Результат.Вставить("ЕстьПроблемыДлительнаяОперация", ДлительнаяОперацияЕстьПроблемыБыстро(УникальныйИдентификатор, Организация));
	Возврат Результат;
...
&НаКлиенте
Процедура ОбновитьСтатистикуИПроблемыБыстро()
	Если ВыгружатьВНациональныйКаталог Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ПодсчетСтатистикиИПроблемыБыстро(УникальныйИдентификатор, Организация));
		РаботаСНоменклатуройСлужебныйКлиент.ОбновитьСтатистикуРезультатов(ЭтотОбъект);
	Иначе 
		ЕстьПроблемыДлительнаяОперация = ДлительнаяОперацияЕстьПроблемыБыстро(УникальныйИдентификатор, Организация);
	КонецЕсли;
