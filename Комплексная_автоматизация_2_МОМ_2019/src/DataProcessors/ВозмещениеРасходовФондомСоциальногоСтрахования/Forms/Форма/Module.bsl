...
Процедура ВозмещенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = Элементы.ВозмещенияДокументОснование.Имя Тогда
...
Процедура ОбновитьНаСервере()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаУчетаСтраховыхВзносов КАК СчетаУчета
	|		ПО Расшифровка.СчетУчета = СчетаУчета.СчетУчета
	|ГДЕ
...
Процедура СформироватьДокументыОтражения()
	...
	СтрокиКОтражению = Объект.Возмещения.НайтиСтроки(Новый Структура("Пометка", Истина));
	Если СтрокиКОтражению.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Таблица", Объект.Возмещения.Выгрузить(СтрокиКОтражению));
