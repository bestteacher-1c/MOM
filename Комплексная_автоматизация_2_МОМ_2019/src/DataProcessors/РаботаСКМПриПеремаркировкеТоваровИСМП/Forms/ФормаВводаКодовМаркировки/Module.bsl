...
Процедура ОбработкаНавигационнойСсылкиНоменклатуры(НавигационнаяСсылка, СтруктураПолей, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "ВыбратьНоменклатуру" Тогда
		...
		ЭтотОбъект, Объект, СтандартнаяОбработка, ВидыПродукции, ОписаниеВыборЗначения);
...
Процедура РаспечататьНовыйКодЗавершение(ДанныеОтветаРезервированияИПечати, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОтветаРезервированияИПечати.РезультатРезервирования.Количество() Тогда
		...
		Если ДанныеОтветаРезервированияИПечати.СохраняемыеНастройки.ЗапомнитьВыбор Тогда
			ПередаваяемаяДляСохраненияСтруктура = Новый Структура();
			
			СохраняемыеДанные = ШтрихкодированиеИСКлиент.ИнициализацияСтруктурыДанныхСохраненногоВыбора();
			СохраняемыеДанные.ЗапомнитьВыбор = Истина;
			...
			ПередаваяемаяДляСохраненияСтруктура.Вставить("ДополнительныеПараметры", СохраняемыеДанные);
			
			ШтрихкодированиеИСМПКлиентСервер.ОбработатьСохраненныйВыборДанныхПоМаркируемойПродукции(
				ВладелецФормы, ПередаваяемаяДляСохраненияСтруктура);
...
&НаКлиенте
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ТекущаяСтраница) Экспорт
	
	ПараметрыСканирования   = ПараметрыСканированияШтрихкода(ТекущаяСтраница);
