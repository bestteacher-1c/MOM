...
Процедура СгенерироватьСерии(Команда)
	...
	Если НЕ ПроверкаИПодборПродукцииИСМПКлиентСервер.ТребуетсяУказаниеСерий(СоставУпаковки) Тогда
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	ВидимостьКолонкиШаблон = (ВидПродукции <> Перечисления.ВидыПродукцииИС.Табак
...
Процедура УвеличитьКоличествоОстаткаПоПродукции(СтрокаПродукции, РазупакованноеКоличество = 1)
	...
	Если СтрокиОстатка.Количество() Тогда
		...
		Если НЕ ПустаяСтрока(СтрокаПродукции.ХешСуммаУпаковки) Тогда
...
&НаКлиенте
Процедура УвеличитьКоличествоВУпаковках()
	...
	Если ЭтоАдресВременногоХранилища(АдресРезультата) Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
			Если ДанныеЗаполнения.Свойство("ОстаткиПродукции") Тогда
				...
				ПроверкаИПодборПродукцииИСМППереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, ОстаткиПродукции);
			КонецЕсли;
...
Процедура ПриИзмененииИсторическойУпаковкиНаСервере()
	...
	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПересчитатьНомераСтрок(СоставУпаковки);
	ПроверкаИПодборПродукцииИСМППереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, СоставУпаковки);
...
Процедура ДобавитьНовуюУпаковкуВИтоговоеДеревоУпаковок()
	...
	Для Каждого СтрокаСоставаУпаковки Из СоставУпаковки Цикл
		...
		Если СтрокаПродукции = Неопределено Тогда
			...
			ЗаполнитьЗначенияСвойств(
				СтрокаПродукции, СтрокаСоставаУпаковки, "Номенклатура,Характеристика,Серия,GTIN,Шаблон,ПредставлениеНоменклатуры");
			...
			СтрокаПродукции.Содержимое = ПредставлениеНоменклатуры(СтрокаПродукции.Номенклатура,
					СтрокаПродукции.Характеристика, СтрокаПродукции.Серия, ПредставлениеНоменклатуры);
			
			Если ЗначениеЗаполнено(СтрокаСоставаУпаковки.Номенклатура) Тогда
				НоменклатураУпаковки.Добавить(СтрокаСоставаУпаковки.Номенклатура);
			ИначеЕсли ЗначениеЗаполнено(СтрокаПродукции.Содержимое) Тогда
				НоменклатураУпаковки.Добавить(СтрокаПродукции.Содержимое);
			КонецЕсли;
	...
	СтрокаАгрегации.Содержимое = ПредставлениеСоставаУпаковки(НоменклатураУпаковки);
...
&НаСервереБезКонтекста
Функция ПредставлениеСоставаУпаковки(НоменклатураУпаковки)
	
	Возврат ИнтеграцияИС.ПредставлениеСоставаУпаковки(НоменклатураУпаковки);
...
Процедура СгенерироватьСерииСоставаУпаковки()
	...
	ДанныеДляГенерацииСерий = ПроверкаИПодборПродукцииИСМПКлиент.ДанныеДляГенерацииСерийПоПодобраннойПродукции(СоставУпаковки, ВидПродукции);
	СгенерироватьСерииНаСервере(ДанныеДляГенерацииСерий, ВидПродукции);
	ПроверкаИПодборПродукцииИСМПКлиент.ЗаполнитьСерииВПодобраннойМаркируемойПродукции(ЭтотОбъект, ДанныеДляГенерацииСерий, СоставУпаковки);
