...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьОтборСотрудниковСОшибками();
...
Процедура СотрудникиСОшибкамиПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Сотрудники = ОбщегоНазначения.ВыгрузитьКолонку(Строки.ПолучитьКлючи(), "Ссылка");
	
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудники, "ЗаявленияОПредоставленииСведенийОТрудовойДеятельности");
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		КадровыеДанныеСотрудника = КадровыеДанные.Найти(КлючИЗначение.Ключ);
		Если КадровыеДанныеСотрудника <> Неопределено
			И Не ЗначениеЗаполнено(КадровыеДанныеСотрудника.ЗаявленияОПредоставленииСведенийОТрудовойДеятельности) Тогда
...
&НаСервере
Процедура УстановитьОтборСотрудниковСОшибками()
	...
	Запрос.Текст =
		...
		|		ПО Сотрудники.ФизическоеЛицо = ЗаявленияОВеденииТрудовыхКнижек.ФизическоеЛицо
		|ГДЕ
		|	(Сотрудники.ФизическоеЛицо.ДатаРождения = ДАТАВРЕМЯ(1, 1, 1)
		|			ИЛИ Сотрудники.ФизическоеЛицо.ИНН = """"
		|			ИЛИ Сотрудники.ФизическоеЛицо.СтраховойНомерПФР = """"
		|			ИЛИ ЗаявленияОВеденииТрудовыхКнижек.Заявление ЕСТЬ NULL)";
	
	УстановитьПривилегированныйРежим(Истина);
	
	СписокСотрудников = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		СотрудникиСОшибками, "СписокСотрудников", СписокСотрудников, Истина);
