...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Протокол) Тогда
		...
		РасширениеФайлаПротокола = КонтекстЭДОСервер.РасширениеФайла(ИмяФайлаПротокола);
		РасширениеФайлаПротокола = нрег(РасширениеФайлаПротокола);
		НачалоПротокола = Лев(ТекстПротокола, 8192);
		...
		Если НЕ ЭтоПротоколHTML И СтрНайти(ТекстПротокола, "предупрежден") > 0 Тогда
