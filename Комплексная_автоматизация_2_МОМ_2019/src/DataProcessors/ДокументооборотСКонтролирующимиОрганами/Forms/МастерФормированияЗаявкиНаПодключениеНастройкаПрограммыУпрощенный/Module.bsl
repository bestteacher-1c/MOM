...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ПрограммноеЗакрытие И (ФлагСертификатУжеОтправленВПФР ИЛИ ФлагЗаявлениеУжеПредоставленоВПФР)
		И Элементы.Страницы.ТекущаяСтраница = Элементы.ОтправкаЗаявленияВПФР Тогда
	...
	Иначе
		...
		Если (ПодключитьЭДО ИЛИ ПереиздатьСертификатЭДО) 
			И НовыйСтатусДокумента = ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Одобрено")
			И Элементы.Страницы.ТекущаяСтраница = Элементы.УспешноеЗавершение Тогда
			...
			ИначеЕсли ПереиздатьСертификатЭДО Тогда
				ТекстПредупреждения = НСтр("ru = 'Закрыть мастер без обновления сертификата 1С-ЭДО?'");
			КонецЕсли;
