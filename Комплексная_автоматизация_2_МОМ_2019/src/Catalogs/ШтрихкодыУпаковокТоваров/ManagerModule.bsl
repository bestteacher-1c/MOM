...
Функция ПолучитьПоЗначениюШтрихкода(ЗначениеШтрихкода) Экспорт
	...
	Возврат ШтрихкодыУпаковок[0].Ссылка;
...
Функция СоздатьШтрихкодУпаковки(ДанныеШтрихкодаУпаковки, ВложенныеШтрихкоды = Неопределено, 
	...
	Если ЗначениеЗаполнено(НовыйЭлемент.ЗначениеШтрихкода) Тогда
		ПоискРанееСозданного = ПолучитьПоЗначениюШтрихкода(НовыйЭлемент.ЗначениеШтрихкода);
		
		Если ПоискРанееСозданного <> Неопределено Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Элемент справочника со значением штрихкода %1 уже существует.
					       |Необходимо использователь другое значение штрихкода.'"),
...
Функция ДанныеДляПечатиШтрихкодовУпаковокИСМП(
	...
	Для Каждого Штрихкод Из Штрихкоды Цикл
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Штрихкод, "ЗначениеШтрихкода,Номенклатура,Характеристика,ХешСуммаЗначенияШтрихкода,ТипШтрихкода,ТипУпаковки");
		...
		НоваяСтрока.ТипШтрихкода           = ЗначенияРеквизитов.ТипШтрихкода;
		НоваяСтрока.ЭтоКодМаркировки       = ЗначенияРеквизитов.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар;
	...
	Пока ВыборкаКоды.Следующий() Цикл
		...
		Если ВыборкаКоды.ЭтоКодМаркировки И ПроверятьДоступность И Не ВыборкаКоды.Доступно Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
				НСтр("ru='Код маркировки %1: будет напечатан кратко. Отсутсвует полный код в пуле кодов маркировки'"),
...
Функция СформироватьПечатнуюФормуШтрихкодыУпаковок(МассивОбъектов, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		РезультатРазбора = ШтрихкодированиеИССлужебный.РазобратьКодМаркировки(Выборка.Штрихкод, ВидыПродукции);
...
Функция ВозможныеПараметрыОбластиПечатиШтрихкодовПоТипуШтрихкода(ТипШтрихкодаУпаковки)
	...
	Для каждого ИдентификаторИтипКолонки Из КолонкиПечати Цикл
		...
		Если ТипШтрихкодаУпаковки = Перечисления.ТипыШтрихкодов.GS1_128
			...
			ИЛИ ТипШтрихкодаУпаковки = Перечисления.ТипыШтрихкодов.SSCC
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1(Параметры) Экспорт
	...
	|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
	|ГДЕ
	|	ШтрихкодыУпаковокТоваров.ХешСуммаЗначенияШтрихкодаGS1 = """"
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(СправочникОбъект.ХешСуммаЗначенияШтрихкодаGS1) Тогда
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(Выборка.Ссылка,, Параметры.Очередь);
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			Справочники.ШтрихкодыУпаковокТоваров.ЗаполнитьСвойствоХешСуммаЗначенияШтрихкодаGS1(СправочникОбъект);
...
Процедура ЗаполнитьСвойствоХешСуммаЗначенияШтрихкодаGS1(ДанныеШтрихкодаУпаковок, ПараметрыШтрихкода = Неопределено) Экспорт
	...
	ДанныеШтрихкодаУпаковок.ХешСуммаЗначенияШтрихкодаGS1 = ИнтеграцияИС.ХешированиеДанныхSHA256(ЗначениеШтрихкодаБезСкобок);
