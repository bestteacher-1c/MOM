...
Процедура ОбработатьШтрихкодыНаСервере(ДанныеШтрихкодов, КэшированныеЗначения)
	...
	Для Каждого Строка Из ДанныеШтрихкодов Цикл
		РазобраннаяСтрока = РезультатЧтенияШтрихкодов.ДанныеКодовМаркировки.Найти(Строка.Штрихкод,"Штрихкод");
		Если РазобраннаяСтрока=Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Упаковка или маркированный товар со штрихкодом ""%1"" не найден'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка.Штрихкод);
	...
	Для Каждого Строка Из РезультатЧтенияШтрихкодов.ДанныеКодовМаркировки Цикл
		Если ЗначениеЗаполнено(Строка.ШтрихкодУпаковки) Тогда
			ПрочитанныеШтрихкоды.Вставить(Строка.Штрихкод,Строка.ШтрихкодУпаковки);
		КонецЕсли;
	КонецЦикла;
