...
Процедура ПриИзмененииОтбора(УправляемаяФорма)
	...
	ОтоборПоФилиалу = Неопределено;
	ИспользоватьОтбор = Неопределено;
	
	КоллекцииЭлементовОтбора = Новый Массив;
	КоллекцииЭлементовОтбора.Добавить(УправляемаяФорма.Список.Отбор.Элементы);
	КоллекцииЭлементовОтбора.Добавить(УправляемаяФорма.Список.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы);
	
	Для Каждого КоллекцияЭлементовОбора Из КоллекцииЭлементовОтбора Цикл
		
		Для Каждого ЭлементОбора Из КоллекцияЭлементовОбора Цикл
			...
			Если ОтоборПоФилиалу <> Неопределено И ИспользоватьОтбор <> Неопределено Тогда
				
				Если ОтоборПоФилиалу.Использование <> ИспользоватьОтбор Тогда
					ОтоборПоФилиалу.Использование = ИспользоватьОтбор;
