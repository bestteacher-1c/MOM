...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.УчетныеПолитикиОрганизаций КАК УчетнаяПолитика
	|ГДЕ
	|	УчетнаяПолитика.ВариантУчетаОтложенногоНалога = ЗНАЧЕНИЕ(Перечисление.ВариантыУчетаОтложенногоНалога.ПустаяСсылка)
	|	ИЛИ НЕ УчетнаяПолитика.ИспользуетсяТрудНаемныхРаботниковНеТребуетОбновления
	|	ИЛИ (НЕ УчетнаяПолитика.РаздельныйУчетТоваровНеТребуетОбновления
	|		И УчетнаяПолитика.ОбъектНалогообложенияУСН = ЗНАЧЕНИЕ(Перечисление.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы))
	//-- НЕ УТ
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			//++ НЕ УТ
			Если ДанныеОбъекта.ВариантУчетаОтложенногоНалога.Пустая() Тогда
				Если ДанныеОбъекта.ВедетсяУчетПостоянныхИВременныхРазниц Тогда
					ДанныеОбъекта.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.ЗатратныйМетод;
				Иначе
					ДанныеОбъекта.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.Нет;
				КонецЕсли;
			КонецЕсли;
			Если Не ДанныеОбъекта.ИспользуетсяТрудНаемныхРаботников И Не ДанныеОбъекта.ИспользуетсяТрудНаемныхРаботниковНеТребуетОбновления Тогда
				ДанныеОбъекта.ИспользуетсяТрудНаемныхРаботников = Истина;
				ДанныеОбъекта.ИспользуетсяТрудНаемныхРаботниковНеТребуетОбновления = Истина;
			КонецЕсли;
			Если Не ДанныеОбъекта.РаздельныйУчетТоваров И Не ДанныеОбъекта.РаздельныйУчетТоваровНеТребуетОбновления
				И ДанныеОбъекта.ОбъектНалогообложенияУСН = Перечисления.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы Тогда
				ДанныеОбъекта.РаздельныйУчетТоваров = Истина;
				ДанныеОбъекта.РаздельныйУчетТоваровНеТребуетОбновления = Истина;
			КонецЕсли;
			//-- НЕ УТ
