...
Перем ВыделятьНерабочиеДниОтдельнымВидомВремени Экспорт;
Перем ВидыВремениВключаемыеВНорму;
Перем ПериодНерабочихДней;
Перем ДатыНерабочихПредпраздничныхДней;
...
Функция ЗаполнятьВремяЗаДень(ОбрабатываемаяДата, ВидДня, ПеренесенныеДни)
	...
	Если УчитыватьНерабочиеДни И ОплачиваемыйНерабочийДень(ОбрабатываемаяДата)Тогда
		Возврат ВыделятьНерабочиеДниОтдельнымВидомВремени;
	КонецЕсли;
	
	Если УчитыватьНерабочиеДни И ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Нерабочий Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ВидВремениПоВидуДня(ОбрабатываемаяДата, ВидВремениПоНастройкам)
	...
	Если ОплачиваемыйНерабочийДень(ОбрабатываемаяДата)
		И ВыделятьНерабочиеДниОтдельнымВидомВремени
...
Функция ОплачиваемыйНерабочийДень(ОбрабатываемаяДата)
	...
	Возврат ОбрабатываемаяДата >= ПериодНерабочихДней.ДатаНачала И ОбрабатываемаяДата <= ПериодНерабочихДней.ДатаОкончания;
...
Функция ПредпраздничныйДень(ОбрабатываемаяДата, ВидДня)
	...
	Возврат ДатыНерабочихПредпраздничныхДней.Найти(ОбрабатываемаяДата) <> Неопределено;
...
ПериодНерабочихДней = УчетРабочегоВремениРасширенный.ПериодОплатыНерабочихДней2020();
