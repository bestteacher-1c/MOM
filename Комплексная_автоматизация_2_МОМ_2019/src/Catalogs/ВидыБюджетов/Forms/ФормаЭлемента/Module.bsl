...
Процедура Сгруппировать(Команда)
	...
	Если Таблица = Неопределено Тогда
		Таблица = ФинансоваяОтчетностьКлиент.КорневойЭлементЭлементаФормы(СтрокаПроверки, ВидЭлемента("ТаблицаПоказателиВКолонках"));
		Если Таблица = Неопределено Тогда
			Таблица = ФинансоваяОтчетностьКлиент.КорневойЭлементЭлементаФормы(СтрокаПроверки, ВидЭлемента("ТаблицаПоказателиВСтроках"));
		КонецЕсли;
	КонецЕсли;
	
	Если Таблица <> Неопределено И ФинансоваяОтчетностьКлиент.КорневойЭлементЭлементаФормы(СтрокаПроверки, ВидЭлемента("Строки")) = Неопределено Тогда
		ЕстьОшибки = Истина;
	Иначе
	...
	Для Сч = 0 По УпорядоченныеСтроки.ВГраница() Цикл
		...
		Для Каждого Строка Из СтрокаДерева.ПолучитьЭлементы() Цикл
			Если УпорядоченныеСтроки.Найти(Строка.ПолучитьИдентификатор()) = Неопределено Тогда
				...
				ФинансоваяОтчетностьКлиент.УстановитьНовогоРодителяЭлементаФормы(Строка, МладшаяСтрока, ПараметрыУстановкиРодителя);
			КонецЕсли;
		КонецЦикла;
...
Процедура ЭлементыОтчетаПередУдалениемНаСервере(СписокЭлементов);
	...
	Для Каждого ЭлементСпискаЗначений Из СписокЭлементов Цикл
		...
		ТаблицаСложная = ФинансоваяОтчетностьСервер.КорневойЭлемент(ТекущаяСтрока, Перечисления.ВидыЭлементовФинансовогоОтчета.ТаблицаСложная);
		Если ТаблицаСложная = Неопределено Тогда
			Таблица = ФинансоваяОтчетностьСервер.КорневойЭлемент(ТекущаяСтрока, Перечисления.ВидыЭлементовФинансовогоОтчета.ТаблицаПоказателиВКолонках);
			Если Таблица = Неопределено Тогда
				Таблица = ФинансоваяОтчетностьСервер.КорневойЭлемент(ТекущаяСтрока, Перечисления.ВидыЭлементовФинансовогоОтчета.ТаблицаПоказателиВСтроках);
			КонецЕсли;
		Иначе
	...
	Иначе
		...
		Если НайденныеСтроки <> Неопределено Тогда
			Для Каждого СтрокаНедопустимыхСочетаний Из НайденныеСтроки Цикл
				Элемент = ФинансоваяОтчетностьКлиент.КорневойЭлементЭлементаФормы(СтрокаЭлементаОтчета, СтрокаНедопустимыхСочетаний.Ключ);
