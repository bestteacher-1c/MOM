...
Функция ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, ДополнительныеПараметры)
	...
	ИменаИзмеренийНачислений = "Сотрудник,ВидРасчета";
	ИменаИзмеренийУдержаний = "ФизическоеЛицо,ВидРасчета";
	...
	ИменаИзмеренийВыплачено = "ФизическоеЛицо,ВидРасчета";
	Если НастройкиВывода.ВыводитьПериодыНачислений Тогда
	...
	Для каждого СтрокаГоловногоСотрудника Из СтрокиГоловныхСотрудников Цикл
		...
		Для каждого СтрокаСотрудника Из СтрокаГоловногоСотрудника.Строки Цикл
			...
			Если РабочееМесто.СтрокиНачислений.Количество() > 0 Тогда
					...
					Для Каждого ОписаниеСторноСтроки Из СторноСтроки Цикл
						...
						Если СтрокиРегистратора <> Неопределено Тогда
							...
							Если СтрокаВидаРасчета <> Неопределено Тогда
								ЗаполнитьЗначенияСвойств(СторноСтрока, СтрокаВидаРасчета, "ПериодДействия,ПериодДействияПредставление,ПриоритетВидаРасчета");
							КонецЕсли;
				...
				СвернутьСтрокиВидовРасчета(РабочееМесто.СтрокиНачислений, "ФизическоеЛицо,Группа,Сотрудник,ВидРасчета,ПриоритетВидаРасчета", СуммируемыеКолонки, НастройкиВывода, Ложь);
			...
			Для Каждого СтрокаОтчетаНачислено Из СтрокиОтчетаНачислено Цикл
				...
				СтрокаДанныхРасчетногоЛистка.ПриоритетВидаРасчета =
					ПриоритетПриоритетВидаРасчетаДанныхРасчетныхЛистков(СтрокаДанныхРасчетногоЛистка.ПриоритетВидаРасчета);
			...
			Если СтрокаГруппы <> Неопределено Тогда
				...
				СвернутьСтрокиВидовРасчета(КоллекцииРазделов.Удержано, "ФизическоеЛицо,Группа,Сотрудник,ВидРасчета,ПриоритетВидаРасчета", "Сумма", НастройкиВывода, Истина);
	...
	ПустыеПараметрыЗаполнения = Новый Структура("ПредставлениеРабочегоМеста,ВидРасчета,ПериодДействия,ОтработаноДней,
...
Функция ПриоритетПриоритетВидаРасчетаДанныхРасчетныхЛистков(ПриоритетВидаРасчета)
	
	Если ЗначениеЗаполнено(ПриоритетВидаРасчета) Тогда
		
		Если ПриоритетВидаРасчета > 100000000000000000000 Тогда
			Возврат Цел(ПриоритетВидаРасчета / 100000000000000000000) * 100;
		КонецЕсли;
		...
		Если Не НастройкиВывода.ВыводитьВсеПоказателиНачисленийИУдержаний Тогда
			...
			Если НастройкиВывода.ВыводитьПериодыНачислений Тогда
				...
				Если ЗначениеЗаполнено(СтрокаВидаРасчета.ПриоритетВидаРасчета)
					И СтрокаВидаРасчета.ПриоритетВидаРасчета > 1000000000000 Тогда
					
					СтрокаВидаРасчета.ПриоритетВидаРасчета = Цел(СтрокаВидаРасчета.ПриоритетВидаРасчета / 1000000000000) * 1000000000000;
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаВидаРасчета.ПриоритетВидаРасчета)
					И СтрокаВидаРасчета.ПриоритетВидаРасчета > 100000000000000000000 Тогда
					
					СтрокаВидаРасчета.ПриоритетВидаРасчета = Цел(СтрокаВидаРасчета.ПриоритетВидаРасчета / 100000000000000000000) * 100000000000000000000;
...
Функция ПредставлениеРегистраторовВзаиморасчетов(Регистратор, ВидВзаиморасчетов, СвойстваРегистратора)
	...
	НомерДата = "вед. № " + ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СвойстваРегистратора.Номер, Истина, Истина)
		+ " "  + НСтр("ru='от'") + " "
...
Процедура ДобавитьСтрокиРаздела(СтрокиРаздела, СтрокиДанных, НастройкиВывода, СоответствиеСтрокРазделаСтрокамДанных, СоответствиеПользовательскихПолей, ИменаИзмерений, Макет, ИмяВертикальнойОбласти, СдельныйЗаработокСотрудников = Неопределено, ВывестиСдельныйЗаработокПоФизическомуЛицу = Ложь)
	...
	Для Каждого СтрокаДанных Из СтрокиДанных Цикл
		...
		Если ВидРасчета <> СтрокаДанных.ВидРасчета Тогда
		...
		Если СтрокаВидаРасчета Тогда
			...
			Если ВыводитьОтработанноеОплаченноеВремя Тогда
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ОплаченоДней) И СтрокаДанных.ОплаченоДней <> 0 Тогда
					СтрокаОтчета.ОплаченныеДниЧасы = Формат(СтрокаДанных.ОплаченоДней, "ЧДЦ=2; ЧН=") + " " + НСтр("ru='дн'") + ".";
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ОплаченоЧасов) И СтрокаДанных.ОплаченоЧасов <> 0 Тогда
					...
					Если Не ПустаяСтрока(СтрокаОтчета.ОплаченныеДниЧасы) Тогда
						СтрокаОтчета.ОплаченныеДниЧасы = СтрокаОтчета.ОплаченныеДниЧасы + " " + НСтр("ru='и'") + " ";
					КонецЕсли;
					
					СтрокаОтчета.ОплаченныеДниЧасы = Формат(СтрокаДанных.ОплаченоЧасов, "ЧДЦ=2; ЧН=") + " " + НСтр("ru='чс'") + ".";
...
Процедура ОтключитьНеИспользуемыеПоляРасчетногоЛистка(НастройкиОтчета, НастройкиВывода)
	...
	Если Не НастройкиВывода.ВыводитьОснованияНачисленийИУдержаний Тогда
