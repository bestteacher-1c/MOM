...
Функция ВыгрузитьНоменклатуру(Запрос, Параметры)
	...
	Пока Истина Цикл
		...
		Если ОбработаноПорцийДанных > 0 И ОбработаноПорцийДанных % КоэффициентПорцииПрогресса = 0 Тогда
			СообщитьПрогрессВыгрузкиПоОрганизации(Организация, СтрШаблон(НСтр("ru = 'Выгружено %1'"), Счетчик));
	...
	УспешноОтправлены = ОтправитьДанныеНоменклатурыНаСервер(ПараметрыТелаЗапроса, ПакетыДанных);
...
Функция ОтправитьДанныеНоменклатурыНаСервер(ПараметрыТелаЗапроса, Пакеты)
