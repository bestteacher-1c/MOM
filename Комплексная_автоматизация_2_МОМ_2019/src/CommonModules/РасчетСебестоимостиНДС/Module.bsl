...
// Этап "ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов"
//
// Параметры:
//	ПараметрыРасчета - Структура - параметры расчета себестоимости
//
Процедура ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов(ПараметрыРасчета) Экспорт
	
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов");
	...
	// 5. Помещение данных в таблицу-приемник и очистка вспомогательных временных таблиц.
	// 	- формирует ВТ ПартииНДСПоПартиямПрочихРасширенная
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗавершитьРаспределениеПартий(ПараметрыРасчета);
	
	// 6. Инициализация структуры данных для расчета
	//	- формирует структуру ПараметрыРасчета.РаспределениеПартий.
	...
	// 7. Получение исходных данных для трансляции
	// 	- формирует временную таблицу Данные.
	ПолучитьИтоговыеДанныеДляПартийНДС2_4ПоПартиямПрочихРасходов(ПараметрыРасчета);
	
	// 8. Помещение данных в таблицу-приемник и очистка вспомогательных временных таблиц.
	// 	- формирует движения по регистру ДетализацияПартийТоваровДляНДСиУСН2_4.
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗавершитьРаспределениеПартий(ПараметрыРасчета);
...
Функция ОписаниеНезаписываемыхПартийНДС(ПараметрыРасчета)
	...
	НезаписываемыеТипыЗаписей.Вставить(Перечисления.ТипыЗаписейПартий.ОстатокДляРаспределения, Истина);
...
Функция ТекстДетальныеПриходыСебестоимостиТоваров() // использует ПриходыТоваровНДС
	Возврат
		...
		|	(ВЫБОР
		|		КОГДА ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ДопРасходы)
...
Процедура ПроверитьКорректностьДанныхДляПартийНДС2_4(ПараметрыРасчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстИнформации = НСтр("ru='Не удалось определить стоимость некоторых позиций продукции для целей раздельного учета по налогообложению НДС'");
		
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='По организации ""%1"" за период %2 необходимо указать цену плановой стоимости материалов и работ по номенклатуре ""%3"" с характеристикой ""%4""'"),
			Выборка.Организация,
...
Функция ОписаниеДвиженийПартийНДС2_4ПоПартиямПрочихРасходов(ПараметрыРасчета)
	...
	ОписаниеДвижений = РасчетСебестоимостиПрикладныеАлгоритмы.ОписаниеДвижений();
	ОписаниеДвижений.Вставить("Контекст", "ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов");
	ОписаниеДвижений.Вставить("ИмяВременнойТаблицы", "ПартииНДСПоПартиямПрочихРасширенная");
	ОписаниеДвижений.Вставить("ПоляРасчета", РасчетСебестоимостиПрикладныеАлгоритмы.ПереченьПолей(КолонкиТаблицыРаспределения));
	ОписаниеДвижений.Вставить("КлючиСравнения",	ПоляПотребленийПартииНДС2_4(ПараметрыРасчета));
...
Процедура ПолучитьИтоговыеДанныеДляПартийНДС2_4ПоПартиямПрочихРасходов(ПараметрыРасчета)
	
	РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьДанныеЭтапаРасчета(
		ПараметрыРасчета,
		ТекстЗапросаИтоговыйДляПартийНДС2_4ПоПартиямПрочихРасходов(ПараметрыРасчета));
...
Функция ТекстЗапросаИтоговыйДляПартийНДС2_4ПоПартиямПрочихРасходов(ПараметрыРасчета = Неопределено) Экспорт
	
	ТекстЗапроса = ТекстИтоговыеДанныеДляПартийНДС2_4ПоПартиямПрочихРасходов();
...
Функция ТекстИнициализацияПартийНДС2_4ПоПартиямПрочихРасходов()
	Возврат
		...
		|		И ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) КАК Т
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСтатьиПроизводственныхЗатрат КАК Статьи
		|		ПО Т.СтатьяРасходов = Статьи.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	И ВЫБОР КОГДА НЕ Т.ПартияПроизводства.ВидДеятельностиНДС ЕСТЬ NULL
		|			ТОГДА Т.ПартияПроизводства.ВидДеятельностиНДС
		|		КОГДА Т.ЗаказНаПроизводство ССЫЛКА Документ.ВыпускПродукции
		|			ТОГДА ВЫРАЗИТЬ(Т.ЗаказНаПроизводство КАК Документ.ВыпускПродукции).ВыпускПодДеятельность
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
...
Функция ТекстПартииПрочихРасходовДляРаспределенияПоНЗПРегл()
	Возврат
		...
		|	ДД.НаправлениеДеятельности,
		|
		|	База.СтатьяКалькуляции,
		|	База.ГруппаПродукции,
		|	База.ПравилоОтнесенияНаВыпуск,
		|
		|	ДД.СтоимостьБезНДС,
		|	ДД.НДС,
		|	0 КАК СтоимостьБезНДСУпр,
...
Функция ТекстСписаниеНаВыпускПостатейныеРасходы()
	Возврат
		...
		|	ЛОЖЬ КАК Сторно,
		|	0 КАК Приоритет,
		|	СУММА(ЕСТЬNULL(Выпуски.Знаменатель, 0)) 	КАК Знаменатель,
		|	0											КАК ЗнаменательДляИсточников,
		|	ЛОЖЬ 										КАК РасчетЗавершен,
		|	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) 				 КАК Номенклатура,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика,
		...
		|	ДД.НаправлениеДеятельности,
		|	ЕСТЬNULL(Выпуски.ВидДеятельностиНДС, НЕОПРЕДЕЛЕНО),
		|	ЕСТЬNULL(КорАналитикиУчетаПартий.Ссылка, ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПартий.ПустаяСсылка)),
		|	ДД.Подразделение,
...
Функция ТекстИтоговыеДанныеДляПартийНДС2_4ПоПартиямПрочихРасходов()
	Возврат
		"ВЫБРАТЬ
		|	ДД.ЗапросИсточник,
		|	ДД.ТипЗаписи,
		...
		|//ВоВременнуюТаблицу
		|ИЗ
		|	ПартииНДСПоПартиямПрочихРасширенная КАК ДД
		|";
КонецФункции
...
Процедура ЗаполнитьРасчетнуюПартию(ПараметрыРасчета, Контекст, РасчетнаяПартия, Расход, Приход, ПартияЗаполнена) Экспорт
	...
	//++ НЕ УТ
	 ИЛИ Контекст = "ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов"
	//-- НЕ УТ
	Тогда
		// Этап 11.2 или 11.3
...
Процедура ДополнитьЭтапыСРаспределениемПартий(СписокЭтапов) Экспорт
	...
	//++ НЕ УТ
	СписокЭтапов.Добавить("ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов");
	//-- НЕ УТ
...
Процедура ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета, ТекстЗапроса) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ИмяЭтапа = "ПодготовкаДанныхДляУчетаНДСиУСН2_4ПоПартиямПрочихРасходов" Тогда
