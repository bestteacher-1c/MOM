...
Процедура ДополнитьТаблицуПодобраннойМаркируемойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ВидПродукции) Экспорт
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция Тогда
		...
		Если ТипЗнч(ДанныеДокумента.ПроверяемыйДокумент) = Тип("ДокументСсылка.МаркировкаТоваровИСМП") Тогда
			...
			ОписаниеСвойства = Новый Структура("Имя, ПрисутствуетВДанныхДокумента", "ИдентификаторПроисхожденияВЕТИС", Истина);
			СвойстваВидаПродукции.Добавить(ОписаниеСвойства);
	...
	Если ПодобраннаяМаркируемаяПродукцияНовая.Количество() > 0 Тогда
