...
Функция НовыйКонтекстРасчета(Организация, КонецПериода, ДатаПодписи = '0001-01-01', КППОбособленногоПодразделения = "", Периодичность = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(КонтекстРасчета.Периодичность) Тогда
		ПорядокУплатыАвансовПоНалогуНаПрибыль = РегистрыСведений.УчетнаяПолитикаОрганизаций.ПараметрыУчетнойПолитики(Организация, КонецПериода).ПорядокУплатыАвансовПоНалогуНаПрибыль;
...
Процедура ПолучитьВыручкаОбычныеВидыДеятельности(ПроцессорПолученияДанных)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Регистратор, Счет В (&СчетаВыручки), &СубконтоНоменклатурныеГруппыНоменклатура, Организация В (&Организации), , ) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для Каждого Задача Из ПроцессорПолученияДанных.Задачи Цикл
		...
		Для Каждого Счет Из СчетаВыручки Цикл
			...
			ИначеЕсли Задача.ОтборДанных.Раздел = "УслугиБезНастройки" Тогда
				...
				|ГДЕ
				|	Выручка.Счет = &Счет
				|	И РеализацияПродукции.Номенклатура ЕСТЬ NULL
				|	И Выручка.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))";
