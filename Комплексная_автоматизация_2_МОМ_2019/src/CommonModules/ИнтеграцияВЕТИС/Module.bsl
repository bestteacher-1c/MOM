...
Функция ДатаФормированияРезультатаПоСсылке(ДокументСсылка, ПараметрыОбмена)
	...
	Если ХСПредприятияСОшибкамиСервиса <> Неопределено Тогда
		...
		Для Каждого СтрокаТЧ Из ХСПредприятияСОшибкамиСервиса Цикл
			...
			Для Каждого ЭлементОчереди Из СтрокаТЧ.ЭлементыОчереди Цикл
				...
				ДатаФормированияРезультата = МестноеВремя(ИнтеграцияВЕТИС.ДатаФормированияРезультата(ЭлементОчереди, ПараметрыОбмена, Неопределено));
				...
				ДополнительныеПараметры.Вставить("НачалоПериода",         ДатаФормированияРезультата - 60);
				ДополнительныеПараметры.Вставить("КонецПериода",          ДатаФормированияРезультата + 60);
...
Функция ЗапросДляПроверкиКорректностиОформленияXML(ДокументСсылка) Экспорт
	...
	ДатаФормированияРезультата = ДатаФормированияРезультатаПоСсылке(ДокументСсылка, ПараметрыОбмена);
	Если ДатаФормированияРезультата <> Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("НачалоПериода",         МестноеВремя(ДатаФормированияРезультата) - 60);
		ДополнительныеПараметры.Вставить("КонецПериода",          МестноеВремя(ДатаФормированияРезультата) + 60);
...
//  Отбор           - Структура - параметры поиска документа.
...
Функция ДокументДляВосстановления(ДатаЭлемента, Отбор, ПараметрыОбмена, ТипДокумента, БезОграниченияДатыНазад = Ложь) Экспорт
	...
	Для Каждого ЭлементДанных Из НайденныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(ЭлементДанных.ДатаСоздания) Тогда
			ЭлементДанных.ДатаСоздания = МестноеВремя(ДатаФормированияРезультатаПоСсылке(ЭлементДанных.Документ, ПараметрыОбмена));
		КонецЕсли;
