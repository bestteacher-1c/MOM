...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление графиков работы сотрудников по производственному календарю 2020 года.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.13.174";
	Обработчик.Процедура = "УчетРабочегоВремениРасширенный.ОбновитьГрафикиРаботыСотрудников2020";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("931429f5-f5dd-4bd5-bcb2-3b31362fd656");
	Обработчик.Комментарий = НСтр("ru = 'Обновление графиков работы сотрудников по производственному календарю 2020 года.'");
...
Процедура СоздатьВТДанныеПроизводственногоКалендаряПоСотрудникам(МенеджерВременныхТаблиц, ОписаниеФильтра, ИмяВТРезультат = "ВТДанныеПроизводственногоКалендаряПоСотрудникам") Экспорт
	...
	|						И ДанныеПроизводственногоКалендаря.Дата МЕЖДУ &ДатаНачалаОплатыНерабочих И &ДатаОкончанияОплатыНерабочих)
	|				ТОГДА 1
	|			ИНАЧЕ 0
	...
	|					ИЛИ ДанныеПроизводственногоКалендаря.Дата В (&ДатыНерабочихПредпраздничныхДней)
	|				ТОГДА 7
	...
	ПериодОплатыНерабочихДней = ПериодОплатыНерабочихДней2020();
	ДатыНерабочихПредпраздничныхДней = ДатыНерабочихПредпраздничныхДней2020();
...
Функция ДанныеПроизводственныхКалендарейСводно(ПериодыКалендари) Экспорт
	...
	|			ИНАЧЕ 0
	|		КОНЕЦ + ВЫБОР
	|			КОГДА ДанныеПроизводственногоКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Нерабочий)
	|					И (ДанныеПроизводственногоКалендаря.Дата МЕЖДУ &ДатаНачалаОплатыНерабочих И &ДатаОкончанияОплатыНерабочих)
	|					И (ДанныеПроизводственногоКалендаря.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&ФильтрМесяц_, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ФильтрМесяц_, МЕСЯЦ))
	|				ТОГДА 1
	|			ИНАЧЕ 0
...
Функция ПериодОплатыНерабочихДней2020() Экспорт
	...
	Если ОсновнойПроизводственныйКалендарь = Неопределено Тогда
		Возврат Новый СтандартныйПериод('20200330', '20200430');
	КонецЕсли;
	
	ПериодыНерабочихДней = КалендарныеГрафики.ПериодыНерабочихДней(ОсновнойПроизводственныйКалендарь, Новый СтандартныйПериод);
	Если ПериодыНерабочихДней.Количество() = 0 Тогда
		Возврат Новый СтандартныйПериод;
	КонецЕсли;
	
	НачалоПериода = ЗарплатаКадрыПериодическиеРегистры.МаксимальнаяДата();
	ОкончаниеПериода = '00010101';
	
	Для Каждого ПериодНерабочихДней Из ПериодыНерабочихДней Цикл
		НачалоПериода = Мин(НачалоПериода, ПериодНерабочихДней.Период.ДатаНачала);
		ОкончениеПериода = Макс(ОкончаниеПериода, ПериодНерабочихДней.Период.ДатаОкончания);
	КонецЦикла;
	
	Возврат Новый СтандартныйПериод(НачалоПериода, ОкончениеПериода);
	
КонецФункции

Функция ДатыНерабочихПредпраздничныхДней2020() Экспорт
	
	Даты = Новый Массив;
	Даты.Добавить('20200430');
...
Процедура ЗаполнитьГрафикиСотрудниковЗаГод(ДатаОбновления, Знач ВыделятьНерабочиеДниОтдельнымВидомВремени = Неопределено) Экспорт

	Если ВыделятьНерабочиеДниОтдельнымВидомВремени = Неопределено Тогда
	...
	Пока ВыборкаГрафиков.Следующий() Цикл
		Попытка
			ГрафикОбъект = ВыборкаГрафиков.Ссылка.ПолучитьОбъект();
			ГрафикОбъект.ВыделятьНерабочиеДниОтдельнымВидомВремени = ВыделятьНерабочиеДниОтдельнымВидомВремени;
			ДанныеГрафика = ГрафикОбъект.НовыйДанныеГрафика();
	...
	Для Каждого СтрокаНабора Из НаборЗаписейУчетаВремени Цикл
		...
		Если ПереходящаяЧастьПредыдущейСмены Тогда 
			...
			Иначе
				...
				ИмяУстанавливаемогоПараметра = "УстановленДень" + Строка(НомерДня);
			КонецЕсли;	
		ИначеЕсли НомерДня = День(КонецМесяца(Месяц)) Тогда
