...
Функция РазобратьКодМаркировки(Знач ДанныеДляРазбора, ВидыПродукции = Неопределено, ПримечаниеКРезультатуРазбора = Неопределено) Экспорт
	...
	Для Каждого ШаблонКодаМаркировки Из НайденныеШаблоны Цикл
		...
		ИначеЕсли ФильтрПоВидуПродукции.Использовать Тогда
			ВидПродукцииНайден = Ложь;
			Для Каждого ТекущийВидПродукции Из ФильтрПоВидуПродукции.ВидыПродукции Цикл
...
// 	ПараметрыНормализацииКМ  - См. ШтрихкодированиеИССлужебный.ПараметрыНормализацииКодаМаркировки.
