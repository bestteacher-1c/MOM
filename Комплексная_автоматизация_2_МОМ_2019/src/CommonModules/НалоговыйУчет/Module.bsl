...
Функция ТребуетсяНалоговыйУчет() Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ПлательщикНалогаНаПрибыль");
...
Функция ВыручкаОтРеализации(Период, Организация) Экспорт
	...
	|
	|ИМЕЮЩИЕ
	|	ГОД(ВозвратТоваровОтКлиентаТовары.Ссылка.Дата) > МАКСИМУМ(ГОД(ЕСТЬNULL(ДанныеРеализаций.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1))))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	|;
	...
	|			ПО ВыручкаИСебестоимостьПродаж.Регистратор = Корректировки.Регистратор
	|	ГДЕ
	|		ВыручкаИСебестоимостьПродаж.АналитикаУчетаПоПартнерам В
	|				(ВЫБРАТЬ
	|					КлючиАналитикиУчетаПоПартнерам.Ссылка
	|				ИЗ
	|					КлючиАналитикиУчетаПоПартнерам)
	|		И НЕ ВыручкаИСебестоимостьПродаж.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		И НЕ ВыручкаИСебестоимостьПродаж.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.АгентскаяУслуга)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Функция НалоговаяСтавкаРегиональныйБюджет(Период, Организация, ФормаПредставленияРезультата = "Доля") Экспорт
	...
	ЗапросПоВсемОрганизациям = Новый Запрос;
	ЗапросПоВсемОрганизациям.УстановитьПараметр("Период", Период);
	ЗапросПоВсемОрганизациям.Текст =
	"ВЫБРАТЬ
	...
	Если ПолучитьФункциональнуюОпцию("ПрименяютсяРазныеСтавкиНалогаНаПрибыль") Тогда
		...
		ЗапросПоСубъектамРФ = Новый Запрос;
		ЗапросПоСубъектамРФ.УстановитьПараметр("Период",      Период);
		ЗапросПоСубъектамРФ.УстановитьПараметр("Организация", Организация);
...
Функция РегистрацияОрганизацииВНалоговомОргане(Организация, Период) Экспорт
	Возврат Справочники.Организации.РегистрацияВНалоговомОргане(Организация, Период);
КонецФункции
...
Функция ЭтоПроводкаПоНачислениюНДС(Проводка) Экспорт
	...
	Для Каждого Субконто Из Проводка.СубконтоДт Цикл
		Если ТипЗнч(Субконто.Значение) = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов")
			И (Субконто.Значение.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.КурсовыеРазницыПоРасчетамВУЕ
				ИЛИ Субконто.Значение.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.РасходыПоПередачеТоваровБезвозмездноИДляСобственныхНужд)Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
//
// Возвращаемое значение:
//	ДоляТранспортныхРасходов - Число - доля транспортных расходов для организации (если передан массив, то 0).
//
Функция ДоляТранспортныхРасходовТекущегоМесяца(Период, Организация, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	|	ИНАЧЕ
	|		Продажи.АналитикаУчетаНоменклатуры.Номенклатура.ГруппаФинансовогоУчета
	...
	|		&КонецПериода,
	|		,
	|		ЕСТЬNULL(ВидЗапасов.ТипЗапасов,ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)) = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
	|			И АналитикаУчетаПоПартнерам.Организация В (ВЫБРАТЬ Т.Организация КАК Организация ИЗ Организации КАК Т)
	|			И РазделУчета В (ВЫБРАТЬ Т.РазделУчета ИЗ РазделыУчетаТоваров КАК Т)
	|	) КАК Продажи
	|
	|СГРУППИРОВАТЬ ПО
	|	Продажи.АналитикаУчетаПоПартнерам.Организация,
	|	Продажи.АналитикаУчетаНоменклатуры.МестоХранения,
	|	ВЫБОР КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета ТОГДА
	|		Продажи.ВидЗапасов.ГруппаФинансовогоУчета
	|	ИНАЧЕ
	|		Продажи.АналитикаУчетаНоменклатуры.Номенклатура.ГруппаФинансовогоУчета
	...
	|	ИНАЧЕ
	|		Товары.АналитикаУчетаНоменклатуры.Номенклатура.ГруппаФинансовогоУчета
	|	КОНЕЦ КАК ГруппаФинансовогоУчета,
	...
	|				ГДЕ
	|					Продажи.Склад = НастройкиИсключений.Склад
	|					И Продажи.ГруппаФинансовогоУчета = НастройкиИсключений.ГруппаФинансовогоУчета
	|					И Продажи.Организация = НастройкиИсключений.Организация
	...
	|				ГДЕ
	|					Продажи.ГруппаФинансовогоУчета = НастройкиПоУмолчанию.ГруппаФинансовогоУчета)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|					НастройкиИсключений КАК НастройкиИсключений
	|				ГДЕ
	|					Товары.Склад = НастройкиИсключений.Склад
	|					И Товары.ГруппаФинансовогоУчета = НастройкиИсключений.ГруппаФинансовогоУчета
	|					И Товары.Организация = НастройкиИсключений.Организация
	|					И НастройкиИсключений.СчетУчетаНаСкладе В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Товары)))
	|			ИЛИ НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	...
	|					НастройкиПоУмолчанию КАК НастройкиПоУмолчанию
	|				ГДЕ
	|					Товары.ГруппаФинансовогоУчета = НастройкиПоУмолчанию.ГруппаФинансовогоУчета)
	|	) КАК ИсходныеДанные
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Организации КАК Организации
	|	ПО
	...
	Если ТипЗнч(Организация) = Тип("Массив") И МенеджерВременныхТаблиц <> Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "
			...
			|;
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ НастройкиИсключений
			|;
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ НастройкиПоУмолчанию
			|;
			|////////////////////////////////////////////////////////////////////////////////
