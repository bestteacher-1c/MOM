...
//
Процедура ПрименитьСохраненныйВыбор(Форма) Экспорт
	...
	КонецЕсли;
	Пакет = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(Форма.ЗагрузкаДанныхТСД, Ложь);
	...
	Пока ДоступноеКоличествоПоСохраненномуВыбору <> 0 И ЗаполняемыйКодМаркировки <> ВсегоКодовМаркировки Цикл
		...
		Если СтрокаШтрихкод.РезультатОбработки.ТребуетсяУточнениеДанных
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаШтрихкод.РезультатОбработки.ДанныеШтрихкода.ШтрихкодУпаковки) Тогда
	...
	Если ОбновляемыеШтрихкодыУпаковок.Количество() Тогда 
		Если Форма.ИмяФормы = "Обработка.ПроверкаИПодборПродукцииИСМП.Форма.ПроверкаИПодбор"
			Или Форма.ИмяФормы = "Обработка.ПроверкаИПодборТабачнойПродукцииМОТП.Форма.ПроверкаИПодбор" Тогда
			ШтрихкодированиеИСВызовСервера.ОбработатьГрупповоеУточнениеДанныхДляФормыПроверкиИПодбора(СохраненныйВыбор, ОбновляемыеШтрихкодыУпаковок);
		КонецЕсли;
	КонецЕсли;
	
	Форма.ЗагрузкаДанныхТСД.ШтрихкодыТСД = Новый ФиксированныйМассив(Пакет.ШтрихкодыТСД);
