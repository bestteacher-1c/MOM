...
Процедура ПрочитатьРеквизитыИсправления(Форма, РежимИсправления = "РасчетЗарплаты", ПолеПериодРегистрации = "ПериодРегистрации", ПроверятьНаличиеНачислений = Истина) Экспорт
	...
	Форма.ПараметрыИсправленногоДокумента = ПараметрыИсправляемогоДокумента();
	Форма.ПараметрыИсправленногоДокумента.Ссылка = ИсправленныйДокумент;
	Если ВозможностиИсправления.Исправление И ЭтоРасчетныйДокумент И ИсправленныйДокумент <> Неопределено Тогда
		ЗаполнитьПараметрыИсправляемогоДокумента(Форма.ПараметрыИсправленногоДокумента,	ИсправленныйДокумент, ПолеПериодРегистрации);
	КонецЕсли;
...
Функция ПриПроведенииИсправления(ДокументСсылка, Движения, РежимПроведения, Отказ, РеквизитыДляПроведения = Неопределено, СтруктураВидовУчета = Неопределено, Объект = Неопределено, ИмяПериодРегистрации = "ПериодРегистрации") Экспорт
	...
	ПараметрыИсправляемогоДокумента = ПараметрыИсправляемогоДокумента();
	
	Если ИсправлениеВТекущемПериодеВозможно Тогда
		
		УстановитьПривилегированныйРежим(Истина);
		Если ОтменаДокумента Тогда
			ПараметрыИсправляемогоДокумента.ПериодРегистрации = РеквизитыДляПроведения.ПериодРегистрацииСторнируемого;
			ПараметрыИсправляемогоДокумента.ВыполнилДоначисление = ЕстьПерерасчеты(ИсправленныйДокумент)
				Или НайтиИсправленныйВПрошломПериоде(ИсправленныйДокумент, ИмяПериодРегистрации) <> Неопределено;
		Иначе
			...
			КонецЕсли;
			ЗаполнитьПараметрыИсправляемогоДокумента(ПараметрыИсправляемогоДокумента, ИсправленныйДокумент, ИмяПериодРегистрацииИсправленного);
		КонецЕсли;
...
Функция НайтиИсправленныйВПрошломПериоде(Документ, ИмяПоляПериодРегистрации = "ПериодРегистрации")
	
	ПериодРегистрацииИсправляющего = Неопределено;
	
	ИменаРеквизитов = Новый Структура;
	ИменаРеквизитов.Вставить("ИсправленныйДокумент");
	ИменаРеквизитов.Вставить("ПериодРегистрации", ИмяПоляПериодРегистрации);
	...
	Пока Истина Цикл
		...
		Значения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийДокумент, ИменаРеквизитов);
		
		Если ПериодРегистрацииИсправляющего <> Неопределено Тогда
			Если Значения.ПериодРегистрации <> Неопределено И Значения.ПериодРегистрации < ПериодРегистрацииИсправляющего Тогда
				Возврат ТекущийДокумент;
			КонецЕсли;
		КонецЕсли;
			
		Если Не ЗначениеЗаполнено(Значения.ИсправленныйДокумент) Тогда
			Прервать;
		КонецЕсли;
		
		ПериодРегистрацииИсправляющего = Значения.ПериодРегистрации;
		ТекущийДокумент = Значения.ИсправленныйДокумент;
	КонецЦикла;
...
Функция ПараметрыИсправляемогоДокумента()
...
Процедура ЗаполнитьПараметрыИсправляемогоДокумента(Параметры, ИсправляемыйДокумент, ПолеПериодРегистрации, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если Параметры = Неопределено Тогда
		Параметры = ПараметрыИсправляемогоДокумента();
	КонецЕсли;
	
	ПолучаемыеРеквизиты = Новый Структура;
	ПолучаемыеРеквизиты.Вставить("Организация");
	ПолучаемыеРеквизиты.Вставить("ПериодРегистрации", ПолеПериодРегистрации);
	
	Если ДополнительныеПараметры <> Неопределено Тогда
		Для Каждого Параметр Из ДополнительныеПараметры Цикл
			ПолучаемыеРеквизиты.Вставить(Параметр.Ключ, Параметр.Значение);
			Параметры.Вставить(Параметр.Ключ);
		КонецЦикла;
	КонецЕсли;
	
	Значения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИсправляемыйДокумент, ПолучаемыеРеквизиты);
	ЗаполнитьЗначенияСвойств(Параметры, Значения);
	Параметры.Ссылка = ИсправляемыйДокумент;
	Параметры.ВыполнилДоначисление = ЕстьПерерасчеты(ИсправляемыйДокумент)
		Или НайтиИсправленныйВПрошломПериоде(ИсправляемыйДокумент, ПолеПериодРегистрации) <> Неопределено;
