...
Процедура ОткрытьФормуПроверкиИПодбора(Форма, ВидМаркируемойПродукции) Экспорт
	...
	Если НЕ ПустаяСтрока(ПараметрыОткрытияФормыПроверки.ИмяРеквизитаФормыОбъект) Тогда
		...
		Если ПроверкаИПодборПродукцииИСМПКлиентСервер.ЭтоДокументПриобретения(Форма)
			...
			ДополнительнеПараметрыФормы = СобытияФормИСМПКлиентСервер.ДополнительныеПараметрыИнтеграции(Форма);
			
			Если ДополнительнеПараметрыФормы <> Неопределено Тогда
				ДополнительнеПараметрыФормы.Вставить("ПроверкаЗаполненияОтключитьКонтрольСерий", Истина);
			КонецЕсли;
...
Процедура ОткрытьФормуПроверкиИПодбораМаркируемойПродукции(Форма, Знач ПараметрыОткрытияФормыПроверки) Экспорт
	...
	Если Отказ Тогда
		Возврат;
	Иначе
		Если ПараметрыОткрытияФормыПроверки.ВидМаркируемойПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табак") Тогда
