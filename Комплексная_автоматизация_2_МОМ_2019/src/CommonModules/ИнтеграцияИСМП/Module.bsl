			...
			Если Выборка.ТипСообщения = Перечисления.ТипыЗапросовИС.Исходящий Тогда
				...
				ИначеЕсли Выборка.Передан Тогда
					...
					Иначе
						...
						ПараметрыОбновленияСтатуса.ОперацияКвитанции = Операция;
...
Функция МассивЗначенийGTINДляВыбора(ТекущиеДанные, Объект, ВключатьGTINНаОстатки = Ложь) Экспорт
	...
	Если ВключатьGTINНаОстатки Тогда
		...
		КодТНВЭД = ИнтеграцияИСМПСлужебный.КодТНВЭДДляПередачиВИСМП(ТекущиеДанные.КодТНВЭД);
		...
		ПараметрыОписанияОстатков = ИнтеграцияИСМПСлужебный.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(
		...
		|	ПО ЗаказНаЭмиссиюКодовМаркировкиСУЗШапка.Ссылка = ЗаказНаЭмиссиюКодовМаркировкиСУЗШапка.Ссылка
		|ГДЕ
...
// Возвращает признак доступности маркировки остатков, включая проверку тестового перида.
// 
// Параметры:
//  ВидМаркируемойПродукции - ПеречислениеСсылка.ВидыПродукцииИС - вид маркируемой продукции
// Возвращаемое значение:
//  Булево - Истина, если в системе установлен признак ведения учета по переданному виду маркируемой продукции,
//  текущая дата менее даты обязательной регистрации,
//  по виду продукции доступна маркировка остатков.
//
Функция ДоступностьМаркировкиОстактов(ВидМаркируемойПродукции) Экспорт
	
	Если Не ИнтеграцияИСМПКлиентСервер.ВидПродукцииПодлежитМаркировкеОстатков(ВидМаркируемойПродукции) Тогда
		Возврат Ложь;
	КонецЕсли;
