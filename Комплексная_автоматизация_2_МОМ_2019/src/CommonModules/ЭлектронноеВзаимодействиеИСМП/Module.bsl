...
Функция ДанныеДокументаСоответствуютДаннымУпаковок(ПроверяемыйДокумент) Экспорт
	...
	|ИЗ
	|	ДанныеДокумента КАК ДанныеДокумента
	|		ПОЛНОЕ СОЕДИНЕНИЕ ДанныеДерева КАК ДанныеДерева
	|		ПО ДанныеДерева.Номенклатура = ДанныеДокумента.Номенклатура
	|		И ДанныеДерева.Характеристика = ДанныеДокумента.Характеристика
...
Функция ЧастичноеСодержимое(ТаблицаУпаковок) Экспорт
	...
	Пока МультитоварныеУпаковки.Количество() Цикл
		...
		Для Каждого ДанныеШтрихКода Из ДочерниеЗаписи Цикл
			Если ЗначениеЗаполнено(ДанныеШтрихКода.Номенклатура) Тогда
				...
				СтруктураПоиска.Штрихкод = СоответствиеУпаковок.Получить(СтрокаВложение.Штрихкод);
				СтрокаЗаполнения = Результат.НайтиСтроки(СтруктураПоиска)[0];
	...
	ПараметрыНормализацииТабакМаркируемыйТовар = ШтрихкодированиеИССлужебный.ПараметрыНормализацииКодаМаркировки();
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		...
		Если СтрокаТаблицы.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак Тогда
...
//   ВидыПродукцииИС   - (См. ИнтеграцияИСУТ.ШтрихкодыСодержащиеВидыПродукции).
//
// Возвращаемое значение:
...
Функция МетаданныеОбработкиОбменаПоЭДО() Экспорт
	...
	Возврат Метаданные.Обработки[ИмяОбработки];
