...
Функция ОСВ_ПоЛизингу(ПараметрыОСВ)
	...
	|						ДоговорыЛизинга.Ссылка
	|					ИЗ
	|						ДоговорыЛизинга КАК ДоговорыЛизинга)) КАК Ставки
...
Функция ТекстЗапросаВТ_ДокументыПоСчету()
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПоступлениеБезналичныхДенежныхСредств.БанковскийСчетКонтрагента, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
	|		ТОГДА ПоступлениеБезналичныхДенежныхСредств.БанковскийСчетКонтрагента
	|		КОГДА ЕСТЬNULL(ПоступлениеБезналичныхДенежныхСредств.БанковскийСчетОтправитель, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|		ТОГДА ПоступлениеБезналичныхДенежныхСредств.БанковскийСчетОтправитель
	|		КОГДА ЕСТЬNULL(СписаниеБезналичныхДенежныхСредств.БанковскийСчетКонтрагента, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
	|		ТОГДА СписаниеБезналичныхДенежныхСредств.БанковскийСчетКонтрагента
	|		КОГДА ЕСТЬNULL(СписаниеБезналичныхДенежныхСредств.БанковскийСчетПолучатель, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
...
Процедура ДобавитьСчетаЗаполненияОСВПоСчету2017Кв3(ТаблицаСчетов) Экспорт
	...
	ДобавитьСчетВТаблицу(ТаблицаСчетов, "ОсновноеПроизводство"); // 20
...
Процедура УстановитьПараметрыСчета_01(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ОсновныеСредства");
...
Процедура УстановитьПараметрыСчета_04(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "НематериальныеАктивы");
...
Процедура УстановитьПараметрыСчета_07(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Номенклатура");
	
КонецПроцедуры

Процедура УстановитьПараметрыСчета_08(ПараметрыСчета)
	
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ОбъектыСтроительства");
	
КонецПроцедуры

Процедура УстановитьПараметрыСчета_09(ПараметрыСчета)
	
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ВидыАктивовИОбязательств");
...
Процедура УстановитьПараметрыСчета_20(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "СтатьиЗатрат,ГруппыПродукции");
...
Процедура УстановитьПараметрыСчета_25(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "СтатьиЗатрат");
...
Процедура УстановитьПараметрыСчета_40(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ВидыСтоимости");
...
Процедура УстановитьПараметрыСчета_50(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "СтатьиДвиженияДенежныхСредств");
	
КонецПроцедуры

Процедура УстановитьПараметрыСчета_51(ПараметрыСчета)
	
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "БанковскиеСчета,СтатьиДвиженияДенежныхСредств");
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "БанковскиеСчета");
...
Процедура УстановитьПараметрыСчета_58(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Контрагенты,Договоры");
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Контрагенты,ЦенныеБумаги");
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Контрагенты");
...
Процедура УстановитьПараметрыСчета_68(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Контрагенты,Договоры");
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ВидыПлатежейВГосБюджет");
...
Процедура УстановитьПараметрыСчета_79(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Контрагенты,Договоры");
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ОбособленныеПодразделения");
...
Процедура УстановитьПараметрыСчета_86(ПараметрыСчета)
	...
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ЦелевыеСредства,Договоры");
	
КонецПроцедуры

Процедура УстановитьПараметрыСчета_90(ПараметрыСчета)
	
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "НоменклатурныеГруппы");
	
КонецПроцедуры

Процедура УстановитьПараметрыСчета_91(ПараметрыСчета)
	
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "ПрочиеДоходыИРасходы");
	
КонецПроцедуры

Процедура УстановитьПараметрыСчета_001(ПараметрыСчета)
	
	ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, "Контрагенты,ОсновныеСредства");
...
Процедура ДобавитьВидыСубконтоПоСчету(ПараметрыСчета, СписокВидовСубконто)
