...
Процедура ПодобраннаяПродукцияПриДобавленииСтроки(Форма, СтрокаИзменений, Знач ИзменениеКоличества) Экспорт
	...
	Если СтрокаАгрегатор1 = Неопределено И СтрокаАгрегатор2 = Неопределено Тогда
		...
		ОбработатьИзменениеНоменклатурыВСтроке(Форма, НоваяСтрока);
...
Процедура ПодобраннаяПродукцияПриУдаленииСтроки(Форма, СтрокаИзменений, Знач ИзменениеКоличества) Экспорт
	...
	Если Не ВыполнитьПерераспределениеКоличества Тогда
		...
		Если ЭтоШтрихкодВводаОстатков И СтрокаСКоличествомПоДокументу <> Неопределено Тогда
			
			ТекущееКоличество = Мин(СтрокаАгрегатор.Количество, ИзменениеКоличества);
			
			СтрокаАгрегаторGTIN = Неопределено;
			Для Каждого СтрокаМаркируемойПродукции Из ПодобраннаяМаркируемаяПродукция Цикл
				Если Не ЗначениеЗаполнено(СтрокаМаркируемойПродукции.Номенклатура)
					И GTINРавны(СтрокаАгрегатор.GTIN, СтрокаМаркируемойПродукции.GTIN) Тогда
					СтрокаАгрегаторGTIN    = СтрокаМаркируемойПродукции;
					УдалитьСтрокуАгрегатор = (СтрокаАгрегатор.КоличествоПодобрано = 0 И СтрокаАгрегатор <> СтрокаАгрегаторGTIN);
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если СтрокаАгрегаторGTIN = Неопределено Тогда
				СтрокаАгрегаторGTIN = ПодобраннаяМаркируемаяПродукция.Вставить(0);
				СтрокаАгрегаторGTIN.GTIN = СтрокаСКоличествомПоДокументу.GTIN;
			КонецЕсли;
			
			СтрокаАгрегаторGTIN.Количество = Мин(СтрокаСКоличествомПоДокументу.Количество, СтрокаАгрегаторGTIN.Количество + ТекущееКоличество);
			
			ПроверкаИПодборПродукцииИСМПКлиентСервер.УстановитьИндексКартинкиСостояниеПодбораКодаМаркировки(СтрокаАгрегаторGTIN);
			
			СтрокаАгрегатор.Количество = СтрокаАгрегатор.Количество - ТекущееКоличество;
...
Функция СтрокаАгрегаторМаркируемойПродукции(ПодобраннаяМаркируемаяПродукция, СтрокаПодобраннойПродукции, СвойстваВидаПродукции, ПолноеСовпадение = Ложь)
	...
	Для Каждого СтрокаМаркируемойПродукции Из ПодобраннаяМаркируемаяПродукция Цикл
		Если Не НоменклатурыСтрокСовпадают(СтрокаПодобраннойПродукции, СтрокаМаркируемойПродукции) Тогда
			Продолжить;
		КонецЕсли;
		Если GTINРавны(СтрокаПодобраннойПродукции.GTIN, СтрокаМаркируемойПродукции.GTIN)
			И СвойстваВидаПродукцииСовпадают(СтрокаПодобраннойПродукции, СтрокаМаркируемойПродукции, СвойстваВидаПродукции) Тогда
		...
		Если СвойстваВидаПродукции.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукция")
			...
			И Не СвойстваВидаПродукцииСовпадают(СтрокаПодобраннойПродукции, СтрокаМаркируемойПродукции, СвойстваВидаПродукции) Тогда
			Продолжить;
		КонецЕсли;
		СтрокиДляАнализаИРаспределения.Добавить(СтрокаМаркируемойПродукции);
	КонецЦикла;
...
Процедура ОбработатьИзменениеНоменклатурыВСтроке(Форма, НоваяСтрока)
