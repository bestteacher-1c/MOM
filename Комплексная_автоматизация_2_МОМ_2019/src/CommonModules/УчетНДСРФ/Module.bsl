...
Процедура Использование_ПеречислениеНДСВБюджет(ПараметрыОбработчика) Экспорт
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Требуется оплата НДС в бюджет по счетам-фактурам налогового агента (%1 шт.)'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВернутьДокументыКОтражениюВУчете(ПараметрыРасчета) Экспорт
