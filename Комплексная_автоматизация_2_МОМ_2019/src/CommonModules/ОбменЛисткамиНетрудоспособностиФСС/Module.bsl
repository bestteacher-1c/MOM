...
Функция СостояниеЭЛН(LN_STATE, REASON1)
	...
	Иначе
		Состояние.Имя = LN_State;
		Состояние.Представление = СтрШаблон(НСтр("ru = 'Неизвестное состояние (LN_State=""%1"")'"), LN_State);
		Состояние.ЗагрузкаРазрешена = Ложь;
...
Процедура ПроверитьВозможностьЗагрузкиЭЛН(Документ, ДанныеИзСервиса, Отказ) Экспорт
	// Проверка состояния.
	Состояние = СостояниеЭЛН(ДанныеИзСервиса.LN_STATE, ДанныеИзСервиса.REASON1);
