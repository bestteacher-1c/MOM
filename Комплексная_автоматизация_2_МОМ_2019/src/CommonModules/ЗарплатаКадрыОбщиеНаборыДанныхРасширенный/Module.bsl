...
Функция ПолучитьТекстОбщегоЗапроса(ИсточникДанных, ТолькоРазрешенные) Экспорт
	...
	ИначеЕсли ВРег(ИсточникДанных) = ВРег("НачисленияУдержанияАвансом") Тогда
		...
		ТекстЗапроса = 
			...
			|			ТОГДА НачисленияИУдержания.ВидРасчета.РеквизитДопУпорядочивания * 100000000000000000000 + ВЫБОР
			...
			|								КОНЕЦ, ДЕНЬ) * 10000000000000000 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(2000, 1, 1), ВЫБОР
			...
			|								КОНЕЦ, ДЕНЬ) * 1000000000000
			|					ИНАЧЕ 0
			|				КОНЕЦ + ВЫБОР
			|					КОГДА &ВыводитьПоказателиНачисленийИУдержаний
			|							ИЛИ &ВыводитьВсеПоказателиНачисленийИУдержаний
			|						ТОГДА РАЗНОСТЬДАТ(ДАТАВРЕМЯ(2000, 1, 1), ВЫБОР
			|									КОГДА ЕСТЬNULL(НачисленияИУдержания.Регистратор.Дата, ДАТАВРЕМЯ(1, 1, 1)) < ДАТАВРЕМЯ(2000, 1, 1)
			|										ТОГДА ДАТАВРЕМЯ(2000, 1, 1)
			|									ИНАЧЕ НачисленияИУдержания.Регистратор.Дата
			|								КОНЕЦ, СЕКУНДА) * 1000 + ЕСТЬNULL(НачисленияИУдержания.ИдентификаторСтроки, 0)
			|					ИНАЧЕ 0
			|				КОНЕЦ
	...
	Для каждого ТекстПодзапроса Из МассивПодзапросов Цикл
		...
		Если СтрНайти(ВРег(ТекстПодзапроса), ИскомаяВТ) > 0 Тогда
			...
			ТекстЗапросаПоказателейНачислений =
				...
				|	ВЫБОР
				|		КОГДА НЕ ВидыРасчетаБезОсновныхПоказателей.ВидРасчета ЕСТЬ NULL
				|				И НачисленияПоказатели.НомерСтроки = 1
				|			ТОГДА ИСТИНА
				|		ИНАЧЕ НачисленияПоказатели.ОсновнойПоказатель
				...
				|	ВЫБОР
				|		КОГДА НЕ ВидыРасчетаБезОсновныхПоказателей.ВидРасчета ЕСТЬ NULL
				|				И УдержанияПоказатели.НомерСтроки = 1
				|			ТОГДА ИСТИНА
				|		ИНАЧЕ УдержанияПоказатели.ОсновнойПоказатель
				|	КОНЕЦ,
				...
				|	ПоказателиНачислений.ПервыйПоказатель КАК ПервыйПоказатель,
				...
				|	ПоказателиНачислений.ПервыйПоказатель,
