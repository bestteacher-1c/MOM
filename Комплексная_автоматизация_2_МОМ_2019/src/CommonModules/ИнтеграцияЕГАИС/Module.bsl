		...
		Если ВидУпаковкиИС(СтрокаТаблицы.ШтриховойКод, КешДанныхРазбора) = Перечисления.ВидыУпаковокИС.Логистическая Тогда
			...
			Иначе
				...
				Если Детализация = Неопределено
					Или Детализация = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
					Продолжить;
...
Процедура ПолучитьДанныеПоШтрихкодамТСД(РезультатПроверки, ПараметрыСканирования, ПараметрыПроверки, КешДанныхРазбора)
	...
	РезультатыОбработки = ШтрихкодированиеИС.ОбработатьШтрихкоды(
		ДанныеШтрихкодов, ПараметрыСканированияДляГрупповойОбработки, Неопределено,
		ИдентификаторРезультатаОбработки, КешДанныхРазбора);
	...
	Для Каждого ЭлементМассива Из РезультатПроверки.МаркируемаяПродукция Цикл
		...
		РезультатОбработки = РезультатыОбработки[Штрихкод];
