...
Функция УчитываемыеВидыМаркируемойПродукции(НаДату = Неопределено, ТестовыйПериод = Ложь) Экспорт
	...
	ДоступныеВидыПродукцииИС = ИнтеграцияИСКлиентСервер.ВидыПродукцииИСМП(Истина, Истина);
	Для Каждого ДоступныйВидПродукции Из ДоступныеВидыПродукцииИС Цикл
		Если ВидыПродукции.Найти(ДоступныйВидПродукции) <> Неопределено Тогда
			УчитываемыеВидыМаркируемойПродукции.Добавить(ДоступныйВидПродукции);
		КонецЕсли;
	КонецЦикла;
	
	Возврат Новый ФиксированныйМассив(УчитываемыеВидыМаркируемойПродукции);
...
// Возвращает способы ввода в оборот, по которым доступно заполнение документа ИСМП
//
// Параметры:
//   Ссылка - Документ.ПеремаркировкаТоваровИСМП - документ по которому происходит обработка заполнения
//
// Возвращаемое значение:
//   Массив Из ПеречислениеСсылка.СпособыВводаВОборотСУЗ - различные способы ввода в оборот в документе ИСМП, заполненном 
//     по основанию без фильтра по способу ввода.
Функция СпособыВводаВОборотДанныхЗаполнения(Ссылка) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПеремаркировкаТоваровИСМП.СпособВводаВОборот КАК СпособВводаВОборот
	|ИЗ
	|	Документ.ПеремаркировкаТоваровИСМП.Товары КАК ПеремаркировкаТоваровИСМП
	|ГДЕ
	|ПеремаркировкаТоваровИСМП.Ссылка = &ссылка";
	
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("СпособВводаВОборот");
