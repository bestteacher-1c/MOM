...
Процедура ПроверитьЗаполнениеШтрихкодовУпаковок(Объект, Отказ) Экспорт
	...
	РезультатПроверки = ШтрихкодированиеИС.ТаблицаТоварыСоответствуетСпискуУпаковокМаркируемойПродукции(Объект.Товары, СписокУпаковок, ПараметрыСканирования); 
...
Функция ЗаполнитьGTINМаркировкиОстатков(ТаблицаНоменклатуры, Организация, ВидПродукции, ТекстОшибки = Неопределено) Экспорт
	...
	Пока СоответствиеСтрокПоиска.Количество() И ПовторятьИтерацию Цикл
		...
		РезультатЗапроса = ИнтерфейсИСМП.НайтиПродукциюПоИННПроизводителя(ИНН, ВидПродукции, НомерСтраницы, Организация);
			...
			Для Каждого КлючИЗначение Из РезультатЗапроса.ДанныеПродукцииПоШтрихкодуEAN Цикл
				...
				Если КлючИЗначение.Значение <> Неопределено
					...
					Для каждого СтрокаТовары Из КлючИЗначениеИсточник.Значение Цикл
						...
						Если КешОписанияОстатков.Получить(GTIN) = Неопределено Тогда
...
Функция ОбязательныеПоляОписанияОстатковПоСтрокеДокумента(СтрокаТовары, ВидПродукции) Экспорт
	...
	ЗначениеТНВЭД  = ИнтеграцияИСМПСлужебный.КодТНВЭДДляПередачиВИСМП(СтрокаТовары.КодТНВЭД);
	...
	Для Каждого КлючИЗначение Из ТребуетсяЗапроситьУстановитьКлючСессии Цикл
		...
		Для Каждого ЭлементДанных Из ДанныеДляУстановкиКлючаСессии Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(РезультатЗапросаИУстановкиКлючаСессии.ТекстОшибки) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(РезультатЗапросаИУстановкиКлючаСессии.ТекстОшибки);	
			КонецЕсли;
...
Функция ПараметрыСеансаИС() Экспорт
	...
	ЕстьМОТП = УчитываемыеВидыМаркируемойПродукции.Найти(Перечисления.ВидыПродукцииИС.Табак)<>Неопределено;
	ЕстьИСМП = Не(УчитываемыеВидыМаркируемойПродукции.Количество() = ?(ЕстьМОТП,1,0));
...
Функция ПодготовитьКПередачеСообщенияДляРазагрегацииКодовМаркировкиИСМП(МаркированныеТовары, ОбщийЭлементОчереди, Знач ЗагружатьДо, СообщенияJSON, ПараметрыОбмена) Экспорт
	...
	Для Каждого СтрокаМаркированногоТовара из МаркированныеТовары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаМаркированногоТовара.РодительскаяУпаковка) Тогда
			...
			РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(СтрокаМаркированногоТовара.РодительскаяУпаковка, ОбщийЭлементОчереди.ВидПродукции);
...
Функция СтрокаДляПоискаGTINМаркировкиОстатков(ВидПродукции, СтрокаТовары)
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность Тогда
		ТребуетсяВозрастнаяКатегория = КодТНВЭДТребуетВозрастнуюКатегорию(
			ИнтеграцияИСМПСлужебный.КодТНВЭДДляПередачиВИСМП(СтрокаТовары.КодТНВЭД));
...
Функция ОписаниеТоварногоЗнака(ТоварныйЗнак)
	Если ЗначениеЗаполнено(ТоварныйЗнак) Тогда
		Возврат ТоварныйЗнак;
	Иначе
		Возврат "Без товарного знака";
	КонецЕсли;
КонецФункции
...
Функция ЗапроситьКодыМаркировкиИзБизнесЗаказа(ПараметрыЗапроса, ДанныеПула, ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	Если РезультатЗапросаКодовМаркировки.ДанныеБлока <> Неопределено Тогда
		...
		Для Каждого ЗначениеШтрихкода Из РезультатЗапросаКодовМаркировки.ДанныеБлока.КодыМаркировки Цикл
			...
			Если СтрокаЗаказаНаЭмиссию.Количество >= СтрокаЗаказаНаЭмиссию.КоличествоПолучено Тогда
				СтрокаЗаказаНаЭмиссию = Неопределено;
				Для Каждого СтрокаЗазакаПоиск Из СтрокиЗаказа Цикл
					Если СтрокаЗазакаПоиск.Количество > СтрокаЗазакаПоиск.КоличествоПолучено Тогда
						СтрокаЗаказаНаЭмиссию = СтрокаЗазакаПоиск;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			...
			РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
				ЗначениеШтрихкода, СтрокаЗаказаНаЭмиссию.ВидПродукции);
			...
			ЗаписьНабора.КодМаркировки = ШтрихкодированиеИССлужебный.НормализоватьКодМаркировки(
				РезультатРазбора, СтрокаЗаказаНаЭмиссию.ВидПродукции, ПараметрыНормализации);
			...
			ИначеЕсли СтрокаЗаказаНаЭмиссию.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция
			    И РезультатРазбора.СоставКодаМаркировки.Свойство("ГоденДо") Тогда
				ЗаписьНабора.СрокГодности = РезультатРазбора.СоставКодаМаркировки.ГоденДо;
			КонецЕсли;
...
Функция ЗагрузитьДокументы(ЭлементОчереди, ПараметрыОбмена, ПараметрыЗагрузки)
	...
	Для Каждого ИдентификаторДокумента Из ДокументыДляЗагрузки Цикл
		...
		Если СоставДокумента.Количество() > 0 Тогда
			...
			ДополнитьТабличныеЧастиПоДаннымИСМП(ДокументОбъект, ДанныеШтрихкодов);
...
Процедура ДополнитьТабличныеЧастиПоДаннымИСМП(ДокументПриемка, ДанныеШтрихкодов)
