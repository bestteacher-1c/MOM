				...
				Если ФормироватьЗаписиТолькоДляИзменений
					...
					Если ЗначениеЗаполнено(СтрокаПлановыхНачислений.ДействуетДо) Тогда
						...
						Если ЗначениеЗаполнено(СтрокаПредыдущихЗначений.ПериодВозвратногоСобытия) Тогда
							...
							Если Не ПособиеПоУходуЗаРебенком
								И СтрокаПредыдущихЗначений.ПериодВозвратногоСобытия > НоваяСтрока.ДействуетДо + 2 Тогда
...
Процедура ПодготовитьДанныеДляПерезаполнения(СтруктураПараметров, АдресХранилища) Экспорт
	...
	РассчитыватьНачисления = ЕстьНачисления Или ЕстьНачисленияПерерасчет Или ЕстьПособия Или ЕстьПособияПерерасчет Или ЕстьЛьготы;
	РассчитыватьУдержания = ОписаниеДокумента.УдержанияИмя <> Неопределено Или ЕстьУдержанияПерерасчет;
	РассчитыватьНДФЛ = ОписаниеДокумента.НДФЛИмя <> Неопределено;
...
// Описание - вовращает пустую таблицу для базы распределения ранее начисленных сумм 
// 
// Возвращаемое значение:
...
//   * ПериодДействия - Дата
//   * ПериодРегистрации - Дата
//   * ФизическоеЛицо - СправочникСсылка.ФизичческиеЛица
//   * ДокументОснование - ОпределяемыйТип.ОснованиеНачисления
//
// Возвращаемое значение:
...
Функция РаспределениеРанееНачисленныхСумм(РаспределяемыеНачисления) Экспорт
	...
	УстанавлиаемыеНачисления = РаспределяемыеНачисления.Скопировать();
	Если  УстанавлиаемыеНачисления.Колонки.Найти("РассчитыватьПоРазовымНачислениямДокумента") = Неопределено Тогда
		УстанавлиаемыеНачисления.Колонки.Добавить(
			"РассчитыватьПоРазовымНачислениямДокумента",
			 Новый ОписаниеТипов("Булево"));
	 КонецЕсли;
	
	МененджерРасчета = СоздатьМенеджерРасчета(
		РаспределяемыеНачисления[0].МесяцНачисления, 
		РаспределяемыеНачисления[0].Организация);
		
	МененджерРасчета.УстановитьТаблицуНачисления(УстанавлиаемыеНачисления);
	МененджерРасчета.ИнициализироватьРасчетРанееНачисленныхСумм();
	
	Возврат МененджерРасчета.КоэффициентыРанееНачисленныхСумм();	
КонецФункции	
...
Процедура ДобавитьЧастьЗапросаПроверкиПерерасчетовУдержанийПоПоказателям(МенеджерВременныхТаблиц, ЧастиЗапроса) Экспорт
	
	СоздатьВТУдержанияФизическихЛицПоПоказателямПерерасчетов(МенеджерВременныхТаблиц);
...
Процедура ДобавитьЧастьЗапросаПроверкиПерерасчетовУдержанийПоИсполнительнымДокументам(МенеджерВременныхТаблиц, ЧастиЗапроса) Экспорт
	
	СоздатьВТУдержанияФизическихЛицПоИсполнительнымДокументам(МенеджерВременныхТаблиц);
...
Процедура СоздатьВТУдержанияФизическихЛицПоПоказателямПерерасчетов(МенеджерВременныхТаблиц)
...
Процедура СоздатьВТУдержанияФизическихЛицПоИсполнительнымДокументам(МенеджерВременныхТаблиц)
...
Функция ПоказателиСуммированногоУчетаСотрудниковЗаПериод(МассивСотрудников, ПериодСуммированногоУчетаНачало, ПериодСуммированногоУчетаОкончание) Экспорт
	...
	ПериодОплатыНерабочихДней = УчетРабочегоВремениРасширенный.ПериодОплатыНерабочихДней2020();
	ДатыНерабочихПредпраздничныхДней = УчетРабочегоВремениРасширенный.ДатыНерабочихПредпраздничныхДней2020();
	Запрос.УстановитьПараметр("ДатаНачалаОплатыНерабочих", ПериодОплатыНерабочихДней.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончанияОплатыНерабочих", ПериодОплатыНерабочихДней.ДатаОкончания);
	Запрос.УстановитьПараметр("ДатыНерабочихПредпраздничныхДней", ДатыНерабочихПредпраздничныхДней);
...
Процедура ДополнитьЗапросВТПоказателейПоРегиструРасчета(Запрос)
	...
	|				ИЛИ ВТДни.Дата В (&ДатыНерабочихПредпраздничныхДней)
	|				ТОГДА 7
	|			КОГДА ВТДни.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Нерабочий)
	|				И ВТДни.Дата МЕЖДУ &ДатаНачалаОплатыНерабочих И &ДатаОкончанияОплатыНерабочих
	|				ТОГДА 8
	|			ИНАЧЕ 0
	|		КОНЕЦ * ВЫБОР
