...
Процедура ПриЗаписиБольничного(БольничныйОбъект) Экспорт
	Попытка
		...
		Для Каждого Период65 Из Периоды65 Цикл
			
			УстановитьПривилегированныйРежим(Истина);
			
			Если БольничныйОбъект.НомерЛисткаНетрудоспособности = Период65.НомерЛН Тогда
			...
			ИначеЕсли Не БольничныйОбъект.ПометкаУдаления
				...
				СтрокаТаблицы = НайтиЗаявлениеИРеестр(БольничныйОбъект, Период65.ДатаНачалаЛН, Период65.ДатаОкончанияЛН);
				...
				Если СтрокаТаблицы.РеестрОсвобождениеДатаНачала1 = Период65.ДатаНачалаЛН
					И СтрокаТаблицы.РеестрОсвобождениеДатаОкончания1 = Период65.ДатаОкончанияЛН Тогда
...
Функция Периоды65() Экспорт
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("НомерЛН", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("ДатаНачалаЛН", Новый ОписаниеТипов("Дата"));
	Результат.Колонки.Добавить("ДатаОкончанияЛН", Новый ОписаниеТипов("Дата"));
	Результат.Колонки.Добавить("ИдентификаторКоманды", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("КомментарийРеестра", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("КомментарийЗаявления", Новый ОписаниеТипов("Строка"));
	
	Строка = Результат.Добавить();
	Строка.НомерЛН         = "999000000000";
	Строка.ДатаНачалаЛН    = '20200406';
	Строка.ДатаОкончанияЛН = '20200419';
	Строка.ИдентификаторКоманды = "Заполнить65_20200406";
	Строка.КомментарийРеестра   = НСтр("ru = 'Реестр заполнен автоматически в соответствии с постановлением Правительства РФ от 01.04.2020 № 402.'");
	Строка.КомментарийЗаявления = НСтр("ru = 'Заявление подготовлено автоматически в целях отправки реестра прямых выплат в соответствии с постановлением Правительства РФ от 01.04.2020 № 402.'");
	
	Строка = Результат.Добавить();
	Строка.НомерЛН         = "999010000000";
	Строка.ДатаНачалаЛН    = '20200420';
	Строка.ДатаОкончанияЛН = '20200430';
	Строка.ИдентификаторКоманды = "Заполнить65_20200420";
	Строка.КомментарийРеестра   = НСтр("ru = 'Реестр заполнен автоматически в соответствии с постановлением Правительства РФ от 16.04.2020 № 517.'");
	Строка.КомментарийЗаявления = НСтр("ru = 'Заявление подготовлено автоматически в целях отправки реестра прямых выплат в соответствии с постановлением Правительства РФ от 16.04.2020 № 517.'");
	
	Строка = Результат.Добавить();
	Строка.НомерЛН         = "999020000000";
	Строка.ДатаНачалаЛН    = '20200512';
	Строка.ДатаОкончанияЛН = '20200529';
	Строка.ИдентификаторКоманды = "Заполнить65_20200512";
	Строка.КомментарийРеестра   = НСтр("ru = 'Реестр заполнен автоматически в соответствии с постановлением Правительства РФ от 15.05.2020 № 683.'");
	Строка.КомментарийЗаявления = НСтр("ru = 'Заявление подготовлено автоматически в целях отправки реестра прямых выплат в соответствии с постановлением Правительства РФ от 15.05.2020 № 683.'");
...
Функция НайтиЗаявлениеИРеестр(БольничныйОбъект, ДатаНачала65, ДатаОкончания65)
	Если БольничныйОбъект.ДатаНачала < ДатаНачала65
		Или БольничныйОбъект.ДатаОкончания > ДатаОкончания65 Тогда
	...
	МаксимальнаяДатаРождения = НачалоДня(ДобавитьМесяц(ДатаНачала65, -65*12));
	Если ДатаРождения > МаксимальнаяДатаРождения Тогда
		// На дату начала оформления реестров 65+ сотруднику было 64 года или меньше.
		Возврат Неопределено;
	КонецЕсли;
	...
	|			И (СтрокиРеестров.ОсвобождениеДатаОкончания1 = &ОкончаниеПериода)
	...
	Запрос.УстановитьПараметр("НачалоПериода",    ДатаНачала65);
	Запрос.УстановитьПараметр("ОкончаниеПериода", ДатаОкончания65);
	...
	// Определение наилучшего соответствия.
	ФрагментКомментария1 = НСтр("ru = '{Автозаполнение}'");
	ФрагментКомментария2 = НСтр("ru = 'Правительства РФ от 01.04.2020 № 402'");
	ФрагментКомментария3 = НСтр("ru = 'Правительства РФ от 16.04.2020 № 517'");
	ФрагментКомментария4 = НСтр("ru = 'Правительства РФ от 15.05.2020 № 683'");
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		Если    СтрНайти(СтрокаТаблицы.ЗаявлениеКомментарий, ФрагментКомментария1) > 0
			Или СтрНайти(СтрокаТаблицы.ЗаявлениеКомментарий, ФрагментКомментария2) > 0
			Или СтрНайти(СтрокаТаблицы.ЗаявлениеКомментарий, ФрагментКомментария3) > 0
			Или СтрНайти(СтрокаТаблицы.ЗаявлениеКомментарий, ФрагментКомментария4) > 0 Тогда
			СтрокаТаблицы.Вес = СтрокаТаблицы.Вес + 50;
		КонецЕсли;
		Если    СтрНайти(СтрокаТаблицы.РеестрКомментарий, ФрагментКомментария1) > 0
			Или СтрНайти(СтрокаТаблицы.РеестрКомментарий, ФрагментКомментария2) > 0
			Или СтрНайти(СтрокаТаблицы.РеестрКомментарий, ФрагментКомментария3) > 0
			Или СтрНайти(СтрокаТаблицы.РеестрКомментарий, ФрагментКомментария4) > 0 Тогда
			СтрокаТаблицы.Вес = СтрокаТаблицы.Вес + 50;
		КонецЕсли;
