...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(37, НСтр("ru='Не правильный раздел учета для движений в цеховой кладовой.'"));
	РасшифровкаКодовОшибок.Вставить(38, НСтр("ru='Не правильный раздел учета для движений по складу, который не является цеховой кладовой.'"));
	//-- НЕ УТ
...
//-- НЕ УТ
	...
	Запрос.Текст = ТекстЗапросаСтатьиНаПроизводственныеЗатратыВРегистреПартииПрочихРасходов();
...
Функция ТекстЗапросаВключениеУчетаСебестоимостиПоНазначениям() Экспорт
	Возврат "
		...
		// Кроме документов "Распределение материалов и работ", используемых для производстве версии 2.2. Они делают движения по регистру "Материалы и работы в производстве".
		|		И ЕСТЬNULL(ВЫРАЗИТЬ(МатериалыИРаботыВПроизводстве.Регистратор КАК Документ.РаспределениеПроизводственныхЗатрат).НовоеПроизводство, ИСТИНА)
		//-- Устарело_Производство21
		//-- НЕ УТ
		|
		|
