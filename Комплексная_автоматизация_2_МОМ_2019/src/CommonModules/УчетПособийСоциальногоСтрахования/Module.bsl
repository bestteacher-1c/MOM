...
//
// Параметры:
//   ПричинаНетрудоспособности - ПеречислениеСсылка.ПричиныНетрудоспособности - Проверямая причина нетрудоспособности.
...
Процедура ЗаполнитьХешЭЛН(ПараметрыОбновления = Неопределено) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		РегистрыСведений.СведенияОбЭЛН.ЗаписатьХеш(
...
Функция ЗапросРеестраЭЛНПоПервичнымДокументам(ТаблицаОснований, СсылкаРеестра, ИмяВТ = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА БольничныйЛист.СреднийДневнойЗаработок = 0
	|				ИЛИ БольничныйЛист.СреднийДневнойЗаработок > БольничныйЛист.МинимальныйСреднедневнойЗаработок
	|			ТОГДА ЕСТЬNULL(ВТБазыДляРасчетаСреднегоЗаработка.БазаДляРасчетаСреднегоЗаработка, 0)
	|		ИНАЧЕ БольничныйЛист.МинимальныйСреднедневнойЗаработок * ЕСТЬNULL(ВТБазыДляРасчетаСреднегоЗаработка.УчитываемыхДнейВКалендарныхГодах, 0)
