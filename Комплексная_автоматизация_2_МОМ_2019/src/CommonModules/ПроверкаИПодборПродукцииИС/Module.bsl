...
Процедура ЗаполнитьКешШтрихкодовУпаковок(Форма, Настройки = Неопределено, Инициализация = Ложь) Экспорт
	...
	ЕстьИдентификаторПроисхождения = ДанныеШтрихкодовУпаковокГосИС.Колонки.Найти("ИдентификаторПроисхожденияВЕТИС")<>Неопределено;
	ШтрихкодыУпаковокБезИдентификатора = Неопределено;
	Если ЕстьИдентификаторПроисхождения Тогда
		СтрокиОставить = Новый Массив;
		Для Каждого СтрокаШтрихкоды Из ШтрихкодыУпаковокИсходнаяТаблица Цикл
			Если ЗначениеЗаполнено(СтрокаШтрихкоды.ИдентификаторПроисхожденияВЕТИС) Тогда
				СтрокиОставить.Добавить(СтрокаШтрихкоды);
			КонецЕсли;
		КонецЦикла;
		Если СтрокиОставить.Количество() Тогда
			
			ШтрихкодыУпаковокБезИдентификатора = ШтрихкодыУпаковокИсходнаяТаблица.Скопировать(Новый Структура("ИдентификаторПроисхожденияВЕТИС", Неопределено));
		...
		Иначе
			ЕстьИдентификаторПроисхождения = Ложь;
		КонецЕсли;
	КонецЕсли;
				...
				Если СтрокиБезСерий.Количество() Тогда
					...
					Для Каждого ШтрихкодУпаковки Из СтрокаКеша.ШтрихкодыУпаковок Цикл
						Если СтрокаПриемник.ШтрихкодыУпаковок.НайтиПоЗначению(ШтрихкодУпаковки.Значение) = Неопределено Тогда
							СтрокаПриемник.Добавить(ШтрихкодУпаковки.Значение)
						КонецЕсли;
					КонецЦикла;
...
Функция ДанныеШтрихкодаДляДобавленияВКеш(ДанныеШтрихкода, Специфика = Неопределено) Экспорт
	...
	Если Специфика = Перечисления.ВидыПродукцииИС.МолочнаяПродукция Тогда
...
Процедура ДополнитьКешИТаблицуШтрихкодовУпаковок(Форма, ОбновляемаяСтрока) Экспорт
	...
	ЕстьИдентификаторПроисхожденияВЕТИС = КолонкиКеша.Найти("ИдентификаторПроисхожденияВЕТИС") <> Неопределено;
	ПоляПоиска = ИнтеграцияИС.ПоляПоискаМаркируемойПродукции(, ЕстьGTIN, ЕстьИдентификаторПроисхожденияВЕТИС);
	
	ДобавитьШтрихкодУпаковки = Истина;
	ДобавитьШтрихкодУпаковкиВЕТИС = ЕстьИдентификаторПроисхожденияВЕТИС;
	...
	Если ДобавитьШтрихкодУпаковки Тогда
		...
		Иначе
			НоваяСтрока.ШтрихкодУпаковки = ОбновляемаяСтрока.ШтрихкодУпаковки;
		КонецЕсли;
	КонецЕсли;
...
Функция КоличествоПоДаннымКеша(Форма, ОбновляемаяСтрока) Экспорт
	...
	ДанныеШтрихкодовУпаковокГосИС = Форма.ДанныеШтрихкодовУпаковокГосИС;
	ПоляПоиска = ИнтеграцияИС.ПоляПоискаМаркируемойПродукции(,ОбновляемаяСтрока.Свойство("GTIN"),ОбновляемаяСтрока.Свойство("ИдентификаторПроисхожденияВЕТИС"));
	ЗаполнитьЗначенияСвойств(ПоляПоиска, ОбновляемаяСтрока);
	ДанныеКеша = ДанныеШтрихкодовУпаковокГосИС.НайтиСтроки(ПоляПоиска);
	Если ДанныеКеша.Количество() Тогда
