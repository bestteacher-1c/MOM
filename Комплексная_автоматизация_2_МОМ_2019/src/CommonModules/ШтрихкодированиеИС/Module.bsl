...
Функция ШтрихкодыУпаковок(ЗначенияШтрихкодов, ТипыУпаковок = Неопределено, ПроверкаНаGS1 = Ложь) Экспорт
	...
	Для Каждого Штрихкод Из ЗначенияШтрихкодов Цикл
		...
		НоваяСтрока.ИсходныйШтрихкод = Штрихкод;
		...
		Если ПроверкаНаGS1 И СтрНачинаетсяС(Штрихкод, "(") Тогда
			...
			НоваяСтрока = ТаблицаШтрихкодов.Добавить();
			НоваяСтрока.Штрихкод     = ЗначениеШтрихкодаБезСкобок;
			НоваяСтрока.ХэшШтрихкода = ИнтеграцияИС.ХешированиеДанныхSHA256(ЗначениеШтрихкодаБезСкобок);
			НоваяСтрока.ЭтоДобавленный   = Истина;
			НоваяСтрока.ИсходныйШтрихкод = Штрихкод;
		КонецЕсли;
	...
	|ГДЕ
	|	НЕ ШтрихкодыУпаковокТоваров.Ссылка ЕСТЬ NULL
	|	И 1 = 1
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ЗаполнитьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма = Неопределено) Экспорт
	...
	ЗаполнитьПоДаннымУточнения(ДанныеПоШтрихкодам, ПараметрыСканирования);
	ЗаполнитьДаныеПоШтрихкодамНаОснованииСохраненногоВыбора(ДанныеПоШтрихкодам, ПараметрыСканирования);
...
//
// Параметры:
//  ДанныеШтрихкода - Структура - См. ИнициализироватьДанныеШтрихкода.
// Возвращаемое значение:
...
Процедура ВыявитьРасхожденияВСоставахДеревьев(ДеревоУпаковок, ДеревоУпаковокПоДаннымСервиса, ЕстьРасхождения) Экспорт
	...
	ИсходныеЗначенияШрихкода = Новый Соответствие;
	...
	ШтрихкодированиеИССлужебный.НормализоватьДеревоЗначенийРекурсивно(
		ДеревоУпаковок, ПараметрыНормализацииКМ, ИсходныеЗначенияШрихкода);
	
	ВыявитьРасхожденияВСтрокахДеревьев(ДеревоУпаковок, ДеревоУпаковокПоДаннымСервиса, КэшПроверенныхСтрок, ЕстьРасхождения, ТекстОшибкиОтсутствуетПоДаннымСервисаИСМП(), Истина);
	ВыявитьРасхожденияВСтрокахДеревьев(ДеревоУпаковокПоДаннымСервиса, ДеревоУпаковок, КэшПроверенныхСтрок, ЕстьРасхождения, ТекстОшибкиОтсутствуетПоДаннымИнформационнойБазы(), Ложь);
	
	Для Каждого КлючИЗначение Из ИсходныеЗначенияШрихкода Цикл
...
Функция ТаблицаТоварыСоответствуетСпискуУпаковокМаркируемойПродукции(Товары, СписокУпаковок, ПараметрыСканирования) Экспорт
	...
	ВложенныеШтрихкоды  = ВложенныеШтрихкодыПоМассивуУпаковокБезСпецифики(СписокУпаковок, ПараметрыСканирования);
	...
	Для Каждого СтрокаТЧ Из МаркированныеТовары Цикл
		...
		ДанныеСерия.КодовМаркировки = ДанныеСерия.КодовМаркировки + 1;
		СоответствиеХарактеристика.Вставить(СтрокаТЧ.Серия, ДанныеСерия);
		...
		Для Каждого КлючиХарактеристика Из КлючиНоменклатура.Значение Цикл
			Для Каждого КлючиСерия Из КлючиХарактеристика.Значение Цикл
				...
				Если ДанныеСерии.Количество <> ДанныеСерии.КодовМаркировки Тогда
					...
					СтрокаСРасхождением.Номенклатура = КлючиХарактеристика.Ключ;
...
// Возвращаемое значение:
//  См. ИнициализацияВложенныхШтрихкодов
Функция ВложенныеШтрихкодыПоМассивуУпаковокБезСпецифики(МассивУпаковок, ПараметрыСканирования) Экспорт
...
Процедура ЗаполнитьСведенияПоШтрихкодамEAN(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	КодыМаркировкиВводаОстатков = Новый Массив;
	
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		
		Если СтрокаДанных.ЭтоКодВводаОстатков Тогда
			КодыМаркировкиВводаОстатков.Добавить(СтрокаДанных.Штрихкод);
			Продолжить;
		КонецЕсли;
	...
	ДанныеПулаПоКодамМаркировки = РегистрыСведений.ПулКодовМаркировкиСУЗ.ДанныеКодовМаркировки(КодыМаркировкиВводаОстатков);
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		СтрокаИнформации = ИнформацияПоШтрихкодамEAN.Найти(СтрокаДанных.EAN, "ШтрихкодEAN");
		Если СтрокаИнформации <> Неопределено Тогда
...
Процедура ЗаполнитьДанныеКодовМаркировкиВходящимиДанными(ДанныеКодовМаркировки, ДанныеШтрихкодов, ПараметрыСканирования)
	...
	Если ЭтоАдресВременногоХранилища(ПараметрыСканирования.КэшМаркируемойПродукции) Тогда
		...
		Если КэшМаркируемойПродукции.ОтложенныеКодыМаркировки.Количество() > 0 Тогда
			...
			Для Каждого КодМаркировки Из КэшМаркируемойПродукции.ОтложенныеКодыМаркировки Цикл
...
Функция ТекстОшибкиНеПоддерживаетсяВидПродукции(СтрокаДанных, ПоддерживаемыеВидыПродукции, ВидПродукцииШтрихкода)
	...
	Иначе
		ПредставлениеШтрихкода = СтрокаДанных.Штрихкод;
	КонецЕсли;
...
Функция РезультатОбработкиШтрихкодаПриОшибках(ДанныеШтрихкодов, ДанныеШтрихкода, ПараметрыСканирования, ФормаУникальныйИдентификатор) Экспорт
	...
	// Условие для дерева.
	Если ИнтеграцияИСКлиентСервер.ЭтоУпаковкаПоВиду(ДанныеШтрихкода.ВидУпаковки) Тогда
...
Функция РезультатОбработкиТребуетсяСопоставлениеНоменклатуры(ДанныеШтрихкода, СписокШтрихкодов, ПараметрыСканирования)
	...
	Если ЭтоАдресВременногоХранилища(ПараметрыСканирования.КэшМаркируемойПродукции) Тогда
		...
		КэшМаркируемойПродукции = ПолучитьИзВременногоХранилища(ПараметрыСканирования.КэшМаркируемойПродукции);
		КэшМаркируемойПродукции.ОтложенныеКодыМаркировки.Добавить(ДанныеШтрихкода.Штрихкод);
		КэшМаркируемойПродукции.ДанныеДляУточненияСведенийПользователя.Операция = "СопоставлениеНоменклатуры";
		КэшМаркируемойПродукции.ДанныеДляУточненияСведенийПользователя.Данные = ДанныеДляСопоставленияНоменклатуры;
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиПоСтрокамДереваУпаковок(ДанныеКодовМаркировки, ДеревоУпаковок, ДобавленныеСтроки = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	// Для УПД
	ДополнятьДобавленныеСтрокиЕслиНоменклатураНеЗаполнена = ПараметрыСканирования <> Неопределено
		И ЗначениеЗаполнено(ПараметрыСканирования.СсылкаНаОбъект)
		И ТипЗнч(ДобавленныеСтроки) = Тип("Массив");
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если НайденныеСтроки.Количество() Тогда
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Для Каждого КолонкаДерева Из СтрокаДерева.Владелец().Колонки Цикл
				...
				Если Не ЗначениеЗаполнено(НайденнаяСтрока.Номенклатура)
					...
					И НайденнаяСтрока.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
					НайденнаяСтрока.ТребуетсяСопоставление = Истина;
				КонецЕсли;
				
				Если Не ЗначениеЗаполнено(НайденнаяСтрока.Номенклатура) И ДополнятьДобавленныеСтрокиЕслиНоменклатураНеЗаполнена Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.Номенклатура)
				...
				И НоваяСтрока.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
...
//  ДанныеПоШтрихкодам - (См. ИнициализацияДанныхПоШтрихкодам).
//  ПараметрыСканирования - (См. ШтрихкодированиеИСКлиент.ПараметрыСканирования).
Процедура ЗаполнитьПоДаннымУточнения(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Если ПараметрыСканирования.ДополнительныеПараметры.Свойство("ДанныеУточнения") Тогда
		...
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			ОбновитьСоздатьШтрихкодУпаковкиДанныхШтрихкода(
				СтрокаДанных, ИзмененныеСвойства, ПараметрыСканирования);
...
Функция ОбработатьУточнениеДанныхДляУпаковки(Форма, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	Если ТипЗнч(РезультатВыбора) = Тип("Строка") Тогда
	  ...
	  //
		...
		Если Не ЗначениеЗаполнено(СтрокаДерева.ШтрихкодУпаковки)
			И ПараметрыСканирования.СоздаватьШтрихкодУпаковки Тогда
