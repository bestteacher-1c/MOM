...
Процедура ДополнитьДоступныеНедоступныеВидыДеятельностиНДСПоПараметрам(ДоступныеВидыДеятельностиНДС, НедоступныеВидыДеятельностиНДС, ПараметрыЗаполнения) Экспорт
	...
	// Для операций передачи в производство и выбытия товаров запрещается пустой вид деятельности НДС
	Если ОсновнойВидДеятельностиНДСЗакупки = Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию
		И (ПараметрыЗаполнения.ДвижениеТоваровИРаботВПроизводстве)
			ИЛИ ПараметрыЗаполнения.СписаниеТоваровИРаботНаСтатьи
			ИЛИ ПараметрыЗаполнения.ПередачаВЭксплуатацию
			ИЛИ ПараметрыЗаполнения.ПрочееВыбытиеТоваров Тогда
		НедоступныеВидыДеятельностиНДС.Добавить(Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка());
	КонецЕсли;
...
Процедура ЗаполнитьНалогообложениеНДСПродажиПоПараметрам(НовоеНалогообложениеНДС, ТекущееНалогообложениеНДС, ПараметрыЗаполнения) Экспорт
	...
	ИначеЕсли ТекущееНалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт	
			ИЛИ ТекущееНалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров
			ИЛИ ТекущееНалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг Тогда
		...
		ИначеЕсли ПараметрыЗаполнения.ЭтоЗаказ 
			 И ТекущееНалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт
...
#Область ЗаполнениеСуммСФПолученный
	...
	ТекстЗапросаЗаполнениеСуммСФПолученный =
	"ВЫБРАТЬ
	|	Вознаграждение.Ссылка КАК ДокументОснование,
	...
	|	0 КАК СуммаНДСУменьшениеРегл
	|ИЗ
	|	#Ценности КАК Вознаграждение
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСуммыДокументовВВалютахУчета КАК СуммыДокументов
	|		ПО Вознаграждение.Ссылка = СуммыДокументов.Ссылка
