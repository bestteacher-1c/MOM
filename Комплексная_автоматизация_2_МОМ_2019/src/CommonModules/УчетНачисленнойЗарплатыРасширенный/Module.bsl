...
Функция НачисленныеОтпускаПоПериодамИВидам(Сотрудник, ВидыОтпусков, ИсключаемыеРегистраторы = Неопределено, РасчетДенежногоСодержания = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("КатегорииКомпенсаций", КатегорииКомпенсаций);
	...
	РегистрыРасчета.Начисления.СоздатьВТРезультирующийПериодДействия(Запрос.МенеджерВременныхТаблиц,
		"ВидРасчета.ВидОтпуска В (&ВидыОтпусков)
		|И НЕ ВидРасчета.КатегорияНачисленияИлиНеоплаченногоВремени В (&КатегорииКомпенсаций)
