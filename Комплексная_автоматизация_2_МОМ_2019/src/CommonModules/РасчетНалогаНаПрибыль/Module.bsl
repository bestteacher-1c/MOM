...
//
Функция НачислитьНалогНаПрибыль(Движения, ПараметрыПБУ18, Знач Реквизиты, Знач ПараметрРаспределениеНалогаНаПрибыль) Экспорт
	...
	ПрименяетсяПБУ18 = ПараметрыПБУ18.ПрименяетсяПБУ18;
	...
	Если ПараметрыПБУ18.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.Нет Тогда
		СчетДт = ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиНеЕНВД;
	ИначеЕсли ПараметрыПБУ18.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетод Тогда
		СчетДт = ПланыСчетов.Хозрасчетный.ТекущийНалогНаПрибыль;
	Иначе
		СчетДт = ПланыСчетов.Хозрасчетный.РасчетНалогаНаПрибыль;
	КонецЕсли;
...
Функция НачислениеНалогаНаПрибыль(Период, Организация, НалоговаяБаза, СтавкаФедеральныйБюджет, СтавкаРегиональныйБюджет, РаспределениеНалогаНаПрибыль, ПрименяетсяПБУ18 = Истина)
	...
	Если НачалоМесяца(Период) > НачалоНалоговогоПериода Тогда
		...
		Если НЕ ПрименяетсяПБУ18 Тогда
			...
			// Счет текущего налога не используется, проводки по начислению налога - в корр. с 99, 
...
Процедура ОтражениеОНАиОНО(ТаблицаРасчетВременныхРазниц, ВидАктивовОбязательств, ВременныеРазницы, СчетаАктивовИОбязательств = Неопределено)
	...
	Если СуммаДт09 <> 0 Тогда
		...
		Запись.СодержаниеПроводки = НСтр("ru = 'Признание отложенного налогового актива'",
										ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
