...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ВариантПолученияМРЦ = "ВычислениеИЗапрос") Экспорт
	...
	// Установка МРЦ
	Если Кеш.ЗапросИУстановкаМРЦ.Количество() Тогда
		
		РезутатЗапросаСтатусовДляМРЦ = ЗапроситьМРЦДляКодаМаркировки(Кеш.ЗапросИУстановкаМРЦ);
		...
		Если РезутатЗапросаСтатусовДляМРЦ.ДанныеПродукции <> Неопределено Тогда
			...
			Для Каждого СтрокаЗапроса Из Кеш.ЗапросИУстановкаМРЦ Цикл
				
				ДанныеСтроки = ДанныеПродукции.Получить(СтрокаЗапроса.Штрихкод);
		...
		Иначе // Табачная пачка
			...
			Если Детализация = Неопределено
				...
				Если Не ЗначениеЗаполнено(МРЦ)
					И ВариантПолученияМРЦ = "ВычислениеИЗапрос" Тогда
