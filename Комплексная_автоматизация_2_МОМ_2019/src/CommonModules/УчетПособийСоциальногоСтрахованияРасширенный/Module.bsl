...
Функция ДанныеДляРасчетаСреднегоЗаработкаФСССотрудниковСлужебный(ИсходныеДанные, ДополнительныеПараметры)
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТДанныеОВремениОтсутствуют
		|ИЗ
		|	ВТИсходныеДанные КАК ИсходныеДанные
		...
		|			И (ДанныеУчетаВремени.Месяц = ИсходныеДанные.Период)
		|ГДЕ
		|	ДанныеУчетаВремени.ФизическоеЛицо ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|ИЗ
		|	ВТДанныеОВремениОтсутствуют КАК ДанныеОВремениОтсутствуют";
	...
	Иначе
		...
		Запрос.Текст = 
			...
			|	ДанныеОВремениОтсутствуют.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
			|	ДанныеОВремениОтсутствуют.ФизическоеЛицо КАК ФизическоеЛицо,
			|	ДанныеОВремениОтсутствуют.Период КАК Период,
			|	НАЧАЛОПЕРИОДА(ДанныеОВремениОтсутствуют.Период, МЕСЯЦ) КАК ДатаНачала,
			|	КОНЕЦПЕРИОДА(ДанныеОВремениОтсутствуют.Период, МЕСЯЦ) КАК ДатаОкончания
			|ПОМЕСТИТЬ ВТСотрудникиПериоды
			|ИЗ
			|	ВТДанныеОВремениОтсутствуют КАК ДанныеОВремениОтсутствуют
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТаблицаСотрудники КАК ТаблицаСотрудники
			|		ПО ДанныеОВремениОтсутствуют.ФизическоеЛицо = ТаблицаСотрудники.Сотрудник.ФизическоеЛицо
			|			И ДанныеОВремениОтсутствуют.ГоловнаяОрганизация = ТаблицаСотрудники.Сотрудник.ГоловнаяОрганизация
			|
			|СГРУППИРОВАТЬ ПО
			|	ДанныеОВремениОтсутствуют.Период,
			|	ДанныеОВремениОтсутствуют.ФизическоеЛицо,
			|	ДанныеОВремениОтсутствуют.ГоловнаяОрганизация";
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ДанныеУчетаВремени.ДнейБолезниУходаЗаДетьми, ЕСТЬNULL(ДниУходаЗаДетьми.ДнейБолезниУходаЗаДетьми, 0)) КАК ДнейБолезниУходаЗаДетьми,
