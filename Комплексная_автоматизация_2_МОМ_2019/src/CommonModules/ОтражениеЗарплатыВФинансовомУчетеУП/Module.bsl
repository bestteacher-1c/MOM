	...
	Для Каждого Строка Из СтрокиКРаспределению Цикл
		...
		Для Каждого ВидВзноса Из ВидыВзносов Цикл
			...
			Для Индекс = 0 По СтрокиБазы.Количество()-1 Цикл
				...
				Если ВидВзноса.Представление = "Сумма" Тогда
				...
				ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательства
					ИЛИ Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.КомпенсацияЕжегодногоОтпускаОценочныеОбязательства Тогда
					РазницаНУ = НоваяСтрока.СуммаРегл - МассивРаспределения[Индекс];
...
Процедура НачислениеООПриЗакрытииМесяца(Организации, Период) Экспорт
	...
	|		&НачалоМесяца, Организация В (&Организации) ИЛИ &ПоВсемОрганизациям) КАК Настройки
	|ГДЕ
	...
	|			&НачалоМесяца, &КонецМесяца,, Организация В (ВЫБРАТЬ Организация ИЗ ОрганизацииСФормированиемРезервов)) КАК Начисления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("КонецМесяца", Новый Граница(КонецМесяца(Период), ВидГраницы.Включая));
	
	Результат = Запрос.ВыполнитьПакет();
	
	НачисленаЗарплата     = Результат[1].Выбрать();
	СуществующиеДокументы = Результат[2].Выбрать();
	...
	Для Каждого Организация Из Организации Цикл
		...
		НачисленаЗарплата.Сбросить();
		
		Если НЕ НачисленаЗарплата.НайтиСледующий(Организация, "Организация") Тогда
			// Автоматическое формирование документов не требуется, если нет начисленной зарплаты
			Продолжить;
		КонецЕсли;
