...
//		** Значение - Структура - (См. ИнициализироватьПараметрыКодаМаркировки).
...
Функция ИнициализироватьПараметрыКодаМаркировки(ЭлементДанных = Неопределено, ВидПродукции = Неопределено, ВыполнятьРазбор = Истина) Экспорт
	...
	Если ВыполнятьРазбор Тогда
		ПараметрыКодаМаркировки.РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(ИсточникДанных["cis"], ВидПродукции);
	КонецЕсли;
	...
	ПараметрыКодаМаркировки.НаименованиеПродукции = ИсточникДанных["productName"];
	...
	Если ИсточникДанных["countChildren"] <> Неопределено
		...
		Для Каждого СвойстваВложеннойУпаковки Из ИсточникДанных["childrenMap"] Цикл
			...
			ПараметрыКодаМаркировкиВложенный = ИнициализироватьПараметрыКодаМаркировки(СвойстваВложеннойУпаковки, ВидПродукции);
			
			ПараметрыКодаМаркировки.ВложенныеУпаковки.Вставить(
				ПараметрыКодаМаркировкиВложенный.РезультатРазбора, ПараметрыКодаМаркировкиВложенный);
...
Функция ТоварнаяГруппа(Знач ЗначениеПоиска) Экспорт
	...
	Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") Тогда
		...
		ИначеЕсли ЗначениеПоиска = "milk" Тогда
			Возврат Перечисления.ВидыПродукцииИС.МолочнаяПродукция;
	...
	ИначеЕсли ТипЗнч(ЗначениеПоиска) = Тип("ПеречислениеСсылка.ВидыПродукцииИС") Тогда
		...
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.Фотоаппараты Тогда
			Возврат "electronics";
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.МолочнаяПродукция Тогда
			Возврат "milk";
...
Функция ТоварнаяГруппаЧислом(Знач ЗначениеПоиска) Экспорт
	...
	Если ТипЗнч(ЗначениеПоиска) = Тип("Число") Тогда
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(ЗначениеПоиска) = Тип("ПеречислениеСсылка.ВидыПродукцииИС") Тогда
		Если ЗначениеПоиска = Перечисления.ВидыПродукцииИС.Обувь Тогда
			Возврат 1;
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность Тогда
			Возврат 2;
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.Духи Тогда
			Возврат 4;
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.Шины Тогда
