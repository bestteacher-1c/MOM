...
//  ПараметрыСправокНДФЛ - см. УчетНДФЛДокументы.ПараметрыСправокНДФЛ.
//
Процедура СправкиНДФЛОбновитьНалоги(СправкаПоСотруднику, ПараметрыСправокНДФЛ) Экспорт
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанные(ДанныеСправок, ПараметрыСправокНДФЛ);
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанныеУведомлений(ДанныеСправок, ПараметрыСправокНДФЛ.Организация, ПараметрыСправокНДФЛ.НалоговыйПериод);	
	...
	Для каждого ИмяПоля Из УчетНДФЛ.ПоляЗачтеноАвансовыхПлатежей() Цикл
		Если ЗначениеЗаполнено(СправкаПоСотруднику[ИмяПоля]) Тогда
			УчетНДФЛДокументы.УточнитьСтатусРаботающегоПоПатенту(ДанныеСправок, ПараметрыСправокНДФЛ.НалоговыйПериод);
			Прервать;	
		КонецЕсли;
	КонецЦикла;
...
Процедура СправкиНДФЛОтменитьИсправлениеДанныхСотрудника(Форма, СправкаПоСотруднику,НалоговыйПериод, ДатаДокумента) Экспорт
	...
	Если ЧитатьДанныеСотрудников Тогда
		...
		УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанныеСотрудников(ДанныеСправок, НалоговыйПериод, ДатаДокумента, Истина);
...
Процедура СправкиНДФЛОбновитьИтоги(ДанныеСправки, СведенияОДоходах, СведенияОВычетах) Экспорт
	...
	Для Каждого СтрокаИтогов Из ИтогиПоСтавкам Цикл
		...
		УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "ОблагаемаяСуммаДохода");
		УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "ОбщаяСуммаДохода");
	КонецЦикла;
...
Процедура СправкиНДФЛОчиститьДанныеСправки(ДанныеСправки, ДляНалогаНаПрибыль = Ложь) Экспорт
	...
	Для Каждого СтавкаНДФЛ Из Перечисления.НДФЛСтавки Цикл 
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "ОбщаяСуммаДохода", 0);
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "ОблагаемаяСуммаДохода", 0);
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "Исчислено", 0);
		Если Не ДляНалогаНаПрибыль Тогда
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "ЗачтеноАвансовыхПлатежей", 0);
		КонецЕсли;
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "Удержано", 0);
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "Перечислено", 0);
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "Задолженность", 0);
		УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправки, СтавкаНДФЛ, "ИзлишнеУдержано", 0);
	КонецЦикла;	
...
Функция ПоместитьДанныеСправки2НДФЛВХранилище(Форма, ДанныеСотрудника, СтрокиСведенийОДоходах, СтрокиСведенийОВычетах, НомерСправки, Ошибки = Неопределено, НоваяСтрока = Ложь, Ставка = Неопределено, НеВключатьДоходыПоЦеннымБумагам = Ложь, УведомленияОПравеНаВычеты = Неопределено, ПоказыватьИсправляемуюСправку = Ложь, ИсправляемаяСправка = Неопределено, НомерКорректировки = 0) Экспорт
	...
	Если УведомленияОПравеНаВычеты <> Неопределено Тогда
		...
		Для Каждого СтрокаСведений Из УведомленияОПравеНаВычеты Цикл
			СтруктураСведений = Новый Структура(ПоляСведений);
			ЗаполнитьЗначенияСвойств(СтруктураСведений, СтрокаСведений);
...
Процедура ДляНалогаНаПрибыльПрочитатьДанные(ДанныеСправок, СведенияОДоходах, СведенияОВычетах, НалоговыйПериод, ДатаДокумента, Организация,  ОбновлятьНеФиксированныеДанные = Истина) Экспорт
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанныеСотрудников(ДанныеСправок, НалоговыйПериод, ДатаДокумента, ОбновлятьНеФиксированныеДанные);
...
Процедура ДляНалогаНаПрибыльПрочитатьДанныеОДоходахИНалогах(ДанныеСправок, СведенияОДоходах, СведенияОВычетах, НалоговыйПериод, ДатаДокумента, Организация, НомерСправки = Неопределено, Ставка = Неопределено) Экспорт
	...
	Для Каждого ДанныеСправкиПоСотруднику Из ДанныеСправок Цикл
		...
		Для Каждого СтавкаНДФЛ Из Перечисления.НДФЛСтавки Цикл 
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "ОбщаяСуммаДохода", 0);
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "ОблагаемаяСуммаДохода", 0);
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "Исчислено", 0);
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "Удержано", 0);
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "Перечислено", 0);
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "Задолженность", 0);
			УчетНДФЛДокументы.СправкиНДФЛУстановитьЗначениеПоляИтоговПоСтавке(ДанныеСправкиПоСотруднику, СтавкаНДФЛ, "ИзлишнеУдержано", 0);
		КонецЦикла;	
		...
		Для каждого ДанныеСправки Из СправкиПоСотруднику Цикл
			...
			Если ЗаполнятьСправку Тогда
				...
				Если СтрокиИтоговБазы.Количество() > 0 Тогда
					УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокиИтоговБазы[0], "ОбщаяСуммаДохода");
					УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокиИтоговБазы[0], "ОблагаемаяСуммаДохода");
				КонецЕсли;
				УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "Исчислено");
				УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "Удержано");
				УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "Перечислено");
				УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "Задолженность");
				УчетНДФЛДокументы.СправкиНДФЛЗаполнитьПолеИтогов(ДанныеСправки, СтрокаИтогов, "ИзлишнеУдержано");
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;	
