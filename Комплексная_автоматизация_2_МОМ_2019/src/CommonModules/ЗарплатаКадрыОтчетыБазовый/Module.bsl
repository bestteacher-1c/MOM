...
Функция ЗапросДанныеДокументаФизическихЛиц() Экспорт
	...
	Запрос.Текст =
		...
		|	ЛОЖЬ КАК Сторно,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(НачисленияИУдержания.ВидРасчета) = ТИП(Перечисление.ВидыОсобыхНачисленийИУдержаний)
		|			ТОГДА ВЫРАЗИТЬ(НачисленияИУдержания.ВидРасчета КАК Перечисление.ВидыОсобыхНачисленийИУдержаний).Порядок
		|		ИНАЧЕ ВЫБОР
		...
		|			КОНЕЦ * 100000000000000000000 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(2000, 1, 1), ВЫБОР
		...
		|				КОНЕЦ, ДЕНЬ) * 1000000000000 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(2000, 1, 1), ВЫБОР
		|					КОГДА ЕСТЬNULL(НачисленияИУдержания.ДатаНачала, ДАТАВРЕМЯ(1, 1, 1)) < ДАТАВРЕМЯ(2000, 1, 1)
		|						ТОГДА ДАТАВРЕМЯ(2000, 1, 1)
		|					ИНАЧЕ НачисленияИУдержания.ДатаНачала
