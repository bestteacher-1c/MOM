...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, Организация = Неопределено) Экспорт
	...
	Для Каждого СтрокаКодаМаркировки Из ИсходныйМассивСтрок Цикл
		ОбщегоНазначенияКлиентСервер.Проверить(
			ЗначениеЗаполнено(СтрокаКодаМаркировки.ВидПродукции),
			НСтр("ru = 'Внутренняя ошибка. Не заполнен вид продукции в строке'"));
	...
	Для Каждого ПакетКодовМаркировки Из ПакетыКодовМаркировки Цикл
		...
		РезультатЗапросаСтатусовКодовМаркировок = СтатусыКодовМаркировкиПакетно(
			ПакетКодовМаркировки, СтатусыКодовМаркировки);
...
// 
// Параметры:
//  МассивИсходныхСтрок - Массив Из СтрокаТабличнойЧасти - Коллекция строк таблицы формируемой
//                                                    функцией см. ШтрихкодированиеИС.ИнициализацияТаблицыДанныхКодовМаркировки.
// 
// Возвращаемое значение:
...
Функция СтатусыКодовМаркировки(МассивИсходныхСтрок) Экспорт
...
//		** Значение - (См. ИнтерфейсИСМПСлужебный.ПараметрыКодаМаркировки).
//  * ТекстОшибки - Строка - Текст сообщения об ошибке.
Функция СтатусыКодовМаркировкиПакетно(МассивИсходныхСтрок, СтатусыКодовМаркировкиКеш = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из МассивИсходныхСтрок Цикл
		...
		ПараметрыЗапроса.Добавить(
			СтрШаблон(
				"cis=%1",
				КодироватьСтроку(ЗначениеДляЗапроса, СпособКодированияСтроки.КодировкаURL)));
		
	КонецЦикла;
	
	// Для обхода ошибки запроса статусов кодов маркировки с ","
	Если МассивИсходныхСтрок.Количество() = 1
		И СтрНайти(ЗначениеДляЗапроса, ",") Тогда
...
Функция ДанныеПродукцииПоШтрихкодуEAN(Значение) Экспорт
	...
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(
		URLЗапроса,
		КлючСессии,
		ИнтерфейсИСМПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
// 	* ТекстОшибки - Строка - Текст ошибки.
Функция НайтиПродукциюПоИННПроизводителя(ИНН, ВидПродукции, НомерСтраницы = 0, Организация = Неопределено) Экспорт
...
// 	                  - Неопределено - Если при получении данных возникла ошибка.
// 	* ТекстОшибки - Строка - Текст ошибки.
Функция СтатусОбработкиДокументаПоИдентификатору(Идентификатор, Организация = Неопределено, ВидПродукции = Неопределено) Экспорт
...
Функция СоздатьДокументПоСообщению(Организация, Операция, ВидПродукции, ТекстСообщенияJSON, Подпись) Экспорт
	...
	Иначе
		...
		Возврат СоздатьДокумент(URLЗапроса, ТелоЗапроса, Организация, Операция, ВидПродукции);
...
Функция РазобратьИНормализоватьКодМаркировки(КодМаркировки, ВидПродукции = Неопределено) Экспорт
	...
	ДанныеРазбора = ШтрихкодированиеИССлужебный.РазобратьКодМаркировки(КодМаркировки, ВидПродукции);
...
// 	                     - Неопределено - Если при получении данных возникла ошибка.
// 	* ТекстОшибки - Строка - Текст ошибки.
Функция СоздатьДокумент(URLЗапроса, ТелоЗапроса, Организация, Операция, ВидПродукции)
...
Функция ПараметрыЗапроса(ПараметрыЗапроса = Неопределено)
