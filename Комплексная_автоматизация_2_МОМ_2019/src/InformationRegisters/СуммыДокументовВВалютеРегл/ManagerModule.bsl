...
Процедура ОбновитьДвиженияДокументов(РегистраторыСумм) Экспорт
	...
	СоставОбновления.Вставить("ОтражениеДокументовВРеглУчете", РегистрыСведений.ОтражениеДокументовВРеглУчете.СоздатьНаборЗаписей());
	//-- НЕ УТ
			...
			Пока ВыборкаПоТипуЗаписи.Следующий() Цикл
				...
				Иначе
					БазаСНДСРеглПоДокументу = 
						ПредоплатаРегл + Окр(СуммаВзаиморасчетов * КурсВалютыВзаиморасчетов, 2) - Окр(Предоплата * КурсВалютыВзаиморасчетов, 2);
				КонецЕсли;
	...
	Попытка
		...
		Для Каждого ОписаниеОбновления Из СоставОбновления Цикл
			Если ОписаниеОбновления.Ключ = "ОтражениеДокументовВРеглУчете" Тогда
				//++ НЕ УТ
				РеглУчетПроведениеСервер.ЗарегистрироватьКОтражению(СтруктураОбъект, ДопСвойства, СоставОбновления, Ложь);
				Набор = ОписаниеОбновления.Значение;
