...
Процедура ЗаполнитьДеревоСуществующихУпаковокПоДокументу(ДеревоУпаковок,
	...
	|	АгрегированныеКоды.ПредставлениеНоменклатуры           КАК ПредставлениеНоменклатуры,
	...
	|	АгрегированныеКоды.ПредставлениеНоменклатуры
	|
	|ИТОГИ
	...
	Пока ВыборкаХешСуммаУпаковки.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(
				СтрокаПродукции, Выборка, "Номенклатура,Характеристика,GTIN,Количество,Шаблон,СпособВводаВОборот");
			...
			СтрокаПродукции.Содержимое = ИнтеграцияИС.ПредставлениеНоменклатуры(
					Выборка.Номенклатура, Выборка.Характеристика,,, ПредставлениеНоменклатуры);
...
// 	СтруктураМакетаШаблона - Структура - Структура макета шаблона.
Процедура РаспечататьЭтикеткиИЗафиксироватьДатуПечати(
	...
	Для Каждого ОбъектПечати Из ОбъектыПечати Цикл
		...
		ПредставлениеНоменклатуры = ИнтеграцияИСМПКлиентСервер.ПредставлениеGTINОстаткиПоВидуПродукции(
					НоваяСтрока.ПредставлениеНоменклатуры, НоваяСтрока.ВидПродукции);
		НоваяСтрока.ПредставлениеНоменклатуры = ИнтеграцияИС.ПредставлениеНоменклатуры(
			НоваяСтрока.Номенклатура, НоваяСтрока.Характеристика,, НоваяСтрока.Серия, ПредставлениеНоменклатуры);
	...
	Попытка
		...
		ВывестиВТабличныйДокументЭтикетки(ТабличныйДокумент, ОбщаяТаблицаПечати, СтруктураНастроек);
...
Функция КодыМаркировкиПоОснованию(Основания, Напечатанные = Неопределено) Экспорт
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		ТекущаяСтрока.Вставить("Штрихкод",                          ВыборкаИзРезультатаЗапроса.КодМаркировки);
...
Процедура ВывестиВТабличныйДокументЭтикетки(ТабличныйДокумент, ТаблицаОбъектыПечати, СтруктураНастроек)
...
Функция ТаблицаДляПечатиНеРаспечатанныеКодыПоДокументу(ТаблицаНоменклатуры, СтруктураОтбораДокументы, РезервироватьСвободныеКоды, МаркировкаОстатков)
	...
	Для Каждого СтрокаТЧ Из ТаблицаНоменклатуры Цикл
		...
		Для Каждого СтрокаКода Из ПоискСтрокКодов Цикл
			...
			СтрокаКода.Штрихкод      = ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(СтрокаКода.ПолныйКодМаркировки);
...
Процедура СформироватьТаблицуМаркированныхТоваровОснований(Запрос, ИмяВременнойТаблицыСКолонкойДокументОснование)
	...
	|		КОГДА Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.МолочнаяПродукция)
