...
Функция СформироватьКэшируемыеПараметрыУчетнойПолитики(ПоляУчетнойПолитики, ПоляОрганизации = Неопределено) Экспорт
	...
	Если ИсточникПолейУчетнойПолитики <> Неопределено И НЕ ЭтоУправленческаяОрганизация Тогда
		...
		Параметры.ПрименяетсяПБУ18 =
			НалогНаПрибыльБухгалтерскийУчетКлиентСервер.ПрименяетсяПБУ18(ИсточникПолейУчетнойПолитики.ВариантУчетаОтложенногоНалога);
		Параметры.ВедетсяУчетПостоянныхИВременныхРазниц = ИсточникПолейУчетнойПолитики.ВедетсяУчетПостоянныхИВременныхРазниц;
		//-- НЕ УТ
...
Процедура СкорректироватьЗависимыеПараметрыУчетнойПолитики(СтруктураПараметров, Период = Неопределено) Экспорт
	...
	//++ НЕ УТ
	Если НалогНаПрибыльБухгалтерскийУчетКлиентСервер.ВедетсяУчетПостоянныхИВременныхРазниц(СтруктураПараметров.ВариантУчетаОтложенногоНалога) Тогда
		СтруктураПараметров.ВедетсяУчетПостоянныхИВременныхРазниц = Истина;
	Иначе
		СтруктураПараметров.ВедетсяУчетПостоянныхИВременныхРазниц = Ложь;
	КонецЕсли;
	//-- НЕ УТ
	...
	Если НЕ ИдетЗаписьПолитики Тогда
		...
		КонецЕсли;
		//++ НЕ УТ
		СтруктураПараметров.ПрименяетсяПБУ18 = НалогНаПрибыльБухгалтерскийУчетКлиентСервер.ПрименяетсяПБУ18(СтруктураПараметров.ВариантУчетаОтложенногоНалога);
		//-- НЕ УТ
 	КонецЕсли;
