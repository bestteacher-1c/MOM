...
Процедура ОбработатьДанныеДляЗаполненияСловаряСопоставленияНоменклатурыБЭД(Знач НаборСсылок, Знач МетаданныеОбъекта, ЕстьОтработанныеЗаписи,
	...
	Результат = ОбменСКонтрагентамиСлужебный.ПолучитьСловаДляЗаписиВСловарьСопоставленияНоменклатурыБЭД(НаборСсылок, ТипОбъекта);
	...
	Пока ВыборкаНаименований.Следующий() Цикл
		...
		Попытка
			...
			НоваяЗапись.ХешСлова       = ОбменСКонтрагентамиСлужебный.Хеш_MD5_64(НоваяЗапись.Слово);
