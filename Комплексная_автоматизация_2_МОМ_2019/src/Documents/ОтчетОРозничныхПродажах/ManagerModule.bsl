...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если Реквизиты <> Неопределено Тогда
		...
		Запрос.УстановитьПараметр("РеквизитыПервичногоДокумента", УчетУСНСервер.РеквизитыПервичногоДокумента(Реквизиты.Ссылка, ПараметрыПервичногоДокумента));
		ИнформацияОПлатеже = НСтр("ru = 'Магазин: ""%1"".'");
		ИнформацияОПлатеже = СтрШаблон(ИнформацияОПлатеже, Реквизиты.Склад);
		...
		Запрос.УстановитьПараметр("НовыйМетодПризнанияДоходовУСН", Реквизиты.Период >= Константы.ДатаНачалаПризнанияДоходовОтчетомОРозничныхПродажах.Получить());
		//-- НЕ УТ
	КонецЕсли;
