...
Процедура ОснованиеУвольненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		МодульГосударственнаяСлужбаКлиент.УточнитьПараметрыВыбораОснованияУвольненияСГосударственнойСлужбы(Элемент, Объект.Сотрудник, Объект.ДатаУвольнения);
	КонецЕсли;
...
Процедура ОткрытьДенежноеСодержание(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда
		...
		МодульРасчетДенежногоСодержанияКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РасчетДенежногоСодержанияКлиент");
		МодульРасчетДенежногоСодержанияКлиент.ОткрытьФормуВводаСохраняемогоДенежногоСодержания(Объект, "Увольнение","ЕжегодныйОтпуск", ЭтотОбъект, ОписаниеДокумента(ЭтотОбъект),Оповещение);
	КонецЕсли;
...
Процедура ОткрытьДенежноеСодержаниеВыходногоПособия(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда
		Оповещение = Новый ОписаниеОповещения("ОткрытьОткрытьДенежноеСодержаниеЗавершение", ЭтотОбъект);
		МодульРасчетДенежногоСодержанияКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РасчетДенежногоСодержанияКлиент");
		МодульРасчетДенежногоСодержанияКлиент.ОткрытьФормуВводаСохраняемогоДенежногоСодержания(Объект, "Увольнение","ВыходноеПособие", ЭтотОбъект, ОписаниеДокумента(ЭтотОбъект),Оповещение);
	КонецЕсли;
...
Процедура ОбновитьДанныеДляРасчетаСохраняемогоДенежногоСодержания()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
		МодульРасчетДенежногоСодержания = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержания");
		МодульРасчетДенежногоСодержания.ОбновитьДанныеСохраняемогоДенежногоСодержанияДокумента(ЭтотОбъект, ОписаниеДокумента(ЭтотОбъект));
	КонецЕсли;
...
Функция КадровыеДанныеСотрудника(СтрокаДополнительныхРеквизитов = "")
	...
	КадровыеДанные = "ДатаРождения,ДатаПриема,ВидЗанятости,ЗаявленияОПредоставленииСведенийОТрудовойДеятельностиВид";
...
Процедура ЗаполнитьВторичныеДанныеСотрудникаВФорме(ЗаполнитьВидДоговора = Истина)
	...
	Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		...
		Если ЗаполнитьВидДоговора Тогда
			СтрокаДополнительныхРеквизитов = "ВидДоговора";
		КонецЕсли;
		
		КадровыеДанные = КадровыеДанныеСотрудника(СтрокаДополнительныхРеквизитов);
		Если КадровыеДанные <> Неопределено Тогда
			
			ДатаПриемаНаРаботу = КадровыеДанные.ДатаПриема;
			Если ЗаполнитьВидДоговора Тогда
				Объект.ВидДоговора = КадровыеДанные.ВидДоговора;
