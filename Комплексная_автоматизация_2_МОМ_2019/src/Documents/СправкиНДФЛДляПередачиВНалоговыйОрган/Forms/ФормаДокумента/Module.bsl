...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УчетНДФЛФормы.СправкиНДФЛЗаполнитьСписокКонтролируемыхПолей(ЭтотОбъект);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		Если ДанныеТекущейСтроки <> Неопределено Тогда
			УчетНДФЛКлиентСервер.СправкиНДФЛУстановитьИнфонадписьИсправления(ИнфоНадписьИсправления, 
...
Процедура ДатаПриИзмененииНаСервере()
	...
	УчетНДФЛФормы.СправкиНДФЛУстановитьСписокВыбораРегистрацийВНО(ЭтотОбъект);
	УчетНДФЛФормы.СправкиНДФЛУстановитьКодИФНС(Объект.РегистрацияВНалоговомОргане, Объект.КодИФНС);
	УчетНДФЛФормы.СправкаНДФЛУстановитьИнфонадписьОписаниеДоходовОрганизации(ЭтотОбъект,,,,Объект.НалоговыйПериод >= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли()) И Объект.НеВключатьДоходыПоЦеннымБумагам);
...
Процедура РегистрацияВНалоговомОрганеОткрытие(Элемент, СтандартнаяОбработка)
	...
	УчетНДФЛКлиент.СправкиНДФЛРегистрацияВНалоговомОрганеОткрытие(ЭтотОбъект, СтандартнаяОбработка);
...
Процедура НалоговыйПериодПриИзмененииНаСервере()
	...
	УчетНДФЛФормы.СправкиНДФЛУстановитьСписокВыбораРегистрацийВНО(ЭтотОбъект);
	УчетНДФЛФормы.СправкиНДФЛУстановитьКодИФНС(Объект.РегистрацияВНалоговомОргане, Объект.КодИФНС);
	УчетНДФЛФормы.СправкаНДФЛУстановитьИнфонадписьОписаниеДоходовОрганизации(ЭтотОбъект,,,,Объект.НалоговыйПериод 
	...
	ПараметрыСправокНДФЛ = УчетНДФЛДокументы.ПараметрыСправокНДФЛ();
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанные(Объект.Сотрудники, ПараметрыСправокНДФЛ, Истина, Объект.НомерКорректировки);
	
	ЗаполнитьИнфоНадписиОДокументахУдостоверяющихЛичность(Объект.Сотрудники, Объект.НалоговыйПериод);
	
	УчетНДФЛФормы.СправкиНДФЛУстановитьЗаголовокПоляРегистрацияВНО(ЭтотОбъект, Истина);
...
Процедура НеВключатьДоходыПоЦеннымБумагамПриИзмененииНаСервере()
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанныеОДоходахИНалогах(ДанныеСправок, ПараметрыСправокНДФЛ);
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Если Объект.Сотрудники.Количество() > 0 Тогда
		...
		УчетНДФЛКлиентСервер.СправкиНДФЛУстановитьИнфонадписьИсправления(ИнфоНадписьИсправления, ДанныеТекущейСтроки, 
		ДокументПроведен);
...
Процедура ПрочитатьДанныеСправок()
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанныеСотрудников(Объект.Сотрудники, Объект.НалоговыйПериод, Объект.Дата, 
	Не ДокументПроведен);
...
Процедура ОбработкаПодбораНаСервере(ДобавляемыеСотрудники)
	...
	УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанные(ДанныеСправок, ПараметрыСправокНДФЛ, Истина, Объект.НомерКорректировки);
...
Процедура ПрочитатьДанныеФизическогоЛица(ФизическоеЛицо)
	...
	Если СтрокиПоСотруднику.Количество() > 0 Тогда
		УчетНДФЛДокументы.СправкиНДФЛПрочитатьДанныеСотрудников(СтрокиПоСотруднику, Объект.НалоговыйПериод, Объект.Дата, Истина);
...
Процедура ОтменитьИсправлениеДанныхСотрудника()
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		УчетНДФЛФормы.СправкиНДФЛОтменитьИсправлениеДанныхСотрудника(ЭтотОбъект, ДанныеТекущейСтроки, Объект.НалоговыйПериод, Объект.Дата);
...
Процедура ОбновитьНалогиНаСервере()
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		УчетНДФЛФормы.СправкиНДФЛОбновитьНалоги(ДанныеТекущейСтроки, ПараметрыСправокНДФЛ);
