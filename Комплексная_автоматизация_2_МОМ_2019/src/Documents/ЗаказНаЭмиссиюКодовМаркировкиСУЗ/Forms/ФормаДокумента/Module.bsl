...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоМаркировкаОстатков Тогда
		...
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			...
			Если Не СтрокаТовары.ПолноеОписаниеОстатков Тогда
				...
				ИначеЕсли ЭтоЛегкаяПромышленность Тогда
					...
					КодТНВЭД = ИнтеграцияИСМПСлужебный.КодТНВЭДДляПередачиВИСМП(СтрокаТовары.КодТНВЭД);
...
Процедура НастроитьЭлементыФормыНаСервере()
	...
	ТрансграничнаяТорговля            = Перечисления.СпособыВводаВОборотСУЗ.ТрансграничнаяТорговля;
	ДоступностьТрансграничнойТорговли = ИнтеграцияИСМПКлиентСервер.ДоступностьТрансграничнойТорговлиПоВидуПродукции(Объект.ВидПродукции);
	
	ДоступностьСпособаВвода = Новый Соответствие();
	ДоступностьСпособаВвода.Вставить(ТрансграничнаяТорговля, ДоступностьТрансграничнойТорговли);
	ДоступностьСпособаВвода.Вставить(
		Перечисления.СпособыВводаВОборотСУЗ.МаркировкаОстатков,
		ИнтеграцияИСМПКлиентСервер.ВидПродукцииПодлежитМаркировкеОстатков(Объект.ВидПродукции)
	);
	...
	ЭлементВСпискеЗначений = Элементы.ТоварыСпособВводаВОборот.СписокВыбора.НайтиПоЗначению(ТрансграничнаяТорговля);
	Если ДоступностьТрансграничнойТорговли И ЭлементВСпискеЗначений = Неопределено Тогда
		Элементы.ТоварыСпособВводаВОборот.СписокВыбора.Добавить(ТрансграничнаяТорговля);
	ИначеЕсли Не ДоступностьТрансграничнойТорговли И ЭлементВСпискеЗначений <> Неопределено Тогда
		Элементы.ТоварыСпособВводаВОборот.СписокВыбора.Удалить(ЭлементВСпискеЗначений);
	КонецЕсли;
...
Процедура ОчисткаGTINПриИзмененииНоменклатуры(ТекущиеДанные)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Номенклатура)
		Или ТекущиеДанные.ХарактеристикиИспользуются И Не ЗначениеЗаполнено(ТекущиеДанные.Характеристика) Тогда
		ТекущиеДанные.GTIN = "";
	КонецЕсли;
...
Процедура ЗаполнитьПоляСтрокиТоварыПоСтатистикеСервер(ТекущаяСтрока)
	...
	ИначеЕсли Не ЗначениеЗаполнено(ТекущиеДанные.СпособФормированияСерийногоНомера) Тогда
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	ЭтоЛегкаяПромышленность = Ложь;
	ЭтоМолочнаяПродукция    = Ложь;
	ЭтоОбувь                = Ложь;
	ЭтоТабак                = Ложь;
	ЭтоШины                 = Ложь;
	
	Если Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ЛегкаяПромышленность") Тогда
		ЭтоЛегкаяПромышленность = Истина;
	ИначеЕсли Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукция") Тогда
		ЭтоМолочнаяПродукция = Истина;
	ИначеЕсли Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Обувь") Тогда
		ЭтоОбувь = Истина;
	ИначеЕсли Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табак") Тогда
		ЭтоТабак = Истина;
	ИначеЕсли Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Шины") Тогда
		ЭтоШины = Истина;
	КонецЕсли;
	...
	Если ЭтоТабак Тогда
		...
		Элементы.ТоварыМаксимальнаяРозничнаяЦена.Видимость = Истина;
		Элементы.ГруппаОбувнаяПродукция.Видимость          = Ложь;
	...
	ИначеЕсли ИнтеграцияИСКлиентСервер.ЭтоПродукцияИСМП(Объект.ВидПродукции) Тогда
		...
		Элементы.ТоварыМаксимальнаяРозничнаяЦена.Видимость = Ложь;
		Элементы.ГруппаОбувнаяПродукция.Видимость          = Истина;
		
	КонецЕсли;
	
	ВидПродукцииПодлежитМаркировкеОстатков = ИнтеграцияИСМПКлиентСервер.ВидПродукцииПодлежитМаркировкеОстатков(Объект.ВидПродукции);
	...
	Элементы.БезУчетаНоменклатуры.Видимость                                = ВидПродукцииПодлежитМаркировкеОстатков И Форма.ЭтоМаркировкаОстатков;
	Элементы.ВвезенПослеДатыОбязательнойМаркировки.Видимость               = ВидПродукцииПодлежитМаркировкеОстатков И Форма.ЭтоМаркировкаОстатков;
	Элементы.ПриобретенПроизведенПослеДатыОбязательнойМаркировки.Видимость = ВидПродукцииПодлежитМаркировкеОстатков И Форма.ЭтоМаркировкаОстатков;
	Элементы.ТоварыПолноеОписаниеОстатков.Видимость                        = ВидПродукцииПодлежитМаркировкеОстатков И Форма.ЭтоМаркировкаОстатков;
	...
	Иначе
		...
		Элементы.ТоварыТоварныйЗнак.Видимость        = ЭтоТабак;
	...
	ДоступныеШаблоны = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ВидПродукции, ВключатьКомлекты);
...
Процедура ЗапроситьКлючСессииНачало()
	...
	Если Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табак") Тогда
	...
	Если ПроверяемыеGTIN.Количество() > 0 Тогда
		...
		Для Каждого СтрокаGTIN Из СоответствиеGTIN Цикл
			...
			Если РезультатПроверки.Получить(СтрокаGTIN.Ключ) <> Неопределено 
				...
				Продолжить;
			ИначеЕсли РезультатПроверки.Получить(СтрокаGTIN.Ключ) = Неопределено
				И ШтрихкодированиеИСКлиентСервер.ЭтоВнутреннийШтрихкод(СтрокаGTIN.Ключ) Тогда
				Продолжить;
			КонецЕсли;
		...
		Для Каждого СтрокаДокумента Из Объект.Товары Цикл
			...
			Если ЭтоМаркировкаОстатков И Не СтрокаДокумента.ПолноеОписаниеОстатков Тогда
				Продолжить;
			КонецЕсли;
