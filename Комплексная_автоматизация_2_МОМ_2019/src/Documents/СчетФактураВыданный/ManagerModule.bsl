...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ИнициализироватьКурсыВалют(Запрос, ДокументСсылка);
...
Функция ПолучитьТаблицуТоваровИсправление(МенеджерВременныхТаблиц, ДополнительныеПараметры)
	...
	|ГДЕ
	|	НЕ ТаблицаТоваровСФ.Количество + ЕСТЬNULL(ТаблицаТоваровДО.Количество, 0) = 0
	|	ИЛИ НЕ ТаблицаТоваровСФ.Сумма + ЕСТЬNULL(ТаблицаТоваровДО.Сумма, 0) = 0
	|	ИЛИ НЕ ТаблицаТоваровСФ.СуммаНДС + ЕСТЬNULL(ТаблицаТоваровДО.СуммаНДС, 0) = 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
