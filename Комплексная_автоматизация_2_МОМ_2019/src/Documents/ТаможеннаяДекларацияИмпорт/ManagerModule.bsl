	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			Если ВыборкаДокументов.Проведен Тогда
				...
				Регистры.Вставить("СуммыДокументовВВалютеРегл");
				Регистры.Вставить("ПрочиеРасходы");
				Регистры.Вставить("СебестоимостьТоваров");
				...
				ИнициализироватьДанныеДокумента(ДокументОбъект.Ссылка, ДокументОбъект.ДополнительныеСвойства, Регистры);
				
				ДоходыИРасходыСервер.ОтразитьПрочиеРасходы(ДокументОбъект.ДополнительныеСвойства, ДокументОбъект.Движения, Отказ);
				ДоходыИРасходыСервер.ОтразитьСебестоимостьТоваров(ДокументОбъект.ДополнительныеСвойства, ДокументОбъект.Движения, Отказ);
				ВзаиморасчетыСервер.ОтразитьСуммыДокументаВВалютеРегл(ДокументОбъект.ДополнительныеСвойства, ДокументОбъект.Движения, Отказ);
				
				Если Отказ Тогда
					ТекстОшибки = НСтр("ru = 'Возникла ошибка при отражении движений регистров по документу: %Документ%'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДокументОбъект.Ссылка);
				...
				Если Отказ Тогда
					ТекстОшибки = НСтр("ru = 'Возникла ошибка при выполнении контроля результатов проведения при обновлении движений по документу: %Документ%'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДокументОбъект.Ссылка);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;	
