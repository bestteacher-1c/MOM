...
Процедура УстановитьВидимостьКнопкиВыгрузкиФайлаИПодписи()
	...
	Ответы            = КонтекстЭДОСервер.ПолучитьОтветыНаЗапросИОН(Объект.Ссылка); 
	ПоказатьКнопку    = Ответы.Количество() > 0;
	
	Элементы.ВыгрузитьДокументСверкиСПодписьюФНС.Видимость = ПоказатьКнопку;
