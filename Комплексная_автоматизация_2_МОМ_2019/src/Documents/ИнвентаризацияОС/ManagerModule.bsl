...
Функция ДанныеЗаполненияНаОснованииИнвентаризации24(ДокументОснование, Дата, ДействияДокументовНаОсновании = "", Знач МассивНомеровСтрок = Неопределено) Экспорт
	...
	|	ВЫРАЗИТЬ(ИнвентаризацияОС.ОбъектУчета КАК Справочник.ОбъектыЭксплуатации).АмортизационнаяГруппа КАК АмортизационнаяГруппа,
	|	ВЫРАЗИТЬ(ИнвентаризацияОС.ОбъектУчета КАК Справочник.ОбъектыЭксплуатации).НаправлениеДеятельности КАК НаправлениеДеятельности,
...
//
Функция ДанныеУчета(Организация, Подразделение, Дата, ХозяйственнаяОперация, СписокОС = Неопределено) Экспорт
...
Функция ЗаполнитьДанныеУчета(Объект, КоллекцияСтрок, ЭтоЧтениеОбъекта, ЗаполнитьСлужебныеРеквизиты = Ложь) Экспорт
...
Функция ПечатьИНВ1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет КАК ТаблицаРегистра
	...
	|			И (НЕ &ИспользоватьВнеоборотныеАктивы2_4
	|					ИЛИ ТаблицаОС.Дата < &ДатаНачалаУчета24)
	|		
	|		СГРУППИРОВАТЬ ПО
	...
	|			И ТаблицаОС.Дата >= &ДатаНачалаУчета24
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ТаблицаРегистра.Организация,
	|			ТаблицаРегистра.ОсновноеСредство) КАК СрезПериод
	...
	|	ТаблицаРегистра.Период КАК Период,
	|	МАКСИМУМ(ТаблицаРегистра.Регистратор) КАК Регистратор
	|ПОМЕСТИТЬ ПервоначальныеСведенияОС_22
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаРегистра.Организация КАК Организация,
	|		ТаблицаРегистра.ОсновноеСредство КАК ОсновноеСредство,
	...
	|	ИЗ
	|		ТаблицаОС КАК ТаблицаОС
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПервоначальныеСведенияОСБухгалтерскийУчет КАК ТаблицаРегистра
	|			ПО (ТаблицаРегистра.Организация = ТаблицаОС.Организация)
	|				И (ТаблицаРегистра.ОсновноеСредство = ТаблицаОС.ОсновноеСредство)
	|				И (ТаблицаРегистра.Период <= ТаблицаОС.Дата)
	|				И (ТаблицаРегистра.Активность)
	...
	|ПОМЕСТИТЬ ПервоначальныеСведенияОС_24
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаРегистра.Организация КАК Организация,
	|		ТаблицаРегистра.ОсновноеСредство КАК ОсновноеСредство,
	...
	|							ЕСТЬNULL(ВводОстатковВнеоборотныхАктивов.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)))) КАК Арендодатель
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОС_22 КАК ПервоначальныеСведенияОС
	...
	|		ПО (ПервоначальныеСведенияОС.Регистратор = ПринятиеКУчетуОС.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеАрендованныхОС КАК ПоступлениеАрендованныхОС
	|		ПО (ПервоначальныеСведенияОС.Регистратор = ПоступлениеАрендованныхОС.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеПредметовЛизинга КАК ПоступлениеПредметовЛизинга
	|		ПО (ПервоначальныеСведенияОС.Регистратор = ПоступлениеПредметовЛизинга.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов.ОС КАК ВводОстатковВнеоборотныхАктивов
	|		ПО (ПервоначальныеСведенияОС.Регистратор = ВводОстатковВнеоборотныхАктивов.Ссылка)
	...
	|		КОГДА ПервоначальныеСведенияОС.ДокументВводаВЭксплуатациюБУ ССЫЛКА Документ.ПоступлениеАрендованныхОС 
	|			ТОГДА ""Поступление арендованных ОС""
	|		КОГДА ПервоначальныеСведенияОС.ДокументВводаВЭксплуатациюБУ ССЫЛКА Документ.ПоступлениеПредметовЛизинга 
	|			ТОГДА ""Поступление предметов лизинга""
	|		КОГДА НЕ ВводОстатковВнеоборотныхАктивов.Ссылка ЕСТЬ NULL 
	...
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК ДокументАрендыПредставление,
	|	РеестрДокументов.НомерДокументаИБ КАК ДокументАрендыНомер,
	|	ПервоначальныеСведенияОС.ДатаВводаВЭксплуатациюБУ КАК ДокументАрендыДата,
	...
	|			И ТаблицаОС.Организация = МестонахождениеОС.Организация
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОС_24 КАК ПервоначальныеСведенияОС
	|		ПО ТаблицаОС.ОсновноеСредство = ПервоначальныеСведенияОС.ОсновноеСредство
	|			И ТаблицаОС.Организация = ПервоначальныеСведенияОС.Организация
	|
	...
	Запрос.УстановитьПараметр("ИспользоватьВнеоборотныеАктивы2_2", ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_2"));
	Запрос.УстановитьПараметр("ИспользоватьВнеоборотныеАктивы2_4", ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4"));
...
Функция ТекстЗапросаДанныеУчета22(ХозяйственнаяОперация)
	...
	Иначе
		...
		|				ИЛИ НЕ &ПоСпискуОС
		|					И Организация = &Организация) КАК Состояния
		|ГДЕ
		|	(&ПоСпискуОС
		|			ИЛИ Состояния.Состояние В (ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету), ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету)))
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство
		|;
		...
		|ГДЕ
		|	ПервоначальныеСведенияОС.Арендодатель <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство
		|;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	Хозрасчетный.Ссылка В ИЕРАРХИИ(&СчетаВложенияВоВнеоборотныеАктивы)";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("СчетаВложенияВоВнеоборотныеАктивы", ПланыСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы);
	СчетаВложения = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
 	...
 	Пока Выборка.Следующий() Цикл
 		...
 		Попытка
			...
			Если НЕ ЗначениеЗаполнено(ДанныеОбъекта.ХозяйственнаяОперация) Тогда
				...
				ЕстьСчетаУчета = Ложь;
				ЕстьСчетаВложения = Ложь;
				ЕстьСчетаСтроительство = Ложь;
				Для каждого ДанныеСтроки Из ДанныеОбъекта.ОС Цикл
					Если ДанныеСтроки.УдалитьСчетУчета = ПланыСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств Тогда
						ЕстьСчетаСтроительство = Истина;
					ИначеЕсли СчетаВложения.Найти(ДанныеСтроки.УдалитьСчетУчета) <> Неопределено Тогда
						ЕстьСчетаВложения = Истина;
					ИначеЕсли ЗначениеЗаполнено(ДанныеСтроки.УдалитьСчетУчета) Тогда
						ЕстьСчетаУчета = Истина;
					КонецЕсли;
					Если ЕстьСчетаВложения И ЕстьСчетаСтроительство ИЛИ ЕстьСчетаУчета Тогда
						Прервать;
					КонецЕсли; 
				КонецЦикла; 
				
				Если ЕстьСчетаУчета Тогда
					ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИнвентаризацияОС;
				ИначеЕсли ЕстьСчетаВложения Тогда
					ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИнвентаризацияВложенийВОС;
				ИначеЕсли ЕстьСчетаСтроительство Тогда
					ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИнвентаризацияНезавершенногоСтроительства;
				Иначе
					ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИнвентаризацияОС;
				КонецЕсли;
