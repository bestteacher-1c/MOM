...
Процедура РассчитатьПослеЗаполнения(ДокументОбъект)
	...
	ДополнительныйНабор = РегистрыСведений.ИсходныеДанныеПерерасчетов.СоздатьНаборЗаписей();
	ДополнительныйНабор.Отбор.Регистратор.Установить(ВременныйРегистраторПерерасчета);
	ДополнительныйНабор.ОбменДанными.Загрузка = Истина;
	...
	Если ИсправлениеВТекущемПериоде Тогда
		...
		Если МетаданныеРегистра.Имя = "Начисления" Тогда
			РегистрыРасчета.Начисления.СторнироватьДвиженияВТекущемПериоде(НаборДляЗаполненияПерерасчета, ДокументОбъект.СторнируемыйДокумент, Истина);
			УстановитьПривилегированныйРежим(Истина);
			ДополнительныйНабор.Записать();
	...
	Иначе
		...
		Для Каждого СтрокаСторно Из ДокументОбъект.Сторнировано Цикл
			...
			НоваяСтрокаДополнительная = ДополнительныйНабор.Добавить();
