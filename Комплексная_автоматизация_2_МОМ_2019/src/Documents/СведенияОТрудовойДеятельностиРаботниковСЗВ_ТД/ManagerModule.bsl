			...
			Иначе
				...
				Иначе
					...
					НаборЗаписейМероприятие.Должность.Значение = СокрЛП(ВыборкаПоРаботникам.ПредставлениеДолжности) 
						...
						+ ?(ЗначениеЗаполнено(ВыборкаПоРаботникам.РазрядКатегория), ", " + СокрЛП(ВыборкаПоРаботникам.РазрядКатегория), "");
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаМероприятия Из ДокументОбъект.Мероприятия Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаМероприятия.СотрудникЗаписи)
				...
				СтрокаМероприятия.СотрудникЗаписи = ЗарплатаКадрыВызовСервера.ОсновнойСотрудникФизическогоЛица(
...
Функция ПрочитатьПротоколПриема(ДокументОбъект, АдресДанныхПротоколаПриемки, ИмяФайла) Экспорт
	...
	Попытка
		...
		КодФормы = ПолучитьСвойство(ДеревоXML, "УПП/ПроверяемыйДокумент/КодФормы");
		ИмяФайлаПротокола = ПолучитьСвойство(ДеревоXML, "УПП/ПроверяемыйДокумент/Файл/ИмяФайла");
		РегистрационныйНомерПФР = ПолучитьСвойство(ДеревоXML, "СлужебнаяИнформация/Контрагент/РегистрационныйНомер");
		РезультатПроверки = ПолучитьСвойство(ДеревоXML, "СлужебнаяИнформация/РезультатПроверки");
	...
	ДеревоСведений = ПолучитьСвойство(ДеревоXML, "УПП/ПроверочныйМодуль/ПроверкаФайлов");
	СписокФайлов = КоллекцияОбъектовXDTO(ДеревоСведений, "Файл");
	Для Каждого ДанныеФайла Из СписокФайлов Цикл
		
		ИмяПринятогоФайла = ДанныеФайла.ИмяФайла;
		КоллекцияРезультатов = КоллекцияОбъектовXDTO(ДанныеФайла, "Результат");
		Для Каждого Результат Из КоллекцияРезультатов Цикл
			
			БлокиПроверки = КоллекцияОбъектовXDTO(Результат, "БлокПроверок");
			Для Каждого БлокПроверок Из БлокиПроверки Цикл
				
				Проверки = КоллекцияОбъектовXDTO(БлокПроверок, "Проверка");
				Для Каждого Проверка Из Проверки Цикл
					
					Если Проверка.КодРезультата <> "00" Тогда
						
						ОшибкиПроверки = ПолучитьСвойство(Проверка, "Ошибки");
						Если ОшибкиПроверки <> Неопределено Тогда
							
							Ошибки = КоллекцияОбъектовXDTO(ОшибкиПроверки, "Ошибка");
							Для Каждого Ошибка Из Ошибки Цикл
								
								КлючОшибки = ПолучитьСвойство(Ошибка, "Объект/Значение");
								Если КлючОшибки = Неопределено Тогда
									КлючОшибки = ПолучитьСвойство(Ошибка, "ОжидаемоеЗначение");
								КонецЕсли;
...
Функция ПолучитьСвойство(ДеревоXML, Путь)
	
	СписокСвойств = СтрРазделить(Путь, "/");
	
	Объект = ОбъектСвойства(ДеревоXML, СписокСвойств[0]);
	...
	Для Каждого ИмяСвойства Из СписокСвойств Цикл
		...
		Объект = Объект[Свойство.Имя];
		
	КонецЦикла;
	
	Если ТипЗнч(Объект) = Тип("ЗначениеXDTO") Тогда
...
Функция ОбъектСвойства(ДеревоXML, ИмяСвойства)
	
	Для Каждого Свойство Из ДеревоXML.Свойства() Цикл
		
		Объект = ДеревоXML[Свойство.Имя];
		Если Свойство.Имя = ИмяСвойства Тогда
			Возврат Объект;
		КонецЕсли;
		
		Если ТипЗнч(Объект) = Тип("ОбъектXDTO") Тогда
			ПодчиненноеСвойство = ОбъектСвойства(Объект, ИмяСвойства);
...
Функция КоллекцияОбъектовXDTO(ДеревоXDTO, ИмяКоллекции)
