...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СотрудникЗаписи = ЗарплатаКадрыВызовСервера.ОсновнойСотрудникФизическогоЛица(
		Параметры.Сотрудник, Организация, ТекущаяДатаСеанса());
...
Процедура ЗаполнитьМероприятияНаСервере()
	...
	ДанныеЗаполнения = ЭлектронныеТрудовыеКнижки.ДанныеТрудовойДеятельностиСотрудников(
		Истина, СписокСотрудников, Организация, ЭлектронныеТрудовыеКнижки.ДатаНачалаУчета(), Мин(КонецМесяца(ОтчетныйПериод), ДатаДокумента), , РедактируемыйДокументСсылка);
	
	Если ДанныеЗаполнения.Количество() > 0 Тогда
		
		ДанныеЗаполненияСотрудника = ДанныеЗаполнения[0];
		ЗаполнитьДаннымиМероприятий(ДанныеЗаполненияСотрудника.ТрудоваяДеятельность);
