	...
	Если Элементы.РедактироватьСписком.Заголовок = НСтр("ru = 'Редактировать в списке'") Тогда
		...
		Если ТипЗнч(Расшифровка) = Тип("Структура") 
		   И Расшифровка.Свойство("ИДГруппировки", ИДГруппировки) Тогда
			...
			НайденнаяСтрока = ФинансоваяОтчетностьКлиент.ПодчиненныйЭлементЭлементаФормы(ДеревоЭлементовБюджета, "ИДГруппировки", ИДГруппировки);
...
Процедура ВыгрузитьВExcel(Команда)
	...
	#Если МобильныйКлиент Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Функциональность не поддерживается в мобильном клиенте'"));
		Возврат;
	#КонецЕсли
...
Функция МожноВыгрузитьТекущуюТаблицу(ВерхТекущейОбласти, ИДТаблицы, КолонкиПоИзмерениям)
	...
	Если Элементы.ПанельБюджет.ТекущаяСтраница = Элементы.Таблица Тогда
		...
		Если ТипЗнч(Расшифровка) = Тип("Структура") 
			...
			ТекущаяСтрока = ФинансоваяОтчетностьСервер.ПодчиненныйЭлемент(ДеревоЭлементовБюджета, "ИДГруппировки", ИДГруппировки);
	...
	Если ИндексСтроки <> Неопределено Тогда
		...
		Если Расшифровка <> Неопределено Тогда
			...
			Если ТипЗнч(Расшифровка) = Тип("Структура") 
				...
				НайденнаяСтрока = ФинансоваяОтчетностьСервер.ПодчиненныйЭлемент(ДеревоЭлементовБюджета, "ИДГруппировки", ИДГруппировки);
				Если НайденнаяСтрока <> Неопределено Тогда
					Элементы.ДеревоЭлементовБюджета.ТекущаяСтрока = НайденнаяСтрока.ПолучитьИдентификатор();
				КонецЕсли;
...
Процедура ДобавитьСтрокуВРежимеТаблицы(ПараметрыРедактированияОбласти)
	...
	НайденнаяСтрока = ФинансоваяОтчетностьКлиент.ПодчиненныйЭлементЭлементаФормы(ДеревоЭлементовБюджета, "ИДГруппировки",
...
Функция ВставитьОбластьНаСервере(СоответствиеИзмерений, ДополнительныеПараметры)
	...
	ГруппировкаДерева = ФинансоваяОтчетностьСервер.ПодчиненныйЭлемент(ДеревоДляВыводаОтчета, "ИДГруппировки", Расшифровка.ИДГруппировки);
