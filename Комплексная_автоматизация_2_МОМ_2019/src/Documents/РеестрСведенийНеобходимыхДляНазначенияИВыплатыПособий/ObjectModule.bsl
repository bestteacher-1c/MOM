	...
	Если Не ПовторяющиесяЗаявления.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ВыборкаПоЗаявлению.Следующий() Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Выборка, "Заявление", Текст);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПроверитьТребованияФормата(Отказ)
	...
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		...
		Если Строка.ИзвещениеИзФССИспользование Тогда
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ИзвещениеИзФССНомер");
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ИзвещениеИзФССДата");
		КонецЕсли;
		...
		Если РеквизитыЗаявления.СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.ПочтовымПереводом Тогда
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "АдресПолучателяФактический", ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
		...
		Если РеквизитыЗаявления.СтатусНалогоплательщикаПолучателя = СтатусРезидент Тогда
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "АдресПолучателяРегистрации", ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьЗаполнениеПоРегламентуФСС() Экспорт
	...
	ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "КодПодчиненностиФСС");
	ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "ОГРН");
	ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "ИНН");
	ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "Руководитель");
	
	Если ЗарплатаКадры.ЭтоЮридическоеЛицо(Организация) Тогда
		ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "КПП");
		ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "ГлавныйБухгалтер");
		ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "ДолжностьРуководителя");
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокСведенийПоВременнойНетрудоспособности(Отказ)
	...
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		...
		ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "НаименованиеМедицинскойОрганизации");
		ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОГРНМедицинскойОрганизации");
		...
		Если ЗначениеЗаполнено(Строка.Заявление) Тогда
			...
			Если РеквизитыЗаявления.ТипЗанятостиРаботника = Перечисления.ВидыЗанятости.Совместительство
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "НомерЛисткаПоОсновномуМестуРаботы", Текст);
			КонецЕсли;
			...
			Если (РеквизитыЗаявления.ВидПособия = Перечисления.ПособияНазначаемыеФСС.ПособиеПоБеременностиИРодам
				...
				Если Строка.КодПричиныНетрудоспособности <> "05" Тогда
					...
					СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "КодПричиныНетрудоспособности", Текст);
				КонецЕсли;
				...
				Если РеквизитыЗаявления.ПорядокРасчетаСреднегоЗаработка <> Перечисления.СпособыРасчетаСреднегоЗаработкаДляОплатыПособия.СреднийЗаработок Тогда
					...
					Если Строка.ОсвобождениеДатаНачала1 >= ДатаПроверки Тогда
						...
						СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПорядокРасчетаСреднегоЗаработка", Текст);
					КонецЕсли;
				КонецЕсли;
				...
				Если Не ЗначениеЗаполнено(Строка.ДатаИзмененияКодаПричиныНетрудоспособности)
					...
					СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаИзмененияКодаПричиныНетрудоспособности", Текст);
				КонецЕсли;
			...
			Если (Строка.УсловияИсчисленияКод1 = "46" Или Строка.УсловияИсчисленияКод2 = "46" Или Строка.УсловияИсчисленияКод3 = "46")
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаНачалаТрудовогоДоговораПолучателя", Текст);
			КонецЕсли;
		КонецЕсли;
		...
		Если (Строка.УсловияИсчисленияКод1 = "43" Или Строка.УсловияИсчисленияКод2 = "43" Или Строка.УсловияИсчисленияКод3 = "43")
			...
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ФинансированиеФедеральнымБюджетом", Текст);
		КонецЕсли;
		...
		Если ЭтоСанаторнаяПутевка(Строка) Тогда
			...
			Если Не ЗначениеЗаполнено(Строка.ДатаОкончанияПутевки) Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаОкончанияПутевки", Текст);
			КонецЕсли;
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОГРН_Санатория");
		КонецЕсли;
		...
		Если ЭтоУходЗаРодственниками(Строка) Тогда
			Если Не ЗначениеЗаполнено(Строка.ПоУходуФИО1) Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПоУходуФИО1", Текст);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Строка.ПоУходуВозрастЛет1 + Строка.ПоУходуВозрастМесяцев1) Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПоУходуВозрастЛет1", Текст);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Строка.ПоУходуРодственнаяСвязь1) Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПоУходуРодственнаяСвязь1", Текст);
			КонецЕсли;
			Если ЗначениеЗаполнено(Строка.ПоУходуФИО2) Тогда
				Если Не ЗначениеЗаполнено(Строка.ПоУходуВозрастЛет2 + Строка.ПоУходуВозрастМесяцев2) Тогда
					...
					СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПоУходуВозрастЛет2", Текст);
				КонецЕсли;
				Если Не ЗначениеЗаполнено(Строка.ПоУходуРодственнаяСвязь2) Тогда
					...
					СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПоУходуРодственнаяСвязь2", Текст);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоНарушениеРежима(Строка) Тогда
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаНарушенияРежима");
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "КодНарушенияРежима");
		КонецЕсли;
		
		Если ЭЛНКарантинПоКоронавирусу Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Строка.ОсвобождениеДатаНачала2) Тогда
			...
			Если ПериодЗаполнен И Строка.ОсвобождениеДатаОкончания1 + 86400 <> Строка.ОсвобождениеДатаНачала2 Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОсвобождениеДатаНачала2", Текст);
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Строка.ОсвобождениеДатаНачала3) Тогда
			...
			Если ПериодЗаполнен И Строка.ОсвобождениеДатаОкончания2 + 86400 <> Строка.ОсвобождениеДатаНачала3 Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОсвобождениеДатаНачала3", Текст);
			КонецЕсли;
			ПериодыНетрудоспособностиЗаполнены = ПериодыНетрудоспособностиЗаполнены И ПериодЗаполнен;
		КонецЕсли;
		...
		Если Строка.ДатаВыдачиЛисткаНетрудоспособности > Строка.ОсвобождениеДатаНачала1
			...
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаВыдачиЛисткаНетрудоспособности", Текст);
		КонецЕсли;
		...
		Если ЕстьОплатаЗаСчетФСС(Строка) Тогда
			...
			Если ПериодОплатыЗаполнен И ПериодыНетрудоспособностиЗаполнены Тогда
				Если Строка.ДатаНачалаОплаты < Строка.ОсвобождениеДатаНачала1 Тогда
					...
					СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаОкончанияОплаты", Текст);
				КонецЕсли;
		...
		Если ЕстьИзменениеСостоянияНетрудоспособного(Строка) Тогда
			Если Не ЗначениеЗаполнено(Строка.НовыйСтатусНетрудоспособного) Тогда
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "НовыйСтатусНетрудоспособного", Текст);
			ИначеЕсли Не ЗначениеЗаполнено(Строка.ДатаНовыйСтатусНетрудоспособного)
				...
				СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаНовыйСтатусНетрудоспособного", Текст);
			КонецЕсли;
		Иначе
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ПриступитьКРаботеС");
		КонецЕсли;
		
		Если ЭтоПродолжение(Строка) Тогда
			ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "НомерЛисткаПродолжения");
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокСведенийПоУходуЗаРебенком(Отказ)
	...
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.ФамилияРебенка) Тогда
			...
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ФамилияРебенка", Текст);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Строка.ИмяРебенка) Тогда
			Текст = НСтр("ru = 'Не заполнено имя ребенка.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ИмяРебенка", Текст);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Строка.ДатаРожденияРебенка) Тогда
			Текст = НСтр("ru = 'Не заполнена дата рождения ребенка.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаРожденияРебенка", Текст);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Строка.ОчередностьРожденияРебенка) Тогда
			Текст = НСтр("ru = 'Не заполнена очередность рождения ребенка.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОчередностьРожденияРебенка", Текст);
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Строка.ВидПодтверждающегоДокумента) Тогда
			...
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ВидПодтверждающегоДокумента", Текст);
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Строка.НаименованиеПодтверждающегоДокумента) Тогда
			...
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "НаименованиеПодтверждающегоДокумента", Текст);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Строка.ДатаДокумента) Тогда
			Текст = НСтр("ru = 'Не заполнена дата подтверждающего документа.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ДатаДокумента", Текст);
		КонецЕсли;
		...
		Если Строка.ВидПодтверждающегоДокумента = Перечисления.ВидыПодтверждающихДокументовОтпускаПоУходу.СвидетельствоОРождении
			...
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "СерияДокумента", Текст);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Строка.НомерДокумента) Тогда
			Текст = НСтр("ru = 'Не заполнен номер подтверждающего документа.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "НомерДокумента", Текст);
		КонецЕсли;
...
Функция ПроверитьПериодНетрудоспособности(Отказ, Строка, НомерПериода, ИмяТаблицы, Представление)
	...
	Если Не ЗначениеЗаполнено(Строка["ОсвобождениеФИОВрача" + НомерПериода]) Тогда
		...
		СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОсвобождениеФИОВрача1", Текст);
		ПериодЗаполнен = Ложь;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Строка["ОсвобождениеДолжностьВрача" + НомерПериода]) Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, Представление);
		СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, "ОсвобождениеДолжностьВрача1", Текст);
		ПериодЗаполнен = Ложь;
	КонецЕсли;
...
Функция ПроверитьПериод(Отказ, ИмяТаблицы, Строка, ПолеДаты1, ПолеДаты2, ПредставлениеПериодаВРодительномПадеже)
	Дата1 = Строка[ПолеДаты1];
	Дата2 = Строка[ПолеДаты2];
	Если Не ЗначениеЗаполнено(Дата1) Тогда
		...
		СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, ПолеДаты1, Текст);
		Возврат Ложь;
	ИначеЕсли Не ЗначениеЗаполнено(Дата2) Тогда
		...
		СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ЭтотОбъект, ИмяТаблицы, Строка, ПолеДаты2, Текст);
		Возврат Ложь;
