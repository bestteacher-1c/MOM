...
Процедура ОбработкаЗаполнения(ДокументОснование, СтандартнаяОбработка)
	...
	|	Увольнение.ВидДоговора КАК ВидДоговора,
	...
	Если ЭтоУвольнениеСписком Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Увольнение.ВидДоговора", "НЕОПРЕДЕЛЕНО");
	КонецЕсли;
	Запрос.Выполнить();
	...
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВременныхТаблиц, Истина, "ВидЗанятости,ГрафикРаботы");
	...
	|	Увольнение.Сотрудник КАК Сотрудник,
	|	ВЫБОР
	|		КОГДА Увольнение.ВидДоговора = НЕОПРЕДЕЛЕНО
	|			ТОГДА Увольнение.РасчетДенежногоСодержания
	|		КОГДА Увольнение.ВидДоговора В (&ДоговорыДенежногоСодержания)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	Если СотрудникиСоСреднимЗаработком.Количество() > 0 Тогда
		...
		СтрокаКомментария = НСтр("ru = '%1, период оплаты с %2 по %3, %4 %5, средний заработок на дату увольнения %6.'");
		СтрокаКомментарияСПособием = СтрокаКомментария + " " + НСтр("ru = 'Выходное пособие выплаченное при увольнении %7.'");
		...
		Пока Выборка.Следующий() Цикл
			...
			Комментарий = Комментарий + ?(ПервыйСотрудник, "", Символы.ПС) 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					?( Выборка.ВыходноеПособие > 0, СтрокаКомментарияСПособием, СтрокаКомментария),
					Выборка.ФизическоеЛицо,
					Формат(Выборка.НачалоПериодаОплаты, "ДЛФ=D"),
					Формат(Выборка.ОкончаниеПериодаОплаты, "ДЛФ=D"),
					КоличествоДнейЧасов,
					?(ЭтоСреднеЧасовойЗаработок, "час.", "дн."),
					СреднийЗаработок,
					Выборка.ВыходноеПособие);
