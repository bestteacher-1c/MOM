...
Функция СтатусПослеПередачиДанных(ДокументСсылка, Операция, СтатусОбработки) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаПродажа
		...
		СтатусыБазовыйПроцесс.Принят = Перечисления.СтатусыОбработкиОтгрузкиТоваровИСМП.Подтвержден;
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаАннулирование 
		...
		СтатусыБазовыйПроцесс.Принят = Перечисления.СтатусыОбработкиОтгрузкиТоваровИСМП.Аннулирован;
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.ОтгрузкаТоваровИСМП - Документ, по которому требуется рассчитать статус оформления.
...
Функция ОтгрузкаТоваровJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|	Шапка.Контрагент.ИНН                           КАК КонтрагентИНН,
	...
	Иначе
		ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(
				НСтр("ru = 'Не известный тип операции Уведомление об отгрузке маркированныхтоваров ИСМП: %1'"), Шапка.Операция));
	КонецЕсли;
	...
	ТелоЗапроса.Вставить("receiver_inn",             Шапка.КонтрагентИНН);
	ТелоЗапроса.Вставить("receiver",                 Шапка.КонтрагентПредставление);
	...
	ТелоЗапроса.Вставить("withdrawal_from_turnover", ВыведенИзОборота);
	...
	Для Каждого СтрокаТоваров Из ИсточникДанных Цикл
		...
		СтрокаТЧ.Вставить("product_tax",         Окр(СуммаНДСУпаковки * 100, 2));
		
		РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
			СтрокаТоваров.Штрихкод, Шапка.ВидПродукции);
