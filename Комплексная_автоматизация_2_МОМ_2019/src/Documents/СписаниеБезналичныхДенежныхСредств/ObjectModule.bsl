...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТ
	ПроверитьВедомостиНаВыплатуЗарплаты(ФлагОбменСБанками, ОшибкиЗаполнения, Отказ);
...
//++ НЕ УТ
Процедура ПроверитьВедомостиНаВыплатуЗарплаты(ФлагОбменСБанками, ОшибкиЗаполнения, Отказ)
	
	Если Не ДенежныеСредстваСервер.ОперацияПоЗарплате(ХозяйственнаяОперация)
	...
	Если Не КоличествоСтрок = МассивВедомостей.Количество() Тогда
		...
		Если ФлагОбменСБанками Тогда
			ДенежныеСредстваСервер.ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
		Иначе
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "РасшифровкаПлатежа",, Отказ);
		КонецЕсли;
	КонецЕсли; 
	...
	СтруктураПараметров.Вставить("ИсключитьДокументОплаты", Ссылка);
	
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета Тогда
		СтруктураПараметров.Вставить("Работник", ПодотчетноеЛицо);
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			Запрос.УстановитьПараметр("РегистрацияВНалоговомОргане",
				ЗарплатаКадры.РегистрацияВНалоговомОргане(Организация, ДатаПроведенияБанком));
		КонецЕсли;
