...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Основание") И ЗначениеЗаполнено(Параметры.Основание) Тогда
		...
		Если ТипЗнч(Параметры.Основание) = Тип("Структура") И Параметры.Основание.Свойство("МассивВедомостей") Тогда
			...
			ДанныеВедомостей = ДенежныеСредстваВызовСервера.ДанныеВедомостей(СтруктураПараметров);
...
Функция ПечатьРеестраПеречисленныхСуммНДФЛ(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Строка Из РеквизитыПлатежа Цикл
		...
		Если ЗначениеЗаполнено(Строка.НомерВходящегоДокумента) Тогда
			НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Строка.НомерВходящегоДокумента);
		Иначе
			НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Строка.Номер);
		КонецЕсли;
		...
		Строка.РеквизитыПлатежногоПоручения = ДенежныеСредстваСервер.РеквизитыПлатежногоПорученияПоДатеНомеру(
			Строка.ДатаПлатежа, НомерНаПечать);
