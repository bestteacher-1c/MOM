...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ВидыОперацийИСМП.ВводВОборотМаркировкаОстатков;
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.ТоварыGTIN.Видимость                     = ЭтоМаркировкаОстатков;
	...
	Элементы.Контрагент.Видимость       = ЭтоПроизводствоПоДоговору Или ЭтоТрансграничнаяТорговля;
	Элементы.ДатаПроизводства.Видимость = ЭтоПроизводствоПоДоговору
	...
	ИначеЕсли ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукция") Тогда
		...
		Элементы.ОперацияНанесения.Видимость = Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ")
		...
		Элементы.ТоварыИдентификаторПроисхожденияВЕТИС.Видимость            = ПрисутствуетОперацияНанесения;
		Элементы.ТоварыСрокГодности.Видимость                               = ПрисутствуетОперацияНанесения;
		Элементы.ШтрихкодыУпаковокИдентификаторПроисхожденияВЕТИС.Видимость = ПрисутствуетОперацияНанесения;
		...
		Если ПрисутствуетОперацияНанесения Тогда
			...
			Если ТипЗнч(Форма.Объект.ДокументОснование) = Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС") Тогда
				МассивТиповИдентификаторПроисхожденияВЕТИС.Добавить(Тип("СправочникСсылка.ЗаписиСкладскогоЖурналаВЕТИС"));
				ЗаголовокКолонкиВЕТИС = НСтр("ru = 'Запись складского журнала ВетИС'");
			ИначеЕсли ТипЗнч(Форма.Объект.ДокументОснование) = Тип("ДокументСсылка.ПроизводственнаяОперацияВЕТИС") Тогда
				МассивТиповИдентификаторПроисхожденияВЕТИС.Добавить(Тип("СправочникСсылка.ВетеринарноСопроводительныйДокументВЕТИС"));
				ЗаголовокКолонкиВЕТИС = НСтр("ru = 'ВСД ВетИС'");
			ИначеЕсли ЭтоПроизводствоВнеЕАЭС Или ЭтоТрансграничнаяТорговля Тогда
				МассивТиповИдентификаторПроисхожденияВЕТИС.Добавить(Тип("СправочникСсылка.ЗаписиСкладскогоЖурналаВЕТИС"));
				ЗаголовокКолонкиВЕТИС = НСтр("ru = 'Запись складского журнала ВетИС'");
			Иначе
				МассивТиповИдентификаторПроисхожденияВЕТИС.Добавить(Тип("СправочникСсылка.ВетеринарноСопроводительныйДокументВЕТИС"));
...
Процедура ПослеОбработкиШтрихкодовСервер(РезультатОбработкиШтрихкода, КэшированныеЗначения, ПараметрыСканирования)
	...
	ЭтоПотоковаяПечать = ЭтоШтрихкодНоменклатуры
		...
		И ТипЗнч(РезультатОбработкиШтрихкода.ДанныеШтрихкода) = Тип("Структура")
...
Процедура ИнициализироватьКэшСтроки()
	...
	ДанныеКешаСтроки.Вставить("ИдентификаторПроисхожденияВЕТИС");
	ДанныеКешаСтроки.Вставить("СрокГодности");
	ДанныеКешаСтроки.Вставить("GTIN");
...
Процедура РаспечататьНовыйКодМаркировки(РезультатОбработкиШтрихкода, КэшированныеЗначения, ПараметрыСканирования)
	...
	ДанныеШтрихкодаНоменклатура = РезультатОбработкиШтрихкода.ДанныеШтрихкода;
	...
	Если ДанныеВыбора.Номенклатура = ДанныеШтрихкодаНоменклатура.Номенклатура
		...
		Если СтруктураРезультата <> Неопределено Тогда
			...
			Если СтруктураРезультата.РезультатРезервирования.Количество() Тогда
				...
				СтруктураДанныхШтрихкода = Новый Структура("Штрихкод, Количество", СтрокаРезультат.КодМаркировки, 1);
				ШтрихкодированиеИСКлиентСервер.ЗакодироватьШтрихкодДанныхBase64(СтруктураДанныхШтрихкода);
				...
				ОбработатьДобавленныеИзмененныеСтроки(РезультатОбработки.ДобавленныеСтроки, РезультатОбработки.ИзмененныеСтроки, КэшированныеЗначения);
...
Процедура ЗаполнитьДоступныеОперации()
	...
	ЭтоТабачнаяПродукция = (Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табак"));
	...
	Если ЭтоТабачнаяПродукция Или ЭтоМолочнаяПродукция Тогда
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Объект.ВидПродукции, Ложь, Истина) Тогда
		...
		Если Не ЭтоМолочнаяПродукция Тогда
		...
		Если ИнтеграцияИСМП.ДоступностьМаркировкиОстактов(Объект.ВидПродукции) Тогда
...
Функция ОбработатьПолученныеДанныеТСДНаСервере(ШтрихкодыТСД, ПараметрыСканирования, КэшированныеЗначения)
	...
	Если Результат.ТребуетсяАвторизация
		Или Результат.ЕстьОшибкиВДереве
...
Процедура ЗаполнитьДанныеВЕТИСПоСтроке(Форма, ДобавленныеСтроки)
	...
	Для Каждого ТекущиеДанные Из ДобавленныеСтроки Цикл
		...
		Если ТекущиеДанные.СрокГодности <> СрокГодности Тогда
			ТекущиеДанные.СрокГодности = СрокГодности;
		КонецЕсли;
		
		Если ТекущиеДанные.СкоропортящаясяПродукцияВЕТИС <> Скоропортящаяся Тогда
...
Процедура ЗаполнитьСлужебныеРеквизитыВЕТИСВКоллекции()
	...
	|	ЕСТЬNULL(втТаблица.ИдентификаторыПроисхождения.СкоропортящаясяПродукция, ЛОЖЬ) КАК Скоропортящаяся
	|ИЗ
	|	втТаблица КАК втТаблица
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	Запрос.УстановитьПараметр("КоллекцияДанных", Объект.Товары.Выгрузить(, "НомерСтроки, ИдентификаторПроисхожденияВЕТИС"));
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Объект.Товары[Выборка.НомерСтроки - 1].СкоропортящаясяПродукцияВЕТИС = Выборка.Скоропортящаяся;
	КонецЦикла;
