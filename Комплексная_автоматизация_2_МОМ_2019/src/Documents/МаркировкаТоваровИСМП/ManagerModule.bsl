...
Функция ПоследовательностьОпераций(ДокументСсылка, ЛинейныйСписок = Ложь) Экспорт
	...
	Если ЛинейныйСписок
		Или Реквизиты.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак
...
Функция ВводВОборотJSON(ДокументСсылка, ДополнительныеПараметры, ПередатьОтчетОНанесении = Ложь)
		...
		ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотМаркировкаОстатков Тогда
			...
			Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
				...
				Иначе
					...
					ДанныеКодаМаркировки         = ШтрихкодированиеИСВызовСервера.РазобратьКодМаркировки(
			...
			Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
				...
				Если НайденныеСтроки.Количество() = 1 Тогда
					...
					СтрокаТЧ.Вставить(
						"ki",
						ШтрихкодированиеИСМП.КодМаркировкиДляПередачиИСМП(РезультатРазбора, ПараметрыНормализацииПрочее));
					СтрокаТЧ.Вставить("cost",      Формат(СтрокаТЧТовары.Цена,     "ЧДЦ=2; ЧРД=.; ЧГ=0;"));
					СтрокаТЧ.Вставить("vat_value", Формат(СтрокаТЧТовары.СуммаНДС, "ЧДЦ=2; ЧРД=.; ЧГ=0;"));
	...
	ИначеЕсли Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция
		...
		|	Т.ИдентификаторПроисхожденияВЕТИС                                 КАК ИдентификаторПроисхожденияВЕТИС,
		|	Т.ГоденДо                                                         КАК ГоденДо,
		|	Т.ИдентификаторПроисхожденияВЕТИС.Идентификатор                   КАК Идентификатор,
		|	Т.Скоропортящаяся                                                 КАК Скоропортящаяся,
		|	Т.КодМаркировки                                                   КАК КодМаркировки,
		...
		Пока ВыборкаПоИдентификатору.Следующий() Цикл
			...
			ИдентификаторПримененияСтрокой = Строка(ВыборкаПоИдентификатору.Идентификатор);
...
Функция АгрегацияJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	РеквизитыОрганизации      = ИнтеграцияИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(Шапка.Организация);
	ВключатьМолочнуюПродукцию = Шапка.Операция = Перечисления.ВидыОперацийИСМП.Агрегация;
	
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции, Ложь, ВключатьМолочнуюПродукцию) Тогда
	...
	ИначеЕсли Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция Тогда
		...
		Если Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак Тогда
			ПараметрыСканирования.ЗапрашиватьСтатусыМОТП = Ложь;
		ИначеЕсли Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукция Тогда
			ПараметрыСканирования.ЗапрашиватьСтатусыИСМП = Ложь;
		КонецЕсли;
...
Процедура ЗаполнениеДанныхОбАгрегацииВСУЗ(СтрокаДерева, ТелоЗапроса, ВидПродукции, ПараметрыНормализацииКМ)
	...
	Если ТипЗнч(СтрокаДерева) <> Тип("ДеревоЗначений") Тогда
		КодМаркировкиУпаковки = ШтрихкодированиеИССлужебный.НормализоватьКодМаркировки(
...
Процедура ЗаполнитьМаркируемуюПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка) Экспорт
	...
	|	ЕСТЬNULL(МаркировкаТоваровИСМПТовары.ИдентификаторПроисхожденияВЕТИС.СкоропортящаясяПродукция, ЛОЖЬ) КАК Скоропортящаяся,
...
Функция ДанныеИдентификаторовПроисхождения(ИдентификаторыПроисхождения) Экспорт
	...
	|			ТОГДА ИдентификаторПроисхожденияВЕТИС.СрокГодностиНачалоПериода
	|		ИНАЧЕ ИдентификаторПроисхожденияВЕТИС.СрокГодностиКонецПериода
	|	КОНЕЦ КАК СрокГодностиКонецПериода,
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеВЕТИС.Вставить("СрокГодности",    СрокГодностиСТочностьюЗаполнения);
