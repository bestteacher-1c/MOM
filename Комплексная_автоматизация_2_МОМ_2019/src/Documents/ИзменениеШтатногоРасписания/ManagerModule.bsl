...
Функция ПечатнаяФормаПриказа(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеДляПечати.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Если ЗначениеЗаполнено(ОбластьПодвал.Параметры.ФИОИсполнителя) Тогда
			РезультатСклонения = "";
			Если ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(ОбластьПодвал.Параметры.ФИОИсполнителя), 4, РезультатСклонения, ДанныеДляПечати.ПолИсполнителя, ДанныеДляПечати.РуководительКадровойСлужбы) Тогда
