...
Процедура ОтправитьОтчет(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанныеНаименованиеОтчета = "Расчет совокупного размера прибыли"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.расч.сов.разм.приб.") Тогда
			
			ПоказатьПредупреждение(, НСтр(
			"ru='Выгрузка, проверка выгрузки, отправка, проверка
			|в интернете формы расчета, утвержденной приказом
			|Минфина России от 28.05.2019 № 79н, будут
			|реализованы в будущих версиях программы'"));
			Возврат;
		КонецЕсли;
		...
		Если ВыполнитьСтандартнуюОтправкуОтчета Тогда
			...
			Если ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
				...
				ИЛИ ТекущиеДанныеНаименованиеОтчета = "Реестр по НДС: Декларации, таможенные декларации CN 23"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.реестр.НДС тамож.23")
				ИЛИ ТекущиеДанныеНаименованиеОтчета = "Реестр по НДС: Декларации на товары для экспресс-грузов"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.реестр.НДС экс.грузы") Тогда
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Уведомление об освобождении от НДС для участников ""Сколково"""
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ув.осв.НДС Сколково") Тогда
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Расчет совокупного размера прибыли" Тогда
		ПоказатьПредупреждение(, НСтр(
		"ru='Печать со штрихкодом PDF417 формы расчета,
		|утвержденной приказом Минфина России
		|от 28.05.2019 № 79н, будет реализована
...
Процедура ВыгрузкаРегламентированногоОтчета(Ссылка, ОрганизацияСсылка, НаименованиеОтчета, ВариантОтчета, ПредставлениеПериода);
	...
	Если НаименованиеОтчета = "Расчет совокупного размера прибыли"
		И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.расч.сов.разм.приб.") Тогда
...
Процедура ОнлайнПроверка(Закладка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если Представление = "Расчет совокупного размера прибыли"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.расч.сов.разм.приб.") Тогда
