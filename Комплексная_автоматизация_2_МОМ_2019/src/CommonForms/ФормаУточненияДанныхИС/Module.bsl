...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не (ТребуетсяУказаниеСерии И ЗначениеЗаполнено(Склад)) Тогда
...
Процедура ИдентификаторПроисхожденияВЕТИСОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если ТребуетсяУказаниеСерии И ЗначениеЗаполнено(Склад) И Не ЗначениеЗаполнено(Серия) Тогда
	...
	ПродукцияВЕТИС = СоответствиеНоменклатурыВЕТИС(Номенклатура, Характеристика, Серия);
	
	Отбор = Новый Структура;
	Отбор.Вставить("Продукция", ПродукцияВЕТИС);
	Если ЭтоКодМаркировкиСоСрокомГодности Тогда
		Отбор.Вставить("СкоропортящаясяПродукция", Скоропортящаяся);
	КонецЕсли;
	...
	Если ТипЗнч(ПараметрыСканирования.ТипИдентификатораПроисхожденияВЕТИС) = Тип("СправочникСсылка.ЗаписиСкладскогоЖурналаВЕТИС") Тогда
		ИмяФормыВыбора = "Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ФормаВыбора";
	Иначе
		ИмяФормыВыбора = "Справочник.ВетеринарноСопроводительныйДокументВЕТИС.ФормаВыбора";
	КонецЕсли;
	
	ОповещениеВыбора = Новый ОписаниеОповещения("ВыборИдентификатораПроисхожденияВЕТИСЗавершение", ЭтотОбъект);
...
Процедура ВывестиПоясняющийТекст()
	...
	Если Элементы.ИдентификаторПроисхожденияВЕТИС.СписокВыбора.Количество() > 0 Тогда
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Укажите ВСД.'"),, ЦветаСтиля.ПоясняющийТекст));
	КонецЕсли;
...
Процедура ЗаполнитьДанныеФормыПриСозданииНаСервере()
	...
	Если ПараметрыУказанияСерий = Неопределено И Параметры.ПараметрыСканирования.ПараметрыУказанияСерий <> Неопределено Тогда
		ПараметрыУказанияСерий = Параметры.ПараметрыСканирования.ПараметрыУказанияСерий;
	КонецЕсли;
	Склад                     = Параметры.Склад;
	Если Не ЗначениеЗаполнено(Склад) И ЗначениеЗаполнено(Параметры.ПараметрыСканирования.Склад) Тогда
	...
	УказатьИдентификаторВЕТИС = Параметры.УказатьИдентификаторВЕТИС;
	
	ЭтоКодМаркировкиСоСрокомГодности = УказатьИдентификаторВЕТИС И ЗначениеЗаполнено(Параметры.ГоденДо);
	Если ЭтоКодМаркировкиСоСрокомГодности Тогда
		ГоденДо         = Параметры.ГоденДо;
		Скоропортящаяся = Параметры.Скоропортящаяся;
	КонецЕсли;
	
	СоставКодаМаркировки = ШтрихкодированиеИССлужебный.РазобратьКодМаркировки(КодМаркировки, ПараметрыСканирования.ДопустимыеВидыПродукции);
...
Процедура ДобавитьИдентификаторыВЕТИСВСписокВыбора(Идентификаторы, РежимОткрытияСпискаВыбора = Ложь)
	...
	Если ПараметрыСканирования.ТипИдентификатораПроисхожденияВЕТИС = Тип("СправочникСсылка.ЗаписиСкладскогоЖурналаВЕТИС") Тогда
		ПредставлениеИдентификатораВЕТИС = НСтр("ru = 'Запись журнала'");
	Иначе
		ПредставлениеИдентификатораВЕТИС = НСтр("ru = 'ВСД'");
	КонецЕсли;
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ИдентификаторПроисхожденияВЕТИС.Ссылка                         КАК Ссылка,
	|	ИдентификаторПроисхожденияВЕТИС.Дата                           КАК Дата,
	...
	|ИЗ
	|	Справочник." + ИмяИдентификатораПроисхожденияВЕТИС + " КАК ИдентификаторПроисхожденияВЕТИС
	|ГДЕ
	|	ИдентификаторПроисхожденияВЕТИС.Ссылка В(&ИдентификаторыПроисхожденияВЕТИС)");
	Запрос.УстановитьПараметр("ИдентификаторыПроисхожденияВЕТИС", Идентификаторы);
	
	МодульИнтеграцияВЕТИСКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияВЕТИСКлиентСервер");
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		ПредставлениеПериода = МодульИнтеграцияВЕТИСКлиентСервер.ПредставлениеПериодаВЕТИС(
			Выборка.ТочностьЗаполнения, Выборка.НачалоПериода, Выборка.КонецПериода);
		
		ПредставлениеСсылкиШаблон = НСтр("ru = '%1 от %2 (%3)'");
		
		ПредставлениеСсылки = СтрШаблон(ПредставлениеСсылкиШаблон, ПредставлениеИдентификатораВЕТИС, Формат(Выборка.Дата, "ДФ=dd.MM.yyyy"), ПредставлениеПериода);
		
		Элементы.ИдентификаторПроисхожденияВЕТИС.СписокВыбора.Вставить(0, Выборка.Ссылка, ПредставлениеСсылки);
...
Процедура ЗаполнитьДанныеДляПечатиЭтикеток()
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, Истина) Тогда
		...
		Если ДоступныеШаблоны.Количество() = 1 Тогда
			ШаблонКодаМаркировки = ДоступныеШаблоны.Получить(0).Значение;
		ИначеЕсли ДоступныеШаблоны.Количество() > 1 Тогда
			Элементы.ШаблонКодаМаркировки.РежимВыбораИзСписка = Истина;
		КонецЕсли;
		
		Если ЭтоКодМаркировкиСоСрокомГодности Тогда
			Если Скоропортящаяся Тогда
				ШаблонКодаМаркировки = Перечисления.ШаблоныКодовМаркировкиСУЗ.СкоропортящаясяМолочнаяПродукция;
			Иначе
				ШаблонКодаМаркировки = Перечисления.ШаблоныКодовМаркировкиСУЗ.МолочнаяПродукция;
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьЗаголовокЭлементаИдентификаторВЕТИС()
	...
	Если ПараметрыСканирования.ТипИдентификатораПроисхожденияВЕТИС = Тип("СправочникСсылка.ЗаписиСкладскогоЖурналаВЕТИС") Тогда
		Элементы.ИдентификаторПроисхожденияВЕТИС.Заголовок = НСтр("ru = 'Запись журнала'");
	Иначе
		Элементы.ИдентификаторПроисхожденияВЕТИС.Заголовок = НСтр("ru = 'ВСД'");
	КонецЕсли;
...
Функция ДанныеИдентификаторовПроисхожденияВЕТИС(ИдентификаторыПроисхождения)
	Возврат Документы.МаркировкаТоваровИСМП.ДанныеИдентификаторовПроисхождения(ИдентификаторыПроисхождения);
КонецФункции

&НаСервереБезКонтекста
Функция СоответствиеНоменклатурыВЕТИС(Номенклатура, Характеристика, Серия)
	...
	УсловиеНаХарактеристику = "ИСТИНА";
	Если ЗначениеЗаполнено(Характеристика) Тогда
		УсловиеНаХарактеристику = "СоответствиеНоменклатурыВЕТИС.Характеристика = &Характеристика";
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = СтрШаблон(ТекстЗапроса, УсловиеНаХарактеристику, УсловиеНаСерию);
...
Функция ЗакрытьПослеСканированияШтрихкодаНоменклатуры()
	...
	Если РежимПечатиЭтикеток Тогда
		Возврат Ложь;
	ИначеЕсли УказатьИдентификаторВЕТИС Тогда
		Возврат Ложь;
	...
	ИначеЕсли Не ЗначениеЗаполнено(Номенклатура) Тогда
		Возврат Ложь;
	ИначеЕсли Не ЗначениеЗаполнено(Характеристика) И ХарактеристикиИспользуются Тогда
		Возврат Ложь;
	ИначеЕсли Не ЗначениеЗаполнено(Серия) И ТребуетсяУказаниеСерии Тогда
		Возврат Ложь;
	КонецЕсли;
